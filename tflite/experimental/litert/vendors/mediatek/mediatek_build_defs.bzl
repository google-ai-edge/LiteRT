...
load("//tflite/experimental/litert/build_common:litert_build_defs.bzl", "append_rule_kwargs", "litert_lib", "make_rpaths")

_MTK_STD_LIBS_HOST = [
    # copybara:uncomment_begin(google-only)
    # "@org_tensorflow//third_party/neuro_pilot:latest/host/lib/libc++.so.1",
    # "@org_tensorflow//third_party/neuro_pilot:latest/host/lib/libstdc++.so.6",
    # copybara:uncomment_end
]  # @unused

_MTK_NEURON_ADAPTER_SO = [
    # copybara:uncomment_begin(google-only)
    # "@org_tensorflow//third_party/neuro_pilot:latest/host/lib/libneuron_adapter.so",
    # copybara:uncomment_end
]
...
def _litert_with_mtk_base(
    ...
    append_rule_kwargs(
        litert_rule_kwargs,
        data = select({
            "@org_tensorflow//tensorflow:linux_x86_64": _MTK_NEURON_ADAPTER_SO,
            "//conditions:default": [],
        }),
        linkopts = select({
