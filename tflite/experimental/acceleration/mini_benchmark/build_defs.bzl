...
load("//tflite/core/shims:cc_library_with_tflite.bzl", "add_suffix")
load("//tflite/experimental/acceleration/mini_benchmark:special_rules.bzl", "libjpeg_handle_deps")
...
def embedded_binary(name, binary, array_variable_name, testonly = False, exec_properties = None):
    ...
    native.genrule(
        ...
        cmd = """
      $(location //tflite/experimental/acceleration/compatibility:convert_binary_to_cc_source) \
      ...
      """ % (binary, h_name, cc_name, array_variable_name),
        tools = ["//tflite/experimental/acceleration/compatibility:convert_binary_to_cc_source"],
        testonly = testonly,
    )
...
def validation_model(
    ...
    native.genrule(
        ...
        srcs = [
            main_model,
            jpegs,
            "@org_tensorflow//tensorflow/compiler/mlir/lite/schema:schema.fbs",
            metrics_model,
        ],
        outs = [name + ".tflite"],
        cmd = """
          JPEGS='$(locations %s)'
          JPEGS=$${JPEGS// /,}
          $(location //tflite/experimental/acceleration/mini_benchmark/model_modifier:embedder_cmdline) \
              --schema=$(location //third_party/tensorflow/compiler/mlir/lite/schema:schema.fbs) \
          ...
          $(location //tflite/experimental/acceleration/mini_benchmark:copy_associated_files) \
              '$(@D)/%s.tflite.tmp' \
              $(location %s) \
              $(location %s.tflite)
          rm '$(@D)/%s.tflite.tmp'
        ...
        tools = [
            "//tflite/experimental/acceleration/mini_benchmark/model_modifier:embedder_cmdline",
            "//tflite/experimental/acceleration/mini_benchmark:copy_associated_files",
        ],
    )
...
def validation_test(name, validation_model, tags = [], copts = [], deps = []):
    ...
    native.cc_test(
        name = name,
        srcs = ["//tflite/experimental/acceleration/mini_benchmark:model_validation_test.cc"],
        ...
        deps = deps + [
            ...
            "//tflite/experimental/acceleration/mini_benchmark:big_little_affinity",
            "//tflite/experimental/acceleration/mini_benchmark:status_codes",
            "//tflite/experimental/acceleration/mini_benchmark:validator",
...
def cc_library_with_forced_in_process_benchmark_variant(
    ...
    native.cc_library(
        ...
        deps = deps + in_process_deps + _concat([select(map) for map in non_in_process_deps_selects]) + [
            clean_dep("//tflite/experimental/acceleration/mini_benchmark:tflite_acceleration_in_process_default"),
        ],
        **kwargs
    )
    ...
    native.cc_library(
        ...
        deps = deps + in_process_deps_renamed + forced_in_process_deps + [
            clean_dep("//tflite/experimental/acceleration/mini_benchmark:tflite_acceleration_in_process_enable"),
        ],
        **kwargs
    )
