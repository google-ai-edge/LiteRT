...
from tflite.testing import _pywrap_string_util
from tflite.testing import generate_examples_report as report_lib
...
def make_zip_of_tests(options,
  ...
  if not is_expected_failures and not (
      options.make_edgetpu_tests or options.make_tf_ptq_tests
  ):
    raise RuntimeError(
        (
            "Expected TF to fail %s times while generating '%s', "
