...
RunResult RunArgMinMaxCase(const ArgMinMaxCase& test_case) {
  ...
  const auto axis_tensor = CreateTensor(
      builder, axis_shape, test_case.axis_type, test_case.dynamic_axis ? 0 : 1);
  const auto output_tensor =
      CreateTensor(builder, output_shape, test_case.output_type);

  const BuiltinOperator op =
      test_case.is_arg_max ? BuiltinOperator_ARG_MAX : BuiltinOperator_ARG_MIN;
