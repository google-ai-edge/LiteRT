...
StablehloOptions MakeStablehloOptions(const StablehloPadCase& test_case) {
  ...
  options.low =
      fuzzing::MaterializeValues(test_case.edge_padding_low, option_size);
  options.high =
      fuzzing::MaterializeValues(test_case.edge_padding_high, option_size);
  options.interior =
      fuzzing::MaterializeValues(test_case.interior_padding, option_size);
  if (test_case.options_kind == StablehloOptionsKind::kInconsistent) {
    options.interior.push_back(
        test_case.interior_padding.empty() ? 0 : test_case.interior_padding[0]);
  }
  options.has_low = test_case.options_kind != StablehloOptionsKind::kMissingLow;
  options.has_high =
...
bool StablehloPadOutputIsWithinFuzzerBudget(const StablehloPadCase& test_case) {
  ...
  for (size_t i = 0; i < rank; ++i) {
    if (!fuzzing::CheckedMultiply(output_elements,
                                  static_cast<size_t>(output_shape[i]),
                                  &output_elements)) {
      return false;
    }
  }
  ...
  if (!fuzzing::CheckedMultiply(output_elements,
      ...
      output_bytes > static_cast<size_t>(std::numeric_limits<int64_t>::max())) {
    return false;
  }
  ...
  for (int i = static_cast<int>(rank) - 2; i >= 0; --i) {
    if (!fuzzing::CheckedMulInt64(output_dimension_sizes[i], interior_step[i],
                                  &output_strides[i])) {
      return false;
    }
  }
  ...
  for (size_t i = 0; i < rank; ++i) {
    ...
    if (!fuzzing::CheckedMulInt64(
        ...
        !fuzzing::CheckedAddInt64(output_offset, offset_term, &output_offset)) {
      return false;
    }
  }
  ...
  for (int i = static_cast<int>(rank) - 1; i >= 1; --i) {
    if (!fuzzing::CheckedMulInt64(test_case.input_shape[i], input_strides[i],
                                  &input_strides[i - 1])) {
      return false;
    }
  }
  ...
  for (size_t i = 0; i < rank; ++i) {
    ...
    if (!DivNegRoundAwayOrZero(OptionValueForDimension(options.low, i),
        ...
        !fuzzing::CheckedAddInt64(test_case.input_shape[i], low_crop,
                                  &input_shape[i]) ||
        !fuzzing::CheckedAddInt64(input_shape[i], high_crop, &input_shape[i])) {
      return false;
    }
    if (input_shape[i] <= 0) {
...
StablehloPadCase MakeStablehloPadStressCase(
  ...
  for (int32_t i = 0; i < rank; ++i) {
    ...
    int64_t target_output_dim = spec.target_output_dims[static_cast<size_t>(i)];
...
fuzzing::RunResult RunStablehloPadCase(const StablehloPadCase& test_case) {
  ...
  const auto padding_value_tensor =
      CreateTensor(builder, builder.CreateVector(padding_value_shape),
  ...
  if (options.has_builtin_options) {
    const auto low_offset =
        options.has_low ? builder.CreateVector(options.low)
                        : flatbuffers::Offset<flatbuffers::Vector<int64_t>>();
    const auto high_offset =
        options.has_high ? builder.CreateVector(options.high)
                         : flatbuffers::Offset<flatbuffers::Vector<int64_t>>();
    const auto interior_offset =
        options.has_interior
            ? builder.CreateVector(options.interior)
            : flatbuffers::Offset<flatbuffers::Vector<int64_t>>();
    builtin_options_2 = CreateStablehloPadOptions(builder, low_offset,
                                                  high_offset, interior_offset)
                            .Union();
