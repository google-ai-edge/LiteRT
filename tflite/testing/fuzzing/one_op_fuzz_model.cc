...
#include "tflite/testing/fuzzing/fuzzer_quota_allocator.h"
...
flatbuffers::Offset<OperatorCode> MakeOperatorCode(
  ...
  if (static_cast<int>(op) >
    ...
    return CreateOperatorCode(*builder,
...
RunResult BuildAndRunOneOpModel(flatbuffers::FlatBufferBuilder* builder,
  ...
  const auto opcode = MakeOperatorCode(builder, model_spec.builtin_operator,
                                       model_spec.version);
  ...
  const auto subgraph =
      CreateSubGraph(*builder, builder->CreateVector(model_spec.tensors),
  ...
  const auto model = CreateModel(*builder, TFLITE_SCHEMA_VERSION,
                                 builder->CreateVector(&opcode, 1),
