 ...
 public:
  ...
  static TfLiteStatus VisitStridedSliceNode(
    ...
    for (size_t i = 0; i < num_dims; i++) {
      ...
      const size_t size = ends[i] <= 0 ? ends[i] + extent + offset : ends[i];
      // TF_LITE_MAYBE_KERNEL_LOG(
      //     logging_context,
      //     "DEL: i %d begins %d->%d ends %d->%d extent %d offset %d size %d",
      //     (int)i, (int)begin_data[i], (int)begins[i], (int)end_data[i],
      //     (int)ends[i], (int)input_tensor.dims->data[i], (int)offset,
      //     (int)size);
    ...
    if (subgraph != nullptr) {
      const xnn_status status = xnn_define_static_slice_v3(
          subgraph, num_dims, begins.data(), ends.data(),
