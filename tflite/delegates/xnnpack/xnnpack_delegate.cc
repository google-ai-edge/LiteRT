 ...
 public:
  ...
  static TfLiteStatus VisitStridedSliceNode(
    ...
    for (size_t i = 0; i < num_dims; i++) {
      ...
      // if (begin_data[i] < 0 || end_data[i] < 0 ||
      //     ((params->end_mask >> i) & 1)) {
      //   TF_LITE_MAYBE_KERNEL_LOG(
      //       logging_context,
      //       "zzzzzz bit %zu begin %d->%zu end %d->%zu size %zu masks %x %x "
      //       "input_shape->data[i] %d",
      //       i, begin_data[i], begins[i], end_data[i], ends[i], sizes[i],
