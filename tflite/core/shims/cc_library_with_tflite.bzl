    ...
    if generate_opaque_delegate_target:
        ...
        native.cc_library(
            ...
            deps = deps + tflite_deps_renamed + _concat([select(map) for map in tflite_deps_selects_renamed]) + [
                clean_dep("//tflite/core/shims:tflite_use_opaque_delegate"),
            ],
            **kwargs
        )
...
def custom_c_library_with_tflite(
    ...
    if experimental:
        hdrs = [
            clean_dep("//tflite/core/shims:c/c_api.h"),
            clean_dep("//tflite/core/shims:c/c_api_experimental.h"),
            clean_dep("//tflite/core/shims:c/c_api_opaque.h"),
        ]
    else:
        hdrs = [
            clean_dep("//tflite/core/shims:c/c_api.h"),
