...
TEST(BasicInterpreter, RejectNegativeTensorDimensions) {
  ...
  EXPECT_EQ(interpreter.SetTensorParametersReadWrite(0, kTfLiteString, "",
                                                     {2, -1}, quantization),
            kTfLiteError);
  EXPECT_EQ(
      interpreter.SetTensorParametersReadOnly(1, kTfLiteString, "", {2, -1},
                                              quantization, data, sizeof(data)),
      kTfLiteError);
}
