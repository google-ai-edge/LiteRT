...
load("@org_tensorflow//third_party/bazel_rules/rules_cc/cc:cc_test.bzl", "cc_test")
load("@org_tensorflow//third_party/bazel_rules/rules_cc/cc:cc_library.bzl", "cc_library")
load("@org_tensorflow//third_party/bazel_rules/rules_cc/cc:cc_binary.bzl", "cc_binary")
...
def tflite_copts():
    ...
    copts = [
        ...
        clean_dep("//tflite:tflite_with_xnnpack_explicit_false"): ["-DTFLITE_WITHOUT_XNNPACK"],
        "//conditions:default": [],
    }) + select({
        clean_dep("//tflite:tensorflow_profiler_config"): ["-DTF_LITE_TENSORFLOW_PROFILER"],
        "//conditions:default": [],
    }) + select({
        clean_dep("//tflite/delegates:tflite_debug_delegate"): ["-DTFLITE_DEBUG_DELEGATE"],
        "//conditions:default": [],
    }) + select({
        clean_dep("//tflite:tflite_mmap_disabled"): ["-DTFLITE_MMAP_DISABLED"],
        "//conditions:default": [],
    })
...
EXPORTED_SYMBOLS = clean_dep("//tflite/java/src/main/native:exported_symbols.lds")
LINKER_SCRIPT = clean_dep("//tflite/java/src/main/native:version_script.lds")
...
def tflite_symbol_opts():
    ...
    return select({
        clean_dep("@org_tensorflow//tensorflow:debug"): [],
        clean_dep("//tflite:tflite_keep_symbols"): [],
        "//conditions:default": [
...
def tflite_to_json(name, src, out):
    ...
    schema = "//tflite/schema:schema.fbs"
    native.genrule(
        name = name,
        srcs = [schema, src],
        outs = [out],
...
def json_to_tflite(name, src, out):
    ...
    flatc = "@flatbuffers//:flatc"
    schema = "//tflite/schema:schema_fbs"
    native.genrule(
        name = name,
        srcs = [schema, src],
        outs = [out],
    ...
    attrs = {
        ...
        "_generate_op_registrations": attr.label(
            ...
            default = Label(clean_dep(
                "//tflite/tools:generate_op_registrations",
            )),
            cfg = "exec",
        ),
    },
...
def flex_dep(target_op_sets):
    if "SELECT_TF_OPS" in target_op_sets:
        return ["//tflite/delegates/flex:delegate"]
    else:
        return []
...
def gen_model_coverage_test(
    ...
    for target_op_sets in ["TFLITE_BUILTINS", "TFLITE_BUILTINS,SELECT_TF_OPS", "SELECT_TF_OPS"]:
        ...
        full_deps = [
            ...
            "//tflite/testing/model_coverage:model_coverage_lib",
            "//tflite/python:lite",
...
def tflite_custom_cc_library(
    ...
    if models:
        ...
        real_srcs.append("//tflite:create_op_resolver_with_selected_ops.cc")
    else:
        # Support all operators if `models` not specified.
        real_deps.append("//tflite:create_op_resolver_with_builtin_ops")

    if experimental:
        framework = "//tflite:framework_experimental"
    else:
        framework = "//tflite:framework_stable"
    cc_library(
        name = name,
        srcs = real_srcs,
        hdrs = [
        ...
        deps = depset([
            ...
            "//tflite/core:private_create_op_resolver_header",
        ] + real_deps),
        visibility = visibility,
        **kwargs
    )
...
def tflite_custom_android_library(
    ...
    if include_nnapi_delegate:
        delegate_deps.append("//tflite/delegates/nnapi/java/src/main/native")
    if include_xnnpack_delegate:
        delegate_deps.append("//tflite/delegates/xnnpack:xnnpack_delegate")

    if experimental:
        native_framework_only = "//tflite/java/src/main/native:native_experimental_framework_only"
    else:
        native_framework_only = "//tflite/java/src/main/native:native_stable_framework_only"
    ...
    if experimental:
        java_srcs = "//tflite/java:java_srcs"
    else:
        java_srcs = "//tflite/java:java_stable_srcs"

    android_library(
        name = name,
        manifest = "//tflite/java:AndroidManifest.xml",
        srcs = [java_srcs],
        deps = [
            ":%s_jni" % name,
...
def tflite_custom_c_library(
    ...
    """
    op_resolver_deps = "//tflite:create_op_resolver_with_builtin_ops"
    if models:
        gen_selected_ops(
            name = "%s_registration" % name,
            model = models,
        ...
        cc_library(
            ...
            hdrs = ["//tflite:create_op_resolver.h"],
            copts = tflite_copts(),
            deps = [
                "//tflite/core:private_create_op_resolver_header",
                "//tflite:create_op_resolver_with_selected_ops",
                "//tflite:op_resolver",
                framework,
                "//tflite/kernels:builtin_ops",
            ],
    ...
    if experimental:
        hdrs = [
            "//tflite/c:c_api.h",
            "//tflite/c:c_api_experimental.h",
            "//tflite/c:c_api_opaque.h",
        ]
        deps = [
            "//tflite/c:c_api_experimental_without_op_resolver_without_alwayslink",
            "//tflite/core/c:private_c_api_experimental_without_op_resolver_without_alwayslink",
            "//tflite/c:c_api_opaque_without_op_resolver_without_alwayslink",
            "//tflite/core/c:private_c_api_opaque_without_op_resolver_without_alwayslink",
        ]
    else:
        hdrs = [
            "//tflite/c:c_api.h",
    ...
    cc_library(
        ...
        deps = [
            op_resolver_deps,
            "//tflite:builtin_ops",
            "//tflite/c:c_api_without_op_resolver_without_alwayslink",
            # TODO(bekzhan): Remove this dependency after we move c_api_opaque.h to tflite/core/.
            "//tflite/core/c:private_c_api_types",
            "//tflite/core/c:private_c_api_without_op_resolver_without_alwayslink",
