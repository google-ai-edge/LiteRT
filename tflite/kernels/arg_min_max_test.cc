...
class ArgBaseOpModel : public SingleOpModel {
 public:
  ArgBaseOpModel(TensorType input_type, int64_t axis_value, TensorType axis_type,
    ...
    if (constant_axis) {
      if (axis_type == TensorType_INT64) {
        ...
        axis_ = AddConstInput(axis_type, {static_cast<int32_t>(axis_value)},
                              {1});
      }
    } else {
...
TEST(ArgMinMaxOpEdgeTest, ZeroOutputWithHugeNonReducedDimReturnsOk) {
  ArgMaxOpModel model({0, std::numeric_limits<int>::max()}, TensorType_INT32,
                      1, TensorType_INT32, /*constant_axis=*/false,
                      TensorType_INT32);
  model.PopulateAxis();
