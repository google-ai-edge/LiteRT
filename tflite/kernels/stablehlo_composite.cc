...
#include <sys/sdt.h>
#define LITERT_ODML_COMPOSITE_USDT_OP_BEGIN(op_id, seq, work) \
  STAP_PROBE3(litert_odml_composite, op_begin, op_id, seq, work)
#define LITERT_ODML_COMPOSITE_USDT_OP_END(op_id, seq, work) \
  STAP_PROBE3(litert_odml_composite, op_end, op_id, seq, work)
#else
#define LITERT_ODML_COMPOSITE_USDT_OP_BEGIN(op_id, seq, work)
#define LITERT_ODML_COMPOSITE_USDT_OP_END(op_id, seq, work)
#endif
...
enum class TraceOpId : int32_t {
  kCausalConvWithState1d = 1,
  kRecurrentLinearAttention = 2,
  kSelectiveStateSpace = 3,
};

class ScopedCompositeTrace {
 public:
  ScopedCompositeTrace(TraceOpId op_id, int64_t seq, int64_t work)
      : op_id_(static_cast<int32_t>(op_id)), seq_(seq), work_(work) {
    LITERT_ODML_COMPOSITE_USDT_OP_BEGIN(op_id_, seq_, work_);
  }

  ~ScopedCompositeTrace() {
    LITERT_ODML_COMPOSITE_USDT_OP_END(op_id_, seq_, work_);
  }
...
struct CausalConvWithState1dFloatTask : cpu_backend_threadpool::Task {
  CausalConvWithState1dFloatTask(
      const float* input_data, const float* weight_data,
      const float* bias_data, const float* past_data, float* output_data,
      int job_start, int job_end, int seq, int channels, int kernel,
      int state_len, bool weight_channel_major, State::Activation activation)
  ...
  void Run() override {
    ...
    for (int job = job_start; job < job_end; ++job) {
      ...
      if (kernel == 4) {
        const float x0 = t >= 3 ? batch_input[(t - 3) * channels + c]
                                : PastValue(b, t, c);
...
TfLiteStatus EvalCausalConvWithState1d(TfLiteContext* context,
  ...
  const ScopedCompositeTrace trace(
      TraceOpId::kCausalConvWithState1d, seq,
      static_cast<int64_t>(batch) * seq * channels * kernel);
...
struct RecurrentLinearAttentionFloatTask : cpu_backend_threadpool::Task {
  ...
  void Run() override {
    ...
    std::vector<float> output_accumulator(
        std::max(1, q_heads_per_kv_head) * value_dim);
            ...
            if (has_headwise_decay) {
              if (!fuse_headwise_decay) {
                const float decay_value =
                    std::exp(decay_data[ScalarPerHeadOffset(decay_last_dim, b,
                                                            t, h)]);
...
TfLiteStatus EvalRecurrentLinearAttention(TfLiteContext* context,
  ...
  const ScopedCompositeTrace trace(
      TraceOpId::kRecurrentLinearAttention, seq,
      static_cast<int64_t>(batch) * seq * kv_heads * key_dim * value_dim);
  ...
  if (all_float) {
    ...
    for (int i = 0; i < thread_count; ++i) {
      ...
      tasks.emplace_back(
          query_data, key_data, value_data, decay_data, beta_data, output_data,
          state_data, job_start, job_end, seq, kv_heads, q_heads,
          q_heads_per_kv_head, key_dim, value_dim, chunk_size, decay_last_dim,
          beta == nullptr ? 0 : SizeOfDimension(beta, 2), scale, has_gate,
          has_delta, has_headwise_decay);
      job_start = job_end;
    }
...
TfLiteStatus EvalSelectiveStateSpace(TfLiteContext* context, TfLiteNode* node) {
  ...
  const int state_dim = LastDim(b_tensor);
  const ScopedCompositeTrace trace(
      TraceOpId::kSelectiveStateSpace, seq,
      static_cast<int64_t>(batch) * seq * heads * head_dim * state_dim);
