...
TfLiteStatus GetSizeImpl(TfLiteContext* context, T start, T limit, T delta,
  ...
  const UnsignedT range_size = distance == 0 ? 0 : ((distance - 1) / step) + 1;
  TF_LITE_ENSURE(context, range_size <= static_cast<UnsignedT>(
...
TfLiteStatus GetSizeImpl(TfLiteContext* context, T start, T limit, T delta,
  ...
  const T range_size = std::ceil(std::abs((limit - start) / delta));
...
TfLiteStatus GetSize(TfLiteContext* context, T start, T limit, T delta,
  ...
  return GetSizeImpl(context, start, limit, delta, size, std::is_integral<T>());
}
