...
using ElementCountLimit = ::tflite::kernel_utils::ElementCountLimit;
...
TfLiteStatus ValidateTensorData(
    TfLiteContext* absl_nonnull context, const TfLiteTensor& tensor,
    ElementCountLimit limit = ElementCountLimit::kSizeT) {
  return kernel_utils::ValidateTensorElementsAndData(context, tensor, "Reduce",
                                                     limit);
}
...
TfLiteStatus CreateOpContext(TfLiteContext* absl_nonnull context,
                             TfLiteNode* absl_nonnull node,
                             OpContext& op_context) {
  TF_LITE_ENSURE_OK(context, ValidateNode(context, node));
  op_context.params =
      reinterpret_cast<TfLiteReducerParams*>(node->builtin_data);
  TF_LITE_ENSURE_OK(context, GetInputSafe(context, node, 0, &op_context.input));
  TF_LITE_ENSURE_OK(context, GetInputSafe(context, node, 1, &op_context.axis));
  TF_LITE_ENSURE_OK(context,
                    GetOutputSafe(context, node, 0, &op_context.output));
  TF_LITE_ENSURE_OK(context, ValidateTensorShape(context, *op_context.input));
  TF_LITE_ENSURE_OK(context, ValidateTensorShape(context, *op_context.axis));
  return kTfLiteOk;
}
...
TfLiteStatus GetFlatSizeAsInt(TfLiteContext* absl_nonnull context,
                              const RuntimeShape& shape, int& flat_size) {
  TF_LITE_ENSURE_MSG(
      context,
      CheckedNumElements(
...
// Resizes the temp tensor that stores resolved axis to a known axis count.
TfLiteStatus ResizeTempAxis(TfLiteContext* absl_nonnull context, int axis_count,
                            TfLiteTensor& resolved_axis) {
  TfLiteIntArrayUniquePtr axis_size(TfLiteIntArrayCreate(1),
                                    TfLiteIntArrayFree);
  TF_LITE_ENSURE(context, axis_size != nullptr);
  axis_size->data[0] = axis_count;
  return context->ResizeTensor(context, &resolved_axis, axis_size.release());
}

// Validates the axis tensor and resizes the temp tensor that stores resolved
// axis.
TfLiteStatus ResizeTempAxis(TfLiteContext* absl_nonnull context,
  ...
  TF_LITE_ENSURE_OK(context,
                    kernel_utils::CheckedTensorElementsAndData(
                        context, *op_context.axis, "Reduce", axis_count));
  return ResizeTempAxis(context, axis_count, resolved_axis);
}
...
TfLiteStatus Mean(TfLiteContext* absl_nonnull context,
                  const OpContext& op_context, int num_axis, int* temp_index,
...
TfLiteStatus QuantizedMeanOrSum(TfLiteContext* absl_nonnull context,
                                const OpContext& op_context,
                                const OpData& op_data, int num_axis,
...
TfLiteStatus EvalQuantizedMean(TfLiteContext* absl_nonnull context,
                               ...
                               const OpData& data, TfLiteTensor& temp_index,
                               TfLiteTensor& resolved_axis,
                               TfLiteTensor& temp_sum) {
...
TfLiteStatus EvalMean(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_OK(context,
                    kernel_utils::CheckedTensorElementsAndData(
                        context, *op_context.axis, "Reduce", num_axis));
  TfLiteTensor* temp_index;
  TF_LITE_ENSURE_OK(context,
  ...
  if (IsDynamicTensor(op_context.output)) {
    TF_LITE_ENSURE_OK(context,
                      ResizeTempAxis(context, num_axis, *resolved_axis));
    TF_LITE_ENSURE_OK(context, ResizeOutputTensor(context, op_context));
  ...
  switch (op_context.input->type) {
    case kTfLiteFloat32: {
      TfLiteStatus mean_status = Mean<float, float, kernel_type>(
          context, op_context, num_axis, GetTensorData<int>(temp_index),
...
TfLiteStatus ReduceAllDims(const T* input_data,
  ...
  if (thread_count == 1) {
    output_data = num_elems > 0 ? input_data[0] : init_value;
    for (int i = 1; i < num_elems; ++i) {
      output_data = reducer(output_data, input_data[i]);
    }
    return kTfLiteOk;
  }
  ...
  output_data = data[0].output;
  for (size_t i = 1; i < data.size(); ++i) {
    output_data = reducer(output_data, data[i].output);
  }
  return kTfLiteOk;
}
...
TfLiteStatus EvalType(TfLiteContext* absl_nonnull context,
                      TfLiteNode* absl_nonnull node, OpContext& op_context,
  ...
  if (kernel_type == kReference) {
    ...
    switch (reduce_type) {
      case kSum:
        reducer = [](const T current, const T in) -> T {
          return AddTensorValuesWithExpectedOverflow<T>(current, in);
        };
        init_value = T(0);
        break;
      case kProd:
        ...
        reducer = [](const T current, const T in) -> T {
          return MulTensorValuesWithExpectedOverflow<T>(current, in);
        };
        break;
      case kMax:
    ...
    TF_LITE_ENSURE(
        ...
        optimized_ops::ReduceGeneric<T>(
            ...
            GetTensorData<T>(op_context.output), op_context.output->dims->data,
            op_context.output->dims->size, GetTensorData<int>(op_context.axis),
            num_axis, GetTensorData<int>(resolved_axis),
...
TfLiteStatus EvalGeneric(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (op_context.input->type) {
    case kTfLiteFloat32:
      ...
      break;
    case kTfLiteInt32:
      return EvalType<int, kernel_type>(context, node, op_context, reduce_type);
      break;
    case kTfLiteInt64:
