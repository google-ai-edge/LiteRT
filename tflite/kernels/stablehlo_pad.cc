 ...
 public:
  ...
  void Setup(const int* dims, const int rank, const int64_t element_size) {
    ...
    for (int i = 0; i < rank; ++i) {
      const CheckedInt<int64_t> dim = (CheckedInt<int64_t>(dims[i]) - 1) *
    ...
    {
      CheckedInt<int64_t> size =
          std::accumulate(output_shape_, output_shape_ + rank,
                          CheckedInt(element_size), std::multiplies<>());
