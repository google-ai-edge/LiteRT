...
inline void MulElementwise(int size, const ArithmeticParams& params,
                           const uint16_t* input1_data,
                           const uint16_t* input2_data,
                           uint16_t* output_data) {
  for (int i = 0; i < size; ++i) {
    const int32_t input1_val = params.input1_offset + input1_data[i];
    const int32_t input2_val = params.input2_offset + input2_data[i];
    const int64_t input_product =
        static_cast<int64_t>(input1_val) * static_cast<int64_t>(input2_val);
    const int32_t unclamped_result =
        params.output_offset +
        MultiplyByQuantizedMultiplier(input_product, params.output_multiplier,
                                      params.output_shift);
    const int32_t clamped_output =
        std::min(params.quantized_activation_max,
                 std::max(params.quantized_activation_min, unclamped_result));
    output_data[i] = static_cast<uint16_t>(clamped_output);
  }
}
...
inline void BroadcastMul6DSlow(const ArithmeticParams& params,
  ...
  BroadcastMulRecursiveDimensions(
      ...
      [](const ArithmeticParams& params, const uint16_t input1_val,
         const uint16_t input2_val) {
        const int32_t offsetted_input1_val = params.input1_offset + input1_val;
        const int32_t offsetted_input2_val = params.input2_offset + input2_val;
        const int64_t input_product =
            static_cast<int64_t>(offsetted_input1_val) *
            static_cast<int64_t>(offsetted_input2_val);
        const int32_t unclamped_result =
            params.output_offset +
            MultiplyByQuantizedMultiplier(
                input_product, params.output_multiplier, params.output_shift);
        const int32_t clamped_output = std::min(
            params.quantized_activation_max,
            std::max(params.quantized_activation_min, unclamped_result));
        return static_cast<uint16_t>(clamped_output);
      });
}
