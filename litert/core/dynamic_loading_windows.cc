...
LiteRtStatus FindLiteRtCompilerPluginSharedLibs(
  ...
  const std::string lib_pattern = absl::StrCat(
      ToWindowsLibName(kLiteRtSharedLibPrefix), "CompilerPlugin");
...
LiteRtStatus PutLibOnLdPath(absl::string_view search_path,
  ...
  std::vector<std::string> results;
  auto status = FindLiteRtSharedLibsHelper(
      std::string(search_path), std::string(lib_pattern),
      /*full_match=*/false, results);
