...
void LiteRtRewriterT::ApplyChanges(LiteRtSubgraphT* subgraph_to_apply) {
  ...
  auto const is_a_io_tensor = [](const auto& tensor, auto io_Tensors) {
    return std::any_of(
        io_Tensors.begin(), io_Tensors.end(),
        [&tensor](const auto& input) { return input == tensor; });
  };
  ...
  LITERT_LOG(LITERT_INFO, "splice_index starting: %zu", splice_index);
  for (size_t original_op_index = 0;
       original_op_index < subgraph_to_apply->Ops().size();
       ++original_op_index) {
    for (auto op_to_erase : erases_) {
