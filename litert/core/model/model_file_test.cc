...
TEST(ModelLoadTest, BadFileData) {
  ...
  const auto status =
      LiteRtCreateModelFromFile(test_file_path_str.c_str(), &model);
#else
  const auto status = LiteRtCreateModelFromFile(test_file_path.c_str(), &model);
#endif
  EXPECT_EQ(status, kLiteRtStatusErrorFileIO);
  // NOLINTEND
}
