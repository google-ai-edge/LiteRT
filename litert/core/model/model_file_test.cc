...
#include "litert/core/model/model_file_test_util.h"
...
using ModelFactory = std::function<Expected<DetailedModel>()>;
...
Expected<DetailedModel> LoadModelThroughRoundTrip(absl::string_view filename) {
  auto model = DetailedModel::CreateFromFile(GetTestFilePath(filename));
  if (!model) {
    return model.Error();
  }
  ...
  return DetailedModel::CreateFromOwnedHandle(result);
}
...
ModelFactory MakeLoadFactory(absl::string_view filename) {
  return [=]() {
    return DetailedModel::CreateFromFile(GetTestFilePath(filename));
  };
}
...
class TestWithModelFactory : public ::testing::TestWithParam<ModelFactory> {
 protected:
  Expected<DetailedModel> LoadModel() { return GetParam()(); }
};
