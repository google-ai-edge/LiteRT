...
template <typename T>
inline void ReferenceBatchMatmul(const T* lhs_data, const int32_t* lhs_dims,
                                 size_t lhs_rank, const T* rhs_data,
                                 const int32_t* rhs_dims, size_t rhs_rank,
                                 T* out_data, const int32_t* out_dims,
  ...
  for (int64_t b = 0; b < out_batch_size; ++b) {
    ...
    const T* curr_lhs = lhs_data + lhs_batch_offset * (m * k);
    const T* curr_rhs = rhs_data + rhs_batch_offset * (k * n);
    T* curr_out = out_data + b * (m * n);
...
template <typename T>
inline void ReferenceFullyConnected(const T* input_data, const T* weights_data,
  ...
  for (int64_t b = 0; b < batch_size; ++b) {
    for (int64_t o = 0; o < output_dim; ++o) {
      T sum = bias_data ? bias_data[o] : T{0};
  ...
  if constexpr (std::is_same_v<T, float>) {
    litert::internal::ApplyActivation(output_data, batch_size * output_dim,
                                      faf);
  }
}
