...
TEST(BuilderTest, ApplyChanges_RemoveMultiple) {
  LiteRtModelT model;
  auto& subgraph = model.EmplaceSubgraph();
  // Op0, Op1, Op2, Op3
  ...
  auto& t4 = subgraph.EmplaceTensor();

  auto& op0 = subgraph.EmplaceOp();
  op0.SetOpCode(kLiteRtOpCodeTflAdd);
  AttachInput(&t0, op0);
  AttachOutput(&t1, op0);
  ...
  op3.SetOpCode(kLiteRtOpCodeTflDiv);
  AttachInput(&t3, op3);
  AttachOutput(&t4, op3);

  // Replace Op1 with Abs. Remove Op3.
  LiteRtBuilderT builder;
  builder.BuildOp(kLiteRtOpCodeTflAbs, {&t1}, {&t2});
  builder.EraseOp(&op1);
  builder.EraseOp(&op3);
  ...
  ASSERT_EQ(subgraph.Ops().size(), 3);
  // Expected: Op0, Abs, Op2. (Op3 removed).
  EXPECT_EQ(subgraph.Ops()[0]->OpCode(), kLiteRtOpCodeTflAdd);
  EXPECT_EQ(subgraph.Ops()[1]->OpCode(), kLiteRtOpCodeTflAbs);
  EXPECT_EQ(subgraph.Ops()[2]->OpCode(), kLiteRtOpCodeTflSub);
}
...
TEST(BuilderTest, ApplyChanges_Chain_ReplaceMiddle) {
  LiteRtModelT model;
  auto& subgraph = model.EmplaceSubgraph();
  // A -> B -> C
  auto& t0 = subgraph.EmplaceTensor();
  ...
  auto& opA = subgraph.EmplaceOp();
  opA.SetOpCode(kLiteRtOpCodeTflAdd);
  AttachInput(&t0, opA);
  AttachOutput(&t1, opA);

  auto& opB = subgraph.EmplaceOp();
  opB.SetOpCode(kLiteRtOpCodeTflMul);
  AttachInput(&t1, opB);
  AttachOutput(&t2, opB);

  auto& opC = subgraph.EmplaceOp();
  opC.SetOpCode(kLiteRtOpCodeTflSub);
  AttachInput(&t2, opC);
  AttachOutput(&t3, opC);

  // Replace B with Div
  LiteRtBuilderT builder;
  // Use same inputs/outputs as B
  builder.BuildOp(kLiteRtOpCodeTflDiv, {&t1}, {&t2});
  builder.EraseOp(&opB);
  ...
  ASSERT_EQ(subgraph.Ops().size(), 3);
  EXPECT_EQ(subgraph.Ops()[0]->OpCode(), kLiteRtOpCodeTflAdd);
  EXPECT_EQ(subgraph.Ops()[1]->OpCode(), kLiteRtOpCodeTflDiv);  // Replaced B
  EXPECT_EQ(subgraph.Ops()[2]->OpCode(), kLiteRtOpCodeTflSub);
}

TEST(BuilderTest, ApplyChanges_Chain_ReplaceLast) {
  LiteRtModelT model;
  auto& subgraph = model.EmplaceSubgraph();
  // A -> B -> C
  auto& t0 = subgraph.EmplaceTensor();
  ...
  // Replace C with Div
  LiteRtBuilderT builder;
  builder.BuildOp(kLiteRtOpCodeTflDiv, {&t2}, {&t3});
  builder.EraseOp(&opC);
  ...
  EXPECT_EQ(subgraph.Ops()[0]->OpCode(), kLiteRtOpCodeTflAdd);
  EXPECT_EQ(subgraph.Ops()[1]->OpCode(), kLiteRtOpCodeTflMul);
  EXPECT_EQ(subgraph.Ops()[2]->OpCode(), kLiteRtOpCodeTflDiv);  // Replaced C
}

TEST(BuilderTest, ApplyChanges_Chain_ReplaceFirst) {
  LiteRtModelT model;
  auto& subgraph = model.EmplaceSubgraph();
  // A -> B -> C
  auto& t0 = subgraph.EmplaceTensor();
  ...
  // Replace A with Div
  LiteRtBuilderT builder;
  builder.BuildOp(kLiteRtOpCodeTflDiv, {&t0}, {&t1});
  builder.EraseOp(&opA);

  builder.ApplyChanges(&subgraph);

  ASSERT_EQ(subgraph.Ops().size(), 3);
  EXPECT_EQ(subgraph.Ops()[0]->OpCode(), kLiteRtOpCodeTflDiv);  // Replaced A
  EXPECT_EQ(subgraph.Ops()[1]->OpCode(), kLiteRtOpCodeTflMul);
  EXPECT_EQ(subgraph.Ops()[2]->OpCode(), kLiteRtOpCodeTflSub);
}

TEST(BuilderTest, ApplyChanges_Chain_ReplaceMiddleWithTwo) {
  LiteRtModelT model;
  auto& subgraph = model.EmplaceSubgraph();
  // A -> B -> C
  // Replace B with D -> E
  ...
  LiteRtBuilderT builder;
  // B takes t1, produces t2.
  // New ops: D(t1 -> temp), E(temp -> t2).
  auto temp_src = BuildSimpleTensor();
  auto& temp = builder.BuildTensor(temp_src);
  builder.BuildOp(kLiteRtOpCodeTflAbs, {&t1}, {&temp});  // D
  builder.BuildOp(kLiteRtOpCodeTflNeg, {&temp}, {&t2});  // E
  builder.EraseOp(&opB);

  builder.ApplyChanges(&subgraph);

  ASSERT_EQ(subgraph.Ops().size(), 4);
  EXPECT_EQ(subgraph.Ops()[0]->OpCode(), kLiteRtOpCodeTflAdd);
  EXPECT_EQ(subgraph.Ops()[1]->OpCode(), kLiteRtOpCodeTflAbs);  // D
  EXPECT_EQ(subgraph.Ops()[2]->OpCode(), kLiteRtOpCodeTflNeg);  // E
  EXPECT_EQ(subgraph.Ops()[3]->OpCode(), kLiteRtOpCodeTflSub);
}

TEST(BuilderTest, ApplyChanges_Chain_AppendAtEnd) {
  // A -> B. Append C.
  LiteRtModelT model;
  auto& subgraph = model.EmplaceSubgraph();
  auto& t0 = subgraph.EmplaceTensor();
  ...
  LiteRtBuilderT builder;
  auto t3_src = BuildSimpleTensor();
  auto& t3 = builder.BuildTensor(t3_src);
  builder.BuildOp(kLiteRtOpCodeTflDiv, {&t2}, {&t3});
