...
TEST(ErrorReporterTest, DefaultErrorReporter) {
  ErrorReporter* reporter = DefaultErrorReporter();
  ASSERT_NE(reporter, nullptr);
  
  // Default error reporter should be the same instance
  EXPECT_EQ(reporter, DefaultErrorReporter());
}
...
TEST(ErrorReporterTest, StatefulErrorReporterInterface) {
  BufferErrorReporter reporter;
  StatefulErrorReporter* stateful = &reporter;
  
  stateful->Report("Error through interface");
  EXPECT_EQ(stateful->message(), "Error through interface\n");
}
