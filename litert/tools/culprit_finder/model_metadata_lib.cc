...
std::string ModelMetadata::GetNodeShapes(int node_index) {
  auto node_proto = node_index_to_node_proto_[node_index];
  std::string input_shapes = absl::StrJoin(
      node_proto.inputs(), ",", [&](std::string* out, int input) {
  ...
  std::string output_shapes = absl::StrJoin(
...
std::string ModelMetadata::EdgeShapeToString(
  ...
  std::string shape_string = absl::StrJoin(edge.shape(), ",");
