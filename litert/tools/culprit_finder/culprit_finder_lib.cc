...
TfLiteStatus CulpritFinder::RunCulpritFinderLinearSearch() {
  ...
  const int batch_size = params_.Get<int>(kLinearSearchBatchSizeFlag);
  ...
  for (const std::pair<int, int>& node_range : node_ranges) {
    OverallStat overall_stat;
    const std::vector<int> output_tensors =
        model_metadata_->GetOutputTensorsOfNode(node_range.second);
    if (CalculateErrorStats(node_range.first, node_range.second, output_tensors,
                            overall_stat) != kTfLiteOk) {
      LITERT_LOG(LITERT_ERROR, "Failed to calculate error stats");
      return kTfLiteError;
    }
    ...
    LITERT_LOG(LITERT_INFO, "Done with Node range: [%d - %d]", node_range.first,
               node_range.second);
  }
  MakeReport();
  return kTfLiteOk;
...
inline void CulpritFinder::LogOverallStat(const OverallStat& overall_stat) {
  ...
  if (overall_stat.delegated_node_range.first !=
    ...
    LITERT_LOG(
        LITERT_INFO, "  Delegated node range: [%s, %s]",
        model_metadata_
            ->GetNodeIdentifier(overall_stat.delegated_node_range.first, true)
            .c_str(),
        model_metadata_
            ->GetNodeIdentifier(overall_stat.delegated_node_range.second, true)
            .c_str());
  } else {
    LITERT_LOG(
        LITERT_INFO, "  Delegated node range: %s",
        model_metadata_
            ->GetNodeIdentifier(overall_stat.delegated_node_range.first, true)
            .c_str());
  }
...
void CulpritFinder::MakeReport() {
  ...
  for (int i = 0; i < overall_stats_.size() &&
    ...
    LITERT_LOG(LITERT_INFO, "%d - %d, %s - %s, %s, %f", node_start_index,
               node_end_index,
               model_metadata_->GetNodeIdentifier(node_start_index).c_str(),
               model_metadata_->GetNodeIdentifier(node_end_index).c_str(),
  ...
  LITERT_LOG(LITERT_INFO, "Top %d node(s) with most errors (op_name, count):",
             params_.Get<int>(kLinearSearchReportCountFlag));
  for (int i = 0; i < sorted_node_to_overall_stats_index.size() &&
                  i < params_.Get<int>(kLinearSearchReportCountFlag);
       ++i) {
    LITERT_LOG(LITERT_INFO, "%s, %zu",
               sorted_node_to_overall_stats_index[i].first.c_str(),
  ...
  if (!sorted_node_to_overall_stats_index_with_nan.empty()) {
    LITERT_LOG(LITERT_INFO,
               ...
               params_.Get<int>(kLinearSearchReportCountFlag));
    for (int i = 0; i < sorted_node_to_overall_stats_index_with_nan.size() &&
                    i < params_.Get<int>(kLinearSearchReportCountFlag);
         ++i) {
      LITERT_LOG(LITERT_INFO, "%s, %zu",
                 sorted_node_to_overall_stats_index_with_nan[i].first.c_str(),
