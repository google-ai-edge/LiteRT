...
Expected<void> ExtractBytecode(const std::string& model_path) {
  ...
  if (!infile) {
    return Unexpected(kLiteRtStatusErrorInvalidArgument,
                      "Cannot open source file: " + model_path);
  }
  ...
  if (!infile.read(buffer.data(), file_size)) {
    return Unexpected(kLiteRtStatusErrorInvalidArgument,
                      "Failed to read from source file: " + model_path);
  }
  infile.close();
  ...
  for (auto i = 0; i < model_obj.NumSubgraphs(); ++i) {
    ...
    for (auto& op : subgraph->Ops()) {
      ...
      std::string output_filepath = absl::StrCat(
          absl::GetFlag(FLAGS_output_dir), "/", dispatch_opts.name, ".bin");
      std::ofstream outfile(output_filepath, std::ios::binary);
      if (!outfile) {
        return Unexpected(kLiteRtStatusErrorInvalidArgument,
                          "Failed to create output file: " + output_filepath);
      }
