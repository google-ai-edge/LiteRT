...
TEST(BuildCustomNpuModelTest, NonExistentInputFileFailsTest) {
  BuildCustomNpuModelOptions options;
  options.npu_bytecode_path = (std::filesystem::path(::testing::TempDir()) /
                               "non_existent_file_12345.bin")
                                  .string();
  options.output_model_path =
      (std::filesystem::path(::testing::TempDir()) / "dummy_out.tflite")
          .string();
...
TEST(BuildCustomNpuModelTest, MissingTensorSpecsFailsTest) {
  const std::string dummy_bytecode_file =
      (std::filesystem::path(::testing::TempDir()) / "dummy_bytecode_test.bin")
          .string();
  {
  ...
  options.output_model_path =
      (std::filesystem::path(::testing::TempDir()) / "dummy_out2.tflite")
          .string();
...
TEST(BuildCustomNpuModelTest, InvalidOutputFileLocationFailsTest) {
  const std::string dummy_bytecode_file =
      (std::filesystem::path(::testing::TempDir()) / "dummy_bytecode_test2.bin")
          .string();
  {
    std::ofstream ofs(dummy_bytecode_file, std::ios::binary);
    ofs << "DUMMY_BYTECODE_CONTENT";
  ...
  BuildCustomNpuModelOptions options;
  options.npu_bytecode_path = dummy_bytecode_file;
  options.output_model_path = (std::filesystem::path(::testing::TempDir()) /
                               "non_existent_directory_12345" / "out.tflite")
                                  .string();
  options.input_tensors.push_back(
      {"input_0", kLiteRtElementTypeFloat32, {1, 10}});
  options.output_tensors.push_back(
