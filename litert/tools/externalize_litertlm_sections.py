...
def _toml_path(path: Path, output_toml: Path) -> str:
  relative = os.path.relpath(path.resolve(), output_toml.parent.resolve())
  return json.dumps(Path(relative).as_posix())


def _replace_data_path(block: str, path: Path, output_toml: Path) -> str:
...
def build_manifest(input_toml: Path, output_manifest: Path) -> int:
...
def _read_results(results_path: Path) -> dict[str, tuple[Path, Path | None]]:
...
def rewrite_toml(
    input_toml: Path, results_path: Path, output_toml: Path
) -> tuple[int, int]:
...
def verify_peek_layout(
    peek_path: Path, expected_models: int, expected_weights: int
) -> None:
...
def _parse_args() -> argparse.Namespace:
  ...
  manifest.add_argument("--toml", type=Path, required=True)
  manifest.add_argument("--output", type=Path, required=True)

  rewrite = subparsers.add_parser("rewrite")
  rewrite.add_argument("--toml", type=Path, required=True)
  rewrite.add_argument("--results", type=Path, required=True)
  rewrite.add_argument("--output", type=Path, required=True)

  verify = subparsers.add_parser("verify-peek")
  verify.add_argument("--peek", type=Path, required=True)
