...
ABSL_FLAG(std::string, intel_openvino_graph_backends, "",
          ...
          "one of cpu, gpu, npu, auto.  Partitions without an entry default "
          "to npu.");
...
bool AbslParseFlag(absl::string_view text,
                   LiteRtIntelOpenVinoGraphBackend* options, std::string* error) {
  if (text == "cpu") {
  ...
  if (text == "auto") {
    *options = kLiteRtIntelOpenVinoGraphBackendAUTO;
    return true;
  }
...
std::string AbslUnparseFlag(LiteRtIntelOpenVinoGraphBackend options) {
  switch (options) {
    ...
    case kLiteRtIntelOpenVinoGraphBackendNPU:
      return "npu";
    case kLiteRtIntelOpenVinoGraphBackendAUTO:
      return "auto";
  }
}
...
Expected<void> UpdateIntelOpenVinoOptionsFromFlags(
  ...
  if (!graph_backends_str.empty()) {
    ...
    if (graph_backends_str.find(':') == std::string::npos &&
        graph_backends_str.find(';') == std::string::npos) {
  ...
  if (!graph_configs_str.empty()) {
    ...
    for (const auto& entry : entries) {
      ...
      options.SetGraphConfigsMapOption(graph_index, key.c_str(),
                                       value.c_str());
    }
  }
