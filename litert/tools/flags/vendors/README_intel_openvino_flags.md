...
Flag                                  | Type   | Default | Description
------------------------------------- | ------ | ------- | -----------
`--intel_openvino_graph_backends`     | string | `""`    | Either a bare backend name (`cpu`, `gpu`, `npu`, `auto`) applied to all partitions, or semicolon-separated `GRAPH_INDEX:BACKEND` entries (e.g. `0:npu;1:cpu`) for per-partition selection. Partitions without an entry default to NPU.
