...
ABSL_FLAG(litert::qualcomm::QualcommOptions::DspPdSession,
          ...
          "DSP Process Domain session. 'unsigned' (default) uses unsigned "
...
ABSL_FLAG(uint32_t, qualcomm_htp_device_id, 0,
          "The HTP device ID to select from QNN platform information. "
          "A nonzero value requires a matching platform-reported device; 0 "
          "leaves device selection to QNN's default configuration.");
...
ABSL_FLAG(std::string, qualcomm_qnn_lib_dir, "",
          ...
          "(libQnnSystem.so, libQnnHtp.so, etc.). If not set, falls back to "
          "the plugin directory (shared_library_dir), then to standard system "
          "loader paths.");
...
ABSL_FLAG(std::string, qualcomm_dsp_skel_dir, "",
          ...
          "FastRPC. If not set, falls back to qualcomm_qnn_lib_dir, then to "
          "the plugin directory (shared_library_dir).");
...
ABSL_FLAG(litert::qualcomm::QualcommOptions::LpaiTarget, qualcomm_lpai_target,
          litert::qualcomm::QualcommOptions::LpaiTarget::kAdsp,
          "LPAI target environment. Valid settings: \"x86\", "
          "\"arm\", \"adsp\", \"tensilica\".");
...
bool AbslParseFlag(absl::string_view text,
  ...
  *error = "Unknown LPAI core affinity type";
  return false;
}
...
Expected<void> UpdateQualcommOptionsFromFlags(QualcommOptions& opts) {
  ...
  const auto lpai_core_affinity_type =
      absl::GetFlag(FLAGS_qualcomm_lpai_core_affinity_type);
  opts.SetLpaiCoreAffinityType(lpai_core_affinity_type);
