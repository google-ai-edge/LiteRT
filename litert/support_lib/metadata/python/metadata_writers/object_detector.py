...
from litert.support_lib.metadata import metadata_schema_py_generated as _metadata_fb
from litert.support_lib.metadata import schema_py_generated as _schema_fb
from litert.support_lib.metadata.python import metadata as _metadata
from litert.support_lib.metadata.python.metadata_writers import metadata_info
from litert.support_lib.metadata.python.metadata_writers import metadata_writer
from litert.support_lib.metadata.python.metadata_writers import writer_utils
...
def _create_metadata_with_value_range(
  ...
  assert tensor_metadata.content is not None
...
def _extend_new_files(
    file_list: List[str],
    associated_files: Optional[Sequence[metadata_info.AssociatedFileMd]],
):
...
class MetadataWriter(metadata_writer.MetadataWriter):
  ...
  @classmethod
  def create_from_metadata_info(  # pyrefly: ignore[bad-override]
      cls,
      ...
      output_score_md: Union[
          None, metadata_info.TensorMd, metadata_info.ClassificationTensorMd
      ] = None,
  ...
  def create_for_inference(
    ...
    """Creates mandatory metadata for LiteRT Support inference.
