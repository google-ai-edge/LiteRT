...
def get_input_tensor_types(model_buffer: bytearray) -> List[int]:
...
def get_output_tensor_types(model_buffer: bytearray) -> List[int]:
...
def get_tokenizer_associated_files(
    tokenizer_options: Union[
        None,
        _metadata_fb.BertTokenizerOptionsT,
  ...
  def _get_file_path(
      files: Optional[List[_metadata_fb.AssociatedFileT]],
  ) -> List[str]:
    if not files:
      return []
    return [file.name for file in files if file.name is not None]
