...
import static com.google.ai.edge.litert.support.metadata.Preconditions.checkElementIndex;
...
final class BoundedInputStream extends InputStream {
  ...
  BoundedInputStream(SeekableByteChannelCompat channel, long start, long remaining) {
    checkArgument(
        remaining >= 0 && start >= 0,
        String.format(
            Locale.ROOT, "Invalid length of stream at offset=%d, length=%d", start, remaining));
