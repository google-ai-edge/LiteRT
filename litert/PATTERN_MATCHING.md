...
## 1. Introduction to the Matcher API
...
### Basic Op Matching
 *   `m_OpCode(code)`: Matches an operation by its `LiteRtOpCode`.
...
| `m_OpVariadic(c, ...)` | Matches opcode and prefix inputs. | Op |
| `m_CommutativeOp(...)` | Binary op with inputs in any order. | Op / Tensor |
