...
LiteRtStatus LiteRtGetOpaqueOptionsHash(LiteRtOpaqueOptions options,
  ...
  // If no hash function is provided, default to `HashCStringPayload` only if
  // the payload is a known C-string. `HashCStringPayload` assumes a
  // null-terminated string and is unsafe for other types.
  if (options->payload_identifier == "google_tensor" ||
      options->payload_identifier == "darwinn_runtime_options") {
