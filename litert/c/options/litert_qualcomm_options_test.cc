...
TEST(QualcommOptionsTest, CppWrapper) {
  ...
  EXPECT_EQ(options->GetLpaiTarget(), QualcommOptions::LpaiTarget::kUnknown);
  options->SetLpaiTarget(QualcommOptions::LpaiTarget::kAdsp);
...
TEST(LiteRtQualcommOptionsTest, LpaiOptions) {
  ...
  LITERT_ASSERT_OK(LrtQualcommOptionsSetLpaiTarget(
      qualcomm_options, kLiteRtQualcommLpaiTargetAdsp));
  ...
  auto parsed = SerializeAndParse(qualcomm_options);
  EXPECT_EQ(parsed.GetLpaiTarget(), QualcommOptions::LpaiTarget::kAdsp);
