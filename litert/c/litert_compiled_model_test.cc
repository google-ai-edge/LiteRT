...
TEST(CompiledModelTest, ResizeInputTensor) {
  ...
  // Original shape is [2, 5, 5, 1], let's change to [4, 5, 5, 1]
  ...
  size_t new_size;
  ASSERT_EQ(LiteRtGetTensorBufferRequirementsBufferSize(new_requirements,
                                                        &new_size),
            kLiteRtStatusOk);

  // Verify that the size has doubled (batch size doubled)
  EXPECT_EQ(new_size, original_size * 2);
