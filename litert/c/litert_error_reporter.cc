...
// Must be outside anonymous namespace to match the forward declaration
// from LITERT_DEFINE_HANDLE macro
...
LiteRtStatus LiteRtErrorReporterGetMessage(LiteRtErrorReporter reporter,
  ...
  auto* stateful =
      dynamic_cast<litert::StatefulErrorReporter*>(reporter->reporter.get());
  if (!stateful) {
...
LiteRtStatus LiteRtErrorReporterClear(LiteRtErrorReporter reporter) {
  ...
  auto* buffer =
      dynamic_cast<litert::BufferErrorReporter*>(reporter->reporter.get());
  if (!buffer) {
    return kLiteRtStatusErrorUnsupported;
  }
