...
export class LiteRtModelRunner implements ModelRunner {
  ...
  static async load(
      ...
      weightsStream?: ReadableStream<Uint8Array>): Promise<LiteRtModelRunner> {
    await liteRtPromise;
    ...
    if (weightsStream) {
      ...
      if (!device) {
        ...
        gpuModel = await toMaybe(
            () => loadModelAndWeights(data, weightsStream, {
              environment: new Environment({webGpuDevice: device}),
              accelerator: 'webgpu',
            }));
      }
      cpuModel = {
        error: 'CPU execution not supported with externalized weights'
      };
      webnnModel = {
        error: 'WebNN execution not supported with externalized weights'
      };
    } else {
      gpuModel =
