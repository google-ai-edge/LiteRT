...
export async function loadModelAndWeights(
  ...
  try {
    ...
    try {
      // Load Model
      // `_malloc` returns a signed i32, which reads back negative once the
      // heap grows past 2GB. Reinterpret it as unsigned before using it to
      // index the heap, or the `HEAPU8.set` below throws a `RangeError`.
      const ptr = wasm._malloc(modelData.byteLength) >>> 0;
      if (ptr === 0) {
        throw new Error(
      ...
      const fullOptions = fillCompileOptions(
          compileOptions, env, wasm.getThreadCount());
      // This compilation call triggers the WebGPU delegate which will
      // eventually invoke our callback, see the EM_ASYNC_JS block in
      // external_weight_loader_litert.cc for more details.
      ...
      // Unregister the callback to allow stream garbage collection
