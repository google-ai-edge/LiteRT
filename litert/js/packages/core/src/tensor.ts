...
export class Tensor implements Deletable, WithEnvironment {
  ...
  gather(indices: Tensor, axis: number = 0, batchDims: number = 0): Tensor {
 ...
 */
export function embeddingLookup(weights: Tensor, ids: Tensor): Tensor {
  weights.ensureNotDeleted();
  ids.ensureNotDeleted();
  const wasm = getGlobalLiteRt().liteRtWasm;
  const resultHandle = wasm.embeddingLookup(
    weights.liteRtTensorHandle,
    ids.liteRtTensorHandle,
  );
  return new Tensor(resultHandle, weights.environment);
}
...
export function nonMaxSuppression(
  ...
  boxes.ensureNotDeleted();
  scores.ensureNotDeleted();
 ...
 */
export function pRelu(input: Tensor, alpha: Tensor): Tensor {
  input.ensureNotDeleted();
  alpha.ensureNotDeleted();
  const wasm = getGlobalLiteRt().liteRtWasm;
  const resultHandle = wasm.prelu(
    input.liteRtTensorHandle,
