...
TEST_F(RngTensorBufferHelperTest, WriteRandom) {
  ...
  LITERT_ASSERT_OK(buf.WriteRandom<DummyRandomTensorBufferTraits>(device));
  EXPECT_THAT(buf.ConstSpan<int32_t>(), ElementsAre(0, 1, 2, 3));
}
...
TEST_F(RngTensorBufferHelperTest, WriteRandomOffset) {
  ...
  EXPECT_EQ(num_elements, 4);
  auto device = this->TracedDevice();
  LITERT_ASSERT_OK(buf.WriteRandom<DummyRandomTensorBufferTraits>(device, 2));
  EXPECT_THAT(buf.ConstSpan<int32_t>(), ElementsAre(1, 2, 0, 1));
}
...
TEST_F(RngTensorBufferHelperTest, WriteRandomSubArray) {
  ...
  auto device = this->TracedDevice();
  LITERT_ASSERT_OK(
      buf.WriteRandom<DummyRandomTensorBufferTraits>(device, 1, 2));
  EXPECT_THAT(buf.ConstSpan<int32_t>(), ElementsAre(1, 0, 1, 4));
}
