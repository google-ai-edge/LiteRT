...
class HasElementTypeMatcher {
 private:
  ...
  template <typename T>
  using MatcherInterface = ::testing::MatcherInterface<T>;
  using MatchResultListener = ::testing::MatchResultListener;
 ...
 public:
  ...
  class ImplBase {
   ...
   protected:
    ...
    bool MatchAndExplainImpl(const RankedTensorType& value,
                             MatchResultListener* listener) const {
  ...
  class ImplBase {
   public:
    ...
    explicit ImplBase(absl::Span<const int> dims)
        : dims_str_(DimsStr(dims)), dims_(dims) {}
...
// Matches the element type if type matched carries it.
inline auto HasTypeAspect(ElementType type) {
...
template <typename T>
auto EqMse(absl::Span<const T> expected, double tol = 1e-5) {
  auto mse = [expected](absl::Span<const T> actual) -> double {
    double err = 0;
    for (size_t i = 0; i < expected.size(); ++i) {
      err += std::pow(
          static_cast<double>(actual[i]) - static_cast<double>(expected[i]), 2);
    }
    return err / static_cast<double>(expected.size());
  };
  return ::testing::AllOf(::testing::SizeIs(expected.size()),
