...
class FullyConnected : public TestGraph {
 ...
 public:
  using InputTypesT =
      std::conditional_t<kHasBias, TypeList<T, T, T>, TypeList<T, T>>;
  ...
  Expected<VarBuffers> MakeInputs(
    ...
    LITERT_ASSIGN_OR_RETURN(auto weights,
                            SimpleBuffer::Create<T>(params_.weights_shape));
    LITERT_RETURN_IF_ERROR(
        (weights.template WriteRandom<T>(modified_data_builder, device)));
    inputs.push_back(std::move(weights));

    if constexpr (kHasBias) {
      LITERT_ASSIGN_OR_RETURN(auto bias,
                              SimpleBuffer::Create<T>(params_.bias_shape));
      LITERT_RETURN_IF_ERROR(
          (bias.template WriteRandom<T>(modified_data_builder, device)));
  ...
  Expected<void> Reference(const VarBuffers& inputs,
    ...
    if constexpr (kHasBias) {
      ...
      litert::internal::ReferenceFullyConnected<T>(
          input.data.data(), weights.data.data(), bias.data.data(),
          output.data.data(), batch_size, input_dim, output_dim, kFa);
    } else {
      auto [input, weights] = ref_inputs;
      litert::internal::ReferenceFullyConnected<T>(
          input.data.data(), weights.data.data(), nullptr, output.data.data(),
  ...
  static Expected<LiteRtModelT::Ptr> BuildGraph(const Params& params) {
    ...
    TensorTf input =
        litert::tensor::Create(std::string(kInputNames[0]),
                               litert::tensor::ApiType<T>::value, dims_in);
    TensorTf weights =
        litert::tensor::Create(std::string(kInputNames[1]),
                               litert::tensor::ApiType<T>::value, dims_wt);
    ...
    if constexpr (kHasBias) {
      ...
      bias_opt =
          litert::tensor::Create(std::string(kInputNames[2]),
                                 litert::tensor::ApiType<T>::value, dims_bs);
    }
