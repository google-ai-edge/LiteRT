...
std::vector<T> ExpectedTranspose(const std::array<Layout::Dim, Rank>& shape,
                                 const std::array<int32_t, Rank>& permutation,
                                 absl::Span<const T> input) {
...
TYPED_TEST(TransposeTest, TestLogic) {
  ...
  const auto expected_output_shape_span = absl::MakeConstSpan(
      expected_output_shape.data(), expected_output_shape.size());
  ...
  EXPECT_THAT(
      sg.Tensor(0).Type().second.ranked_tensor_type,
  ...
  EXPECT_THAT(
      sg.Tensor(2).Type().second.ranked_tensor_type,
      HasTypeAspect(TypeParam::kElementTypeValue, expected_output_shape_span));
  ...
  const auto* perm_data =
      reinterpret_cast<const int32_t*>(perm_tensor.Weights().Buffer().Data());
...
TEST(TransposeTest, Int4ReferenceMatchesKernelExample) {
  ...
  LITERT_ASSERT_OK_AND_ASSIGN(auto gen, GraphT::Create(params));
  LITERT_ASSERT_OK_AND_ASSIGN(
      auto input, SimpleBuffer::Create(RankedTensorType(
                      ElementType::Int4, Layout(Dimensions({3, 2})))));
