...
template <typename T, typename OpCode, typename Padding, typename Fa = FaC<>>
class Pooling : public TestGraph {
 ...
 public:
  ...
  static Expected<Pooling::Ptr> Create(Rng& rng) {
    ...
    std::uniform_int_distribution<int> batch_dist(1, 2);
    std::uniform_int_distribution<int> dim_dist(5, 20);
    std::uniform_int_distribution<int> channel_dist(1, 10);

    params.input_shape[0] = batch_dist(rng);
    params.input_shape[1] = dim_dist(rng);
    params.input_shape[2] = dim_dist(rng);
    params.input_shape[3] = channel_dist(rng);

    std::uniform_int_distribution<int> fh_dist(1, params.input_shape[1]);
    std::uniform_int_distribution<int> fw_dist(1, params.input_shape[2]);
    params.filter_height = fh_dist(rng);
    params.filter_width = fw_dist(rng);

    std::uniform_int_distribution<int> stride_dist(1, 3);
    params.stride_height = stride_dist(rng);
    params.stride_width = stride_dist(rng);
    ...
    options->stride_w = params.stride_width;
    options->stride_h = params.stride_height;
    options->filter_width = params.filter_width;
    options->filter_height = params.filter_height;
  ...
  Expected<void> Reference(const VarBuffers& inputs,
    ...
    int filter_h = params_.filter_height;
    int filter_w = params_.filter_width;
    int stride_h = params_.stride_height;
    int stride_w = params_.stride_width;
 ...
 private:
  static Expected<LiteRtModelT::Ptr> BuildGraph(const Params& params) {
    ...
    PoolingOpDetails op_details(kPadding, params.stride_width,
                                params.stride_height, params.filter_width,
                                params.filter_height, kFa);
