...
template <>
struct OpDetails<kLiteRtOpCodeTflSum>
    : public ReductionOpDetails<kLiteRtOpCodeTflSum> {
  using ReductionOpDetails::ReductionOpDetails;
};
...
class Reduction : public TestGraph {
 ...
 public:
  ...
  static Expected<Reduction::Ptr> Create(Rng& rng) {
    ...
    // Randomly convert some axes to negative equivalents
  ...
  Expected<void> Reference(const VarBuffers& inputs,
    ...
    if constexpr (kOpCode == kLiteRtOpCodeTflReduceMax) {
      ...
      init_value = 1;
      reducer = [](const T a, const T b) { return (a && b) ? T(1) : T(0); };
    } else if constexpr (kOpCode == kLiteRtOpCodeTflSum) {
      init_value = 0;
      reducer = [](const T a, const T b) { return a + b; };
    }
    ...
    std::vector<int> axes_int;
    for (auto a : params_.axes) {
      axes_int.push_back(a < 0 ? a + kRank : a);
    }

    // Sort axes as ReduceGeneric may assume sorted axes.
    std::sort(axes_int.begin(), axes_int.end());

    if (!litert::internal::ReferenceReduction<T>(
            input.data.data(), input_dims_int.data(), kRank, output.data.data(),
            output_dims_int.data(), output_dims_int.size(), axes_int.data(),
            axes_int.size(), kKeepDims, init_value, reducer)) {
      return Error(kLiteRtStatusErrorInvalidArgument,
                   "ReferenceReduction failed.");
    }
  ...
  static Expected<LiteRtModelT::Ptr> BuildGraph(const Params& params) {
    ...
    op_inputs[0] = TensorDetails{
        std::vector<int32_t>(params.input_shape.begin(),
                             params.input_shape.begin() + kRank),
