...
class Mean : public TestGraph {
 ...
 public:
  ...
  static Expected<Mean::Ptr> Create(Rng& rng) {
    ...
    // Directly compute expected output shape independently of LiteRT runtime
    // inferrers.
    std::vector<bool> is_reduced(kRank, false);
    for (int32_t axis : params.axes) {
      size_t canonical_axis = axis < 0 ? axis + kRank : axis;
      is_reduced[canonical_axis] = true;
    }

    for (size_t i = 0; i < kRank; ++i) {
      if (is_reduced[i]) {
        if constexpr (kKeepDims) {
          params.output_shape.push_back(1);
        }
      } else {
        params.output_shape.push_back(params.input_shape[i]);
      }
    }
