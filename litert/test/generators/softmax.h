...
template <typename Rank, typename T, typename OpCode = OpCodeC<kLiteRtOpCodeTflSoftmax>,
...
class Softmax : public TestGraph {
 ...
 public:
  ...
  Expected<void> Reference(const VarBuffers& inputs,
    ...
    if constexpr (kOpCode == kLiteRtOpCodeTflSoftmax) {
      litert::internal::ReferenceSoftmax<T>(
          in.data.data(), output.data.data(), batch, depth, kBeta);
    } else {
      litert::internal::ReferenceLogSoftmax<T>(
          in.data.data(), output.data.data(), batch, depth);
    }
