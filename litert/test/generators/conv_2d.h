...
#ifndef THIRD_PARTY_ODML_LITERT_LITERT_TEST_GENERATORS_CONV_2D_H_
#define THIRD_PARTY_ODML_LITERT_LITERT_TEST_GENERATORS_CONV_2D_H_
...
// Specialize OpDetails for Conv2D to handle options.
template <>
struct OpDetails<kLiteRtOpCodeTflConv2d> {
  using FbTypes = FbOpTypes<kLiteRtOpCodeTflConv2d>;
  ...
  TflOptions MakeTflOptions() const {
    ...
    res.Set(tflite::Conv2DOptionsT(options));
    return res;
  }
...
class Conv2d : public TestGraph {
 ...
 public:
  ...
  using Ptr = std::unique_ptr<Conv2d>;

  static constexpr absl::string_view Name() { return "Conv2d"; }

  template <typename Rng>
  static Expected<Conv2d::Ptr> Create(Rng& rng) {
    Params params;
    params.input_shape = {
        1, 10, 10, 1};  // Use a larger size to accommodate strides/dilation
    params.filter_shape = {1, 3, 3, 1};  // OHWI
    params.bias_shape = {1};

    // Fill filter and bias with fixed representative values
    params.filter_data.assign(9, static_cast<T_in>(1.0f));
    params.bias_data.assign(1, static_cast<T_out>(0.0f));
    ...
    LITERT_RETURN_IF_ERROR(litert::internal::InferConv2D(
    ...
    return std::make_unique<Conv2d>(std::move(params), std::move(model));
  }
  ...
  ConformanceSpec GetConformanceSpec() const override {
    ...
    spec.accumulation_depth = params_.filter_shape[1] *
                              params_.filter_shape[2] * params_.input_shape[3];
 ...
 private:
  static Expected<LiteRtModelT::Ptr> BuildGraph(const Params& params) {
    ...
    // MakeOwningBufferRef copies data from vector to satisfy TensorDetails.
    ...
    return SingleOpModel<kLiteRtOpCodeTflConv2d>(op_inputs, op_outputs,
                                                 kPadding, kStrideW, kStrideH,
                                                 kDilationW, kDilationH, kFa);
  }
...
#endif  // THIRD_PARTY_ODML_LITERT_LITERT_TEST_GENERATORS_CONV_2D_H_
