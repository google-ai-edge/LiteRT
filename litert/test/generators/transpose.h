...
class Transpose : public TestGraph {
 ...
 public:
  ...
  ConformanceSpec GetConformanceSpec() const override {
    ...
    spec.oracle_kind = ConformanceOracleKind::kCustomImplementation;
 ...
 private:
  ...
  static Expected<LiteRtModelT::Ptr> BuildGraph(const Params& params) {
    ...
    TensorDetails permutation = {{static_cast<int32_t>(kRank)},
                                 kLiteRtElementTypeInt32,
                                 std::string(kPermutationName),
                                 MakeBufferRef(params.permutation.begin(),
                                               params.permutation.end())};
  ...
  static Expected<void> FillDeterministicInput(SimpleBuffer& input) {
    ...
    for (size_t i = 0; i < span.size(); ++i) {
      if constexpr (std::is_same_v<T, bool>) {
        ...
        span[i] =
            static_cast<T>(static_cast<double>(static_cast<int32_t>(i % 29) - 14) /
                           4.0);
 ...
 public:
  ...
  Expected<VarBuffers> MakeInputs(
    ...
    LITERT_ASSIGN_OR_RETURN(auto input,
                            SimpleBuffer::Create(MakeTensorType(params_.shape)));
 ...
 private:
  ...
  void TransposeUnpacked(absl::Span<const int8_t> input,
    ...
    const auto input_strides =
        MakeStrides(absl::MakeConstSpan(params_.shape.data(), params_.shape.size()));
