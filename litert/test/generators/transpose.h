...
class Transpose : public TestGraph {
 ...
 private:
  static Expected<LiteRtModelT::Ptr> BuildGraph(const Params& params) {
    ...
    output.SetType(litert::tensor::ApiType<T>::value);
...
template <typename Rank, typename OpCode = OpCodeC<kLiteRtOpCodeTflTranspose>>
using TransposeInt4 = Transpose<Rank, litert::tensor::int4_t, OpCode>;
