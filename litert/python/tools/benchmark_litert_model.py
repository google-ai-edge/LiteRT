...
def _import_litert():
  """Import LiteRT modules, handling both litert and ai_edge_litert packages."""
  try:
    from litert.python.litert_wrapper.compiled_model_wrapper.compiled_model import CompiledModel  # type: ignore # pytype: disable=import-error
    from litert.python.litert_wrapper.compiled_model_wrapper.hardware_accelerator import HardwareAccelerator  # type: ignore # pytype: disable=import-error
    from litert.python.litert_wrapper.environment_wrapper.environment import Environment  # type: ignore # pytype: disable=import-error

    return CompiledModel, HardwareAccelerator, Environment
  except ImportError:
    pass
  try:
    from ai_edge_litert.compiled_model import CompiledModel  # type: ignore # pytype: disable=import-error
    from ai_edge_litert.hardware_accelerator import HardwareAccelerator  # type: ignore # pytype: disable=import-error
    from ai_edge_litert.environment import Environment  # type: ignore # pytype: disable=import-error
...
def create_environment(args, Environment):
  ...
  if args.compiler_plugin_path:
    kwargs['compiler_plugin_path'] = args.compiler_plugin_path

  dispatch_path = args.dispatch_library_path
  # Auto-discover dispatch library from the wheel package if not specified.
  if not dispatch_path:
    try:
      from litert.python.aot.vendors.intel_openvino import intel_openvino_backend  # pylint: disable=g-import-not-at-top # type: ignore # pytype: disable=import-error

      dispatch_path = intel_openvino_backend.get_dispatch_dir() or ''
    except ImportError:
      try:
        from ai_edge_litert.aot.vendors.intel_openvino import intel_openvino_backend  # pylint: disable=g-import-not-at-top # type: ignore # pytype: disable=import-error
  ...
  if dispatch_path:
    # The runtime expects a directory path, not a file path.
...
def run_benchmark(args):
  ...
  for i in range(args.warmup_runs):
    ...
    if args.verbose:
      logger.debug(
          '  Warmup %d/%d: %.2f ms', i + 1, args.warmup_runs, elapsed_ms
      )
  ...
  # --- Print results ---
  # Results table is printed directly to stdout (not via logger) so it is
  # always visible regardless of log level and easy to parse by scripts.
  print()
  print('=' * 40)
  ...
  print(
      f'Warmup (avg):          {warmup_avg_ms:.2f} ms ({len(warmup_times)}'
      ' runs)'
  )
