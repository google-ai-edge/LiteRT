...
class MixedPrecisionTest(testing.ModelUtilsTestCase):
  ...
  def test_convert_to_fp16_nested_region_skipped(self):
    ...
    # Build nested region with block arguments to avoid referencing outer SSA values directly
  ...
  def test_convert_to_fp16_with_custom_op_predicate(self):
    ...
    # Keep AddOp in FP32. MulOp and other parts of the graph should be converted to FP16.
    mixed_precision.convert_to_fp16(
    ...
    self.assertLen(
        cast_ops, 3
    )  # Two casts for AddOp operands (x, y), one cast for MulOp operand (AddOp result)
    ...
    # MulOp operand should be the result of a CastOp (f32 -> f16) on the AddOp's output
