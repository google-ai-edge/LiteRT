...
absl::StatusOr<tflite::ModelFlags> LiteRtConverter::buildModelFlags() {
  ...
  if (!model_flags.ParseFromString(config_.model_flags_proto_txt_raw)) {
...
absl::StatusOr<tflite::ConverterFlags> LiteRtConverter::buildConverterFlags() {
  ...
  if (!converter_flags.ParseFromString(config_.converter_flags_proto_txt_raw)) {
    return absl::InvalidArgumentError(
        "Failed to convert ConverterFlags to Python String.");
  }

  return converter_flags;
}
...
absl::StatusOr<tensorflow::GraphDef> LiteRtConverter::buildGraphDef(
  ...
  if (!!model_flags.use_hlo_import() &&
    ...
    return absl::InvalidArgumentError("Failed to convert GraphDef.");
  }
