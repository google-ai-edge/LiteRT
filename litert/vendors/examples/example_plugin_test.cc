...
TEST(TestCallDummyPlugin, CompileAoT) {
  ...
  EXPECT_TRUE(example_global_graph.subgraphs_.contains("partition_0"));
...
TEST(TestCallDummyPlugin, CompileJitHandle) {
  ...
  LiteRtCompiledResult result;
  LITERT_ASSERT_OK(LiteRtCompilerPluginCompile(plugin.get(), "Real_Jit_Test",
                                               model.Get(), &result));
  ...
  EXPECT_TRUE(example_global_graph_ptr->subgraphs_.contains("partition_0"));
