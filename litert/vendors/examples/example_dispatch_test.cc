...
TEST_F(ExampleDispatchTest, InvocationContextCustomOp) {
  // clang-format off
  static constexpr absl::string_view kSchema = R"(version:1
inputs:0,1
outputs:2
...
ops:custom(0,1)(2))";
  ...
  std::vector<float> out(4);
  LITERT_ASSERT_OK(output_tb.Read(absl::MakeSpan(out)));
  EXPECT_THAT(out, ElementsAre(-1.0f, -2.0f, -3.0f, -4.0f));
}
...
TEST_F(ExampleDispatchTest, RegisterAssetSuccess) {
  LiteRtDispatchDeviceContext device_context;
  LITERT_ASSERT_OK(Api().device_context_create(&device_context));
  auto device_context_ptr = CreateDevicePtr(Api(), device_context);

  const char* kAssetName = "test_asset";
  static constexpr absl::string_view kAssetData = "asset_data";
  LiteRtMemBuffer asset_buffer;
  asset_buffer.base_addr = kAssetData.data();
  asset_buffer.size = kAssetData.size();
  asset_buffer.fd = -1;
  asset_buffer.offset = 0;

  LITERT_ASSERT_OK(
      Api().register_asset(device_context, kAssetName, &asset_buffer));
  EXPECT_EQ(device_context->GetCustomOpAsset(), kAssetData);
}

TEST_F(ExampleDispatchTest, RegisterAssetInvalidArgsReturnsError) {
  LiteRtDispatchDeviceContext device_context;
  LITERT_ASSERT_OK(Api().device_context_create(&device_context));
  auto device_context_ptr = CreateDevicePtr(Api(), device_context);
  ...
  EXPECT_EQ(Api().register_asset(nullptr, kAssetName, &asset_buffer),
            kLiteRtStatusErrorInvalidArgument);
  EXPECT_EQ(Api().register_asset(device_context, nullptr, &asset_buffer),
            kLiteRtStatusErrorInvalidArgument);
  EXPECT_EQ(Api().register_asset(device_context, kAssetName, nullptr),
            kLiteRtStatusErrorInvalidArgument);
}
