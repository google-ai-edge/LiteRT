...
inline std::string MakeBytecodeHeader(LiteRtIntelOpenVinoGraphBackend graph_backend) {
  OpenVinoBytecodeHeader hdr{};
  std::memcpy(hdr.magic, OpenVinoBytecodeHeader::kMagic,
              sizeof(hdr.magic));
...
inline bool TryParseBytecodeHeader(const void* bytecode, size_t bytecode_size,
  ...
  if (graph_backend != nullptr) {
    *graph_backend = static_cast<LiteRtIntelOpenVinoGraphBackend>(hdr.graph_backend);
  }
