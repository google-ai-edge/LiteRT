...
// friendly_name == LiteRt tensor name) with an ov::op::v0::Parameter of the
// same element type and shape, rewiring all consumers. Constants at or below
// kMinConvertBytes are left unchanged.
//
// |const_map| (must be non-null) is populated with friendly_name -> BufferId
// for every converted weight. The dispatcher resolves each weight-Parameter to
// its pool buffer by friendly_name (robust to input reordering across import).
