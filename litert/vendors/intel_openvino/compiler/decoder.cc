...
inline const char* get_ov_op_type(const LiteRtOpCode op_code) {
    static const std::map<LiteRtOpCode, const char*> litert_ov_map{
    ...
    auto it = litert_ov_map.find(op_code);
    if (it == litert_ov_map.end()) {
        return "";
    }
    return it->second;
}
...
DecoderOperation::DecoderOperation(
    ...
    op_type_ = get_ov_op_type(litert_op_code_);
