...
bool fill_tensor_meta(ov::frontend::tensorflow_lite::TensorMetaInfo& tensor_meta_info,
    ...
    if (litert_tensor.QTypeId() == kLiteRtQuantizationPerTensor) {
        ...
        auto ov_quantization = std::make_shared<ov::frontend::tensorflow_lite::QuantizationInfo>();
        ...
        LITERT_LOG(LITERT_ERROR, "Unsupported Quantization type %d ", litert_tensor.QTypeId());
        return false;
    }
...
std::shared_ptr<ov::frontend::tensorflow_lite::DecoderBase> GraphIteratorDelegate::get_decoder()
    ...
    if (node_index_ < iterator_indices_.input_index_) {
        ...
        for (const auto& input : op.Inputs()) {
            ...
            if (input.HasWeights()) {
                LITERT_LOG(LITERT_VERBOSE, "Data is static or constant for op %d", op.Code());
