...
| ----------- | -------------------- | -------------------------------------- |
| AOT Compile | No                   | Cross-compilation works — e.g. compile |
:             :                      : on Linux, run the resulting `.tflite`  :
:             :                      : on Windows.                            :
| Inference   | Yes                  | The dispatch library talks to the      |
:             :                      : hardware via the Level Zero loader     :
:             :                      : shipped in the NPU driver package.     :
...
> the install command below. On Linux, if distro auto-detection picks the wrong
