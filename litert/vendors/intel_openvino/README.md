...
[Linux Device Setup](#setting-up-an-intel-npu-device-on-linux-ptl--lnl) -
**Windows:** See
...
```bash
# Single command — installs wheel + OpenVINO runtime
...
The `[npu-intel]` extra installs `openvino==2026.1.0` as a pip dependency. If
OpenVINO is already installed, pip skips it automatically.
...
> of the box without PATH setup. See [Windows Troubleshooting](#windows-1) if
...
# Verbose output (shows per-iteration timings)
litert-benchmark --model=model.tflite --use_npu --verbose
```
...
Use `--require_full_delegation` in the benchmark tool to fail fast if the NPU
cannot handle the entire model.
...
1.  **During `Environment.create()` without a dispatch path** —
    ...
    provided. When using the benchmark tool (which sets the dispatch path), NPU
    registers on the first attempt and this warning only appears on an internal
...
## OpenVINO Configuration Options

See [README_options.md](README_options.md) for the full list of runtime
configuration options and
[README_intel_openvino_flags.md](../../tools/flags/vendors/README_intel_openvino_flags.md)
for CLI flag documentation.

### Available Options

Option                            | Values                                           | Default   | Description
--------------------------------- | ------------------------------------------------ | --------- | -----------
`intel_openvino_device_type`      | `cpu`, `gpu`, `npu`, `auto`                      | `npu`     | Target device for inference
`intel_openvino_performance_mode` | `latency`, `throughput`, `cumulative_throughput` | `latency` | Performance optimization hint
`intel_openvino_configs_map`      | Comma-separated `KEY=VALUE` pairs                | `""`      | Custom OpenVINO properties

### Runtime Inference

Pass options directly to `CompiledModel.from_file()`:
...
model = CompiledModel.from_file(
    ...
    intel_openvino_device_type="npu",
    intel_openvino_performance_mode="latency",
    intel_openvino_configs_map="CACHE_DIR=/tmp/ov_cache",
)
```

### AOT Compilation

Pass options via the backend config dict:

```python
from ai_edge_litert.aot.vendors.intel_openvino import intel_openvino_backend

config = {
    "backend_id": "intel_openvino",
    "soc_model": "PTL",
    "intel_openvino_device_type": "npu",
    "intel_openvino_performance_mode": "latency",
    "intel_openvino_configs_map": "CACHE_DIR=/tmp/ov_cache",
...
Options can also be nested under `compilation_config`:
...
```bash
# NPU with latency mode
litert-benchmark --model=model.tflite --use_npu \
  --intel_openvino_device_type=npu \
  --intel_openvino_performance_mode=latency

# CPU inference via OpenVINO (for comparison)
litert-benchmark --model=model.tflite --use_npu \
  --intel_openvino_device_type=cpu

# Custom OpenVINO properties
litert-benchmark --model=model.tflite --use_npu \
  --intel_openvino_configs_map="INFERENCE_PRECISION_HINT=f16,CACHE_DIR=/tmp/ov_cache"
```

### Common configs_map Properties

Property                      | Example                                        | Description
----------------------------- | ---------------------------------------------- | -----------
`INFERENCE_PRECISION_HINT`    | `f16`, `f32`                                   | Preferred inference precision
`CACHE_DIR`                   | `/tmp/ov_cache`                                | Model compilation cache directory
`NUM_STREAMS`                 | `4`                                            | Parallel inference streams
`NPU_COMPILATION_MODE_PARAMS` | `compute-layers-with-higher-precision=Sigmoid` | NPU-specific compilation hints
...
# NPU benchmark
litert-benchmark --model=model.tflite --use_npu --num_runs=20
```
...
# CPU benchmark
litert-benchmark --model=model.tflite --num_runs=20 --num_threads=4
...
bazel test \
  ...
  //litert/python/tools:benchmark_litert_model_test \
...
`//litert/python/tools:benchmark_litert_model_test`                      | CLI arg parsing, HW accelerator logic
...
ai-edge-litert (wheel)
  ai_edge_litert/
    ...
    tools/
      benchmark_litert_model.py                      # Python benchmark tool
  Console scripts:
    litert-benchmark -> ai_edge_litert.tools.benchmark_litert_model:main
  extras_require:
    npu-intel -> openvino==2026.1.0
```

User-managed prerequisites (not in wheel): - `openvino==2026.1.0` (via `pip
install` or `[npu-intel]` extra) - Intel NPU driver (system package, requires
`sudo` / admin) - Level Zero loader v1.27.0 (Linux only, system package)
