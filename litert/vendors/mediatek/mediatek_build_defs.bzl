...
load("//third_party/odml/litert/litert/build_common:litert_build_defs.bzl", "append_rule_kwargs", "litert_lib", "make_rpaths")
...
def _litert_with_mtk_base(
    ...
    append_rule_kwargs(
        litert_rule_kwargs,
        data = select({
            "//third_party/tensorflow:linux_x86_64": _MTK_NEURON_ADAPTER_SO,
            "//conditions:default": [],
        }),
        linkopts = select({
            "//third_party/tensorflow:linux_x86_64": [make_rpaths(_MTK_HOST_RPATHS)],
            "//conditions:default": [],
        }),
    )
