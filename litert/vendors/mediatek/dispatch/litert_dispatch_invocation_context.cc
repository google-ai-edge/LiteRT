...
using litert::mediatek::NEURON_EXECUTION_CONFIG_JOB_PRIORITY;
using litert::mediatek::NEURON_EXECUTION_CONFIG_ORIGINAL_UID;
...
LoadModelAndCompilation(
  ...
  if (auto result = LoadFromCachedNetwork(neuron_adapter_api, bytecode_addr,
                                          bytecode_size);
      !result) {
    return LoadFromDlaBytecode(neuron_adapter_api, bytecode_addr, bytecode_size,
