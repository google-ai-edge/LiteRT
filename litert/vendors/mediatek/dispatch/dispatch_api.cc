...
// Static storage for NeuronAdapter API
static std::unique_ptr<litert::mediatek::NeuronAdapterApi> TheNeuronAdapterApi;
...
LiteRtStatus VendorTraits<MediaTekTag>::Initialize(const std::string& lib_dir) {
  if (!TheNeuronAdapterApi) {
    auto api = litert::mediatek::NeuronAdapterApi::Create();
    if (!api.ok()) {
  ...
  TheBuildId = absl::StrFormat(
      "NeuronAdapter | LiteRT Dispatch v%d.%d.%d",
...
VendorTraits<MediaTekTag>::CreateDeviceContext(
  ...
  auto result = litert::mediatek::LiteRtDispatchDeviceContextT::Create(
      *TheNeuronAdapterApi, *device_context_options);
...
LiteRtStatus VendorTraits<MediaTekTag>::RegisterTensorBuffer(
  ...
  auto* mtk_context = static_cast<litert::mediatek::LiteRtDispatchDeviceContextT*>(context);
...
VendorTraits<MediaTekTag>::CreateInvocationContext(
  ...
  auto* mtk_device_context = 
      static_cast<litert::mediatek::LiteRtDispatchDeviceContextT*>(device_context);
  ...
  // For now, we'll use default values - this should be updated based on actual schema parsing
  int num_inputs = 2;  // Default for testing
  int num_outputs = 1; // Default for testing
  
  auto result = litert::mediatek::LiteRtDispatchInvocationContextT::Create(
      *TheNeuronAdapterApi, *mtk_device_context,
      kLiteRtDispatchExecutableTypeMlModel,
      &mem_buffer, function_name, num_inputs, num_outputs);
...
template <>
LiteRtStatus VendorTraits<MediaTekTag>::GetInputRequirements(
  ...
  auto* mtk_invocation_context = 
      static_cast<litert::mediatek::LiteRtDispatchInvocationContextT*>(
          invocation_context);
  
  auto requirements = mtk_invocation_context->GetInputRequirements(
...
template <>
LiteRtStatus VendorTraits<MediaTekTag>::GetOutputRequirements(
    VendorInvocationContext* invocation_context,
    int output_index,
    const LiteRtRankedTensorType* tensor_type,
    LiteRtTensorBufferRequirements* tensor_buffer_requirements) {
  ...
  auto requirements = mtk_invocation_context->GetOutputRequirements(
