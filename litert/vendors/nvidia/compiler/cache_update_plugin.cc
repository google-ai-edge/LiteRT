  ...
  for (int i = 0; i < desc.dims.nbDims; ++i) {
    if (desc.dims.d[i] <= 0 ||
        static_cast<uint64_t>(desc.dims.d[i]) >
            static_cast<uint64_t>(std::numeric_limits<int64_t>::max()) / bytes ||
        static_cast<uint64_t>(desc.dims.d[i]) >
...
bool ValidDescriptors(const nvinfer1::PluginTensorDesc* inputs,
  ...
  for (int i = 0; i < kNumInputs; ++i) {
    if (!ValidDimensions(inputs[i].dims) ||
        ...
        inputs[i].type != (i == 4 ? nvinfer1::DataType::kINT32
  ...
  if (forward_read &&
      (ForwardBytes(inputs[5]) == 0 ||
       outputs[2].type != inputs[5].type ||
  ...
  const bool valid_values = transposed_value_cache
      ...
      : values.d[1] == cache.d[1] * cache.d[3] &&
            values.d[2] == cache.d[2] && values.d[3] == 1;
  return valid_values &&
         update.d[1] == cache.d[1] && update.d[2] <= cache.d[2] &&
         update.d[3] == cache.d[3] &&
 ...
 public:
  CacheUpdatePlugin(bool ring_buffer, bool transposed_value_cache,
      ...
      : ring_buffer_(ring_buffer),
        ...
        fields_{{{kRingField, &ring_buffer_, nvinfer1::PluginFieldType::kINT32, 1},
                 {kTransposedField, &transposed_value_cache_,
                  nvinfer1::PluginFieldType::kINT32, 1},
                 {kForwardField, &forward_read_,
                  nvinfer1::PluginFieldType::kINT32, 1}}},
        field_collection_{static_cast<int32_t>(fields_.size()), fields_.data()} {}
  ...
  nvinfer1::IPluginV3* clone() noexcept override {
    auto* copy = new (std::nothrow)
        CacheUpdatePlugin(ring_buffer_ != 0, transposed_value_cache_ != 0,
                           forward_read_ != 0);
  ...
  bool supportsFormatCombination(
    ...
    return in_out[position].desc.format == nvinfer1::TensorFormat::kLINEAR &&
           in_out[position].desc.type ==
               (position == 4 ? nvinfer1::DataType::kINT32
                              : nvinfer1::DataType::kHALF);
  }

  int32_t getOutputShapes(const nvinfer1::DimsExprs* inputs, int32_t num_inputs,
                          const nvinfer1::DimsExprs* /*shape_inputs*/,
                          int32_t num_shape_inputs,
    ...
    if (inputs == nullptr || outputs == nullptr ||
        ...
        inputs[2].nbDims != 4 ||
        (forward_read_ != 0 &&
         (inputs[5].nbDims < 0 || inputs[5].nbDims > nvinfer1::Dims::MAX_DIMS))) {
      return 1;
    }
    outputs[0] = inputs[0];
    outputs[1] = inputs[1];
  ...
  int32_t configurePlugin(const nvinfer1::DynamicPluginTensorDesc* inputs,
    ...
    if (!ValidDescriptors(in.data(), copied_inputs, out.data(), ring_buffer_ != 0,
  ...
  int32_t enqueue(const nvinfer1::PluginTensorDesc* input_desc,
    ...
    if (!ValidDescriptors(input_desc, num_inputs_, output_desc, ring_buffer_ != 0,
                          transposed_value_cache_ != 0, forward_read_ != 0) ||
...
class CacheUpdatePluginCreator final : public nvinfer1::IPluginCreatorV3One {
 public:
  CacheUpdatePluginCreator() noexcept
      : fields_{{{kRingField, nullptr, nvinfer1::PluginFieldType::kINT32, 1},
                 {kTransposedField, nullptr, nvinfer1::PluginFieldType::kINT32,
                  1},
  ...
  nvinfer1::IPluginV3* createPlugin(
    ...
    if (fields == nullptr || fields->fields == nullptr || fields->nbFields != 3) {
      return nullptr;
    }
    int32_t ring = -1;
    ...
    for (int i = 0; i < fields->nbFields; ++i) {
      ...
      if (std::strcmp(field.name, kRingField) == 0 && ring == -1) {
        ring = value;
      } else if (std::strcmp(field.name, kTransposedField) == 0 && transposed == -1) {
        transposed = value;
    ...
    return ring >= 0 && transposed >= 0 && forward >= 0
               ? CreateCacheUpdatePlugin(ring != 0, transposed != 0, forward != 0)
               : nullptr;
  }
...
nvinfer1::IPluginV3* CreateCacheUpdatePlugin(
    bool ring_buffer, bool transposed_value_cache, bool forward_read) noexcept {
