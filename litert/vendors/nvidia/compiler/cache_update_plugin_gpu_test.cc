...
void CheckSerializedNativeWriter(bool ring, bool transposed, int capacity,
  ...
  for (size_t i = 0; i < inputs.size(); ++i) {
    inputs[i] = network->addInput(
        kInputs[i], i == 4 ? nvinfer1::DataType::kINT32
  ...
  if (!ring) {
    ...
    auto* write = network->addSlice(*flat->getOutput(0), nvinfer1::Dims{1, {0}},
                                    nvinfer1::Dims{1, {1}},
                                    nvinfer1::Dims{1, {1}});
  ...
  for (int i = 0; i < 2; ++i) {
    auto* writer = network->addKVCacheUpdate(
        *inputs[i], *layer->getOutput(i), *anchor,
  ...
  constexpr std::array<uint16_t, 4> kInitialBits = {
      0x3c00, 0x4000, 0x4200, 0x4400};  // HALF 1,2,3,4.
  constexpr std::array<uint16_t, 4> kNewKeyBits = {
      0x4500, 0x4600, 0x4700, 0x4800};  // HALF 5,6,7,8.
  ...
  const std::array<std::array<int32_t, 2>, 12> cases = {{
      {0, rows - 1}, {rows - 1, rows}, {capacity - 2, 3},
      {capacity + 1, 2}, {capacity - 1, 0}, {-1, 2}, {0, rows + 1},
      {0, -1}, {capacity - 1, 1}, {0, rows},
      {std::numeric_limits<int32_t>::max(), 2},
      {std::numeric_limits<int32_t>::max() - 1, rows}}};
  int invocation = 0;
  for (const auto& [write, valid] : cases) {
    SCOPED_TRACE(::testing::Message() << "invocation=" << invocation
    ...
    for (size_t i = 0; i < update_count; ++i) {
      ...
      update_k[i] = row >= valid ? 0x7bff
                                : kNewKeyBits[(i + invocation) % 4];
      update_v[i] = row >= valid ? 0x7bff
                                : kNewValueBits[(i + invocation) % 4];
    }
    const std::array<float, 2> expected_old_sums = {
        SumHalfValues(expected_k), SumHalfValues(expected_v)};
      ...
      for (int h = 0; h < kHeads; ++h) {
        for (int row = 0; row < valid; ++row) {
          ...
          for (int c = 0; c < kDepth; ++c) {
            ...
            const size_t v = transposed ? k
                : (h * kDepth + c) * capacity + destination;
...
TEST(CacheUpdatePluginGpuTest, NativeLinearWriterPreservesPaddingAndOldReaders) {
  for (bool transposed : {false, true}) {
