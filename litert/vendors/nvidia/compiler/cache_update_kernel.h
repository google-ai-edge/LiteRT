...
// patches contain update_rows rows at clamp(params[0], 0,
// cache_rows-update_rows). Invalid parameters (negative write or length outside
// [0, update_rows]) and zero length produce the unchanged old patch. Cache
// inputs are never mutated. V cache/patch storage is BHSD when transposed,
// otherwise flattened BHDS; both fresh updates are BHSD. Launching never
// synchronizes the supplied stream. An optional even-sized forward buffer is
// copied bitwise in that same launch. Forward output and patches must not
// overlap each other or any read buffer.
