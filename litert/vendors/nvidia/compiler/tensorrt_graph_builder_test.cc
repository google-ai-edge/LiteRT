...
TEST(TensorRtGraphBuilderTest, CudaSubbyteGemvGroupSharesOneLaunch) {
  ...
  for (bool fuse : {true, false}) {
    ...
    for (size_t f = 0; f < rows.size(); ++f) {
      ...
      ASSERT_EQ(
          cudaMemcpy(actual.data(), device_outputs[f],
                     actual.size() * sizeof(float), cudaMemcpyDeviceToHost),
          cudaSuccess);
      for (int32_t n = 0; n < rows[f]; ++n) {
        float expected = 0.0f;
...
TEST(TensorRtGraphBuilderTest, Fp16SoftCapAfterCudaSubbyteGemv) {
  ...
  SetWeightsFromUnownedBuffer(
      cap.Weights(),
      litert::BufferRef<uint8_t>(reinterpret_cast<const uint8_t*>(&kHalf30),
                                 sizeof(kHalf30)));
  ...
  ASSERT_TRUE(
      context->setTensorAddress(built->input_names[0].c_str(), device_input));
  ASSERT_TRUE(
      context->setTensorAddress(built->output_names[0].c_str(), device_output));
  cudaStream_t stream = nullptr;
  ASSERT_EQ(cudaStreamCreate(&stream), cudaSuccess);
  ASSERT_TRUE(context->enqueueV3(stream));
  ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
...
TEST(TensorRtGraphBuilderTest, InPlaceKvCacheUpdate) {
  ...
  for (const auto& test_case : cases) {
    ...
    update.SetType(
        MakeRankedTensorType(kLiteRtElementTypeFloat16, test_case.update_dims));
    ...
    ASSERT_EQ(cudaMemcpy(device_update, update_host.data(), update_elements * 2,
                         cudaMemcpyHostToDevice),
              cudaSuccess);
    if (test_case.native) {
    ...
    ASSERT_TRUE(
        context->setTensorAddress(built->input_names[0].c_str(), device_cache));
    ASSERT_TRUE(context->setTensorAddress(built->input_names[1].c_str(),
                                          device_update));
    ASSERT_TRUE(
        context->setTensorAddress(built->input_names[2].c_str(), device_start));
    ASSERT_TRUE(context->setTensorAddress(built->output_names[0].c_str(),
                                          device_output));
    cudaStream_t stream = nullptr;
    ASSERT_EQ(cudaStreamCreate(&stream), cudaSuccess);
    ...
    for (int32_t position :
         {std::numeric_limits<int32_t>::min(), -1, 0, 5, max_start,
          max_start + 1, std::numeric_limits<int32_t>::max()}) {
          ...
          for (int d = 0; d < depth; ++d) {
            const uint16_t expected =
                seq >= clamped_start && seq < clamped_start + update_length
                    ...
                    : 0;
            const size_t index = seq_major ? (h * seq_len + seq) * depth + d
...
TEST(TensorRtGraphBuilderTest, Fp16CacheUpdateSupportRequiresPrefillReaders) {
  ...
  for (const auto& test_case : cases) {
    SCOPED_TRACE(testing::Message() << "rows=" << test_case.rows << " reader="
                                    << static_cast<int>(test_case.reader)
    ...
    auto& update_k =
        tensor(kLiteRtElementTypeFloat16, {1, 2, test_case.rows, 4});
    auto& update_v =
        tensor(kLiteRtElementTypeFloat16, {1, 2, test_case.rows, 4});
...
TEST(TensorRtGraphBuilderTest,
     Fp16CompositeCacheUpdatePreservesAttentionReaders) {
  ...
  for (bool ring : {false, true}) {
    for (bool transposed : {false, true}) {
      ...
      auto& update_k =
          tensor("update_k", kLiteRtElementTypeFloat16, update_dims);
      auto& update_v =
          tensor("update_v", kLiteRtElementTypeFloat16, update_dims);
      ...
      auto& query =
          tensor("query", kLiteRtElementTypeFloat16, {1, kHeads, 1, kDepth});
      ...
      const auto reader = [&](LiteRtTensorT& lhs, LiteRtTensorT& rhs,
        ...
        auto& output =
            tensor(name, kLiteRtElementTypeFloat16, {1, kHeads, 1, width});
      ...
      EXPECT_EQ(
          ...
          transposed);
      EXPECT_EQ(
          litert::nvidia::IsTransposedValueCacheTensor(built->output_names[1]),
          transposed);
      EXPECT_FALSE(
          litert::nvidia::IsTransposedValueCacheTensor(built->input_names[3]));
      ...
      const auto v_shape =
          engine->getTensorShape(built->input_names[4].c_str());
      ...
      ASSERT_EQ(cudaMemcpy(inputs[8], probability_host.data(), input_sizes[8],
                           cudaMemcpyHostToDevice),
                cudaSuccess);
      // The FP16 composite must use params, not its decomposition's start
      // inputs.
      ...
      // The ring cases cross the end of the cache. Unused rows contain
      // sentinels.
      for (const auto [write_index, valid_length] :
           {std::pair<int, int>{2, 0},
            {2, kRows},
            {ring ? kSeq - 1 : kSeq - 3, 2},
            {ring ? kSeq - 1 : kSeq - kRows, kRows},
            {kSeq - 1, 1}}) {
        SCOPED_TRACE(testing::Message()
                     << "write=" << write_index << " valid=" << valid_length);
        ...
        for (int h = 0; h < kHeads; ++h) {
          for (int s = 0; s < kSeq; ++s) {
            for (int d = 0; d < kDepth; ++d) {
              ...
              old_v[v_index(h, s, d)] = Fp16Bits(100.0f + h * 16 + s * 2 + d);
            }
          }
        }
        ...
        for (int h = 0; h < kHeads; ++h) {
          for (int row = 0; row < kRows; ++row) {
            for (int d = 0; d < kDepth; ++d) {
              ...
              if (row < valid_length) {
                const int s =
                    ring ? (write_index + row) % kSeq : write_index + row;
...
TEST(TensorRtGraphBuilderTest, PrefillAttentionJoinSurvivesCacheUpdate) {
  ...
  for (bool transposed : {false, true}) {
    for (JoinUse use : {JoinUse::kClosed, JoinUse::kEscapingScores,
      ...
      auto& fresh_scores = matmul(query, update_k, "fresh_scores", kRows, true);
      ...
      const uint16_t negative_infinity =
          Fp16Bits(-std::numeric_limits<float>::infinity());
      ...
      EXPECT_EQ(
          layers.find("cache_update_forward_attention") != std::string::npos,
          forward_join)
          << layers;
      ...
      // padding, a wrap, a partial wrap, zero length, and invalid update
      // params.
      for (const auto [write, valid] : {std::pair<int, int>{0, kRows - 1},
                                        {kRows - 1, kRows},
                                        {kSeq - 1, 2},
                                        {0, 0},
                                        {-1, 2},
          ...
          for (int r = 0; r < kRows; ++r) {
            ...
            if (use == JoinUse::kEscapingIntermediate) {
              for (int s = 0; s < kColumns; ++s) {
                ...
                EXPECT_NEAR(
                    Fp16ToFloat(actual[2][(h * kRows + r) * kColumns + s]),
            ...
            for (int d = 0; d < kDepth; ++d) {
              ...
              EXPECT_NEAR(
                  Fp16ToFloat(actual.back()[(h * kRows + r) * kDepth + d]),
                  reference, 0.02f)
                  << "h=" << h << " row=" << r << " d=" << d;
              if (use == JoinUse::kEarlyConsumer ||
                  use == JoinUse::kMultipleConsumers ||
                ...
                EXPECT_NEAR(
                    Fp16ToFloat(actual[2][(h * kRows + r) * kDepth + d]),
...
TEST(TensorRtGraphBuilderTest, TransposedCacheMatmul) {
  ...
  for (const auto& test_case : cases) {
    ...
    const int contracted =
        test_case.transposed_reader ? cache_dims[3] : cache_dims[2];
    const int produced =
        test_case.transposed_reader ? cache_dims[2] : cache_dims[3];
    LiteRtModelT model;
    auto& graph = model.EmplaceSubgraph();
    auto& cache = graph.EmplaceTensor();
    ...
    litert::internal::AttachInput(test_case.read_only ? &cache : &cache_out,
                                  matmul);
    if (test_case.composite) {
    ...
    EXPECT_EQ(
        litert::nvidia::IsTransposedValueCacheTensor(built->input_names[0]),
        seq_major && !test_case.key);
    if (!test_case.read_only) {
      EXPECT_EQ(
          litert::nvidia::IsTransposedValueCacheTensor(built->output_names[0]),
          seq_major && !test_case.key);
    }
    ...
    ASSERT_EQ(
        cudaMalloc(&device_result, kHeads * test_case.rows * produced * 2),
        cudaSuccess);
    ASSERT_EQ(cudaMemcpy(device_cache, cache_host.data(), cache_elements * 2,
                         cudaMemcpyHostToDevice),
              cudaSuccess);
    ...
    ASSERT_TRUE(context->setTensorAddress(
        built->output_names[test_case.read_only ? 0 : 1].c_str(),
        device_result));
    cudaStream_t stream = nullptr;
    ASSERT_EQ(cudaStreamCreate(&stream), cudaSuccess);
    ASSERT_TRUE(context->enqueueV3(stream));
    ...
    for (int h = 0; h < kHeads; ++h) {
      ...
      for (int row = 0; row < test_case.rows; ++row) {
        for (int i = 0; i < produced; ++i) {
          ...
          EXPECT_NEAR(
              Fp16ToFloat(
                  result_actual[(h * test_case.rows + row) * produced + i]),
              expected, 0.02f)
              << "h=" << h << " row=" << row << " i=" << i;
        }
      }
...
TEST(TensorRtGraphBuilderTest, RejectsInvalidReadOnlyValueCacheContract) {
  ...
  for (size_t i = 0; i < cases.size(); ++i) {
    ...
    auto& output = graph.EmplaceTensor();
    output.SetType(
        MakeRankedTensorType(kLiteRtElementTypeFloat16, {1, 2, 1, 8}));
    graph.Outputs().push_back(&output);
    auto& matmul = graph.EmplaceOp();
    matmul.SetOpCode(kLiteRtOpCodeTflBatchMatmul);
  ...
  for (const auto& test_case : cases) {
    ...
    for (const bool plugin : {true, false}) {
      ...
      ASSERT_TRUE(context->setTensorAddress(built->output_names[1].c_str(),
                                            d_v));  // in place
      ASSERT_TRUE(
          context->setTensorAddress(built->output_names[2].c_str(), d_out));
      cudaStream_t stream = nullptr;
      ASSERT_EQ(cudaStreamCreate(&stream), cudaSuccess);
      ASSERT_TRUE(context->enqueueV3(stream));
      ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
      ...
      for (size_t i = 0; i < expected.size(); ++i) {
        max_error =
            std::max(max_error, std::abs(Fp16ToFloat(actual[i]) - expected[i]));
      }
      EXPECT_LT(max_error, 0.03f);
      EXPECT_EQ(cudaStreamDestroy(stream), cudaSuccess);
      for (void* ptr :
           {d_out, d_pos, d_mask, d_q, d_vs, d_ks, d_vu, d_ku, d_v, d_k}) {
...
TEST(TensorRtGraphBuilderTest, RuntimeBmmResultConcatenation) {
  ...
  for (size_t i = 0; i < cache_host.size(); ++i) {
    cache_host[i] =
        Fp16Bits(static_cast<float>(static_cast<int>(i % 7) - 3) * 0.5f);
  }
  ...
  ASSERT_EQ(
      cudaMemcpy(d_q, q_host.data(), q_host.size() * 2, cudaMemcpyHostToDevice),
      cudaSuccess);
  ...
  ASSERT_TRUE(context->setTensorAddress(built->output_names[0].c_str(), d_out));
  cudaStream_t stream = nullptr;
  ASSERT_EQ(cudaStreamCreate(&stream), cudaSuccess);
  ASSERT_TRUE(context->enqueueV3(stream));
  ASSERT_EQ(cudaStreamSynchronize(stream), cudaSuccess);
    ...
    for (int r = 0; r < kRows; ++r) {
      for (int j = 0; j < kSeq + kChunk; ++j) {
        ...
        if (j < kSeq) {
          ...
          expected =
              Fp16ToFloat(chunk_host[(h * kRows + r) * kChunk + j - kSeq]);
        }
        const float value =
