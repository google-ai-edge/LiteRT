 ...
 protected:
  void SetUp() override {
    ...
    runtime_ = static_cast<nvinfer1::IPluginV3OneRuntime*>(
        plugin_->getCapabilityInterface(nvinfer1::PluginCapabilityType::kRUNTIME));
    ...
    for (int i = 0; i < inputs_.size(); ++i) {
      inputs_[i].type = i == 4 ? nvinfer1::DataType::kINT32
...
TEST_F(CacheUpdatePluginTest, ProducesContiguousPatchesWithStandardBuildInterface) {
  ...
  for (bool ring : {false, true}) {
    for (bool transposed : {false, true}) {
      ...
      runtime_ = static_cast<nvinfer1::IPluginV3OneRuntime*>(
          plugin_->getCapabilityInterface(nvinfer1::PluginCapabilityType::kRUNTIME));
      inputs_[1].dims = transposed ? inputs_[0].dims
                                  : nvinfer1::Dims4{1, 2048, 1152, 1};
      outputs_ = {inputs_[0], inputs_[1]};
...
TEST_F(CacheUpdatePluginTest, AcceptsOpaqueFloatingPointOrderingInputs) {
  const std::array<nvinfer1::DataType, 3> types = {
      nvinfer1::DataType::kFLOAT, nvinfer1::DataType::kHALF,
      nvinfer1::DataType::kBF16};
  const std::array<nvinfer1::Dims, 3> dims = {
  ...
  auto* build = static_cast<nvinfer1::IPluginV3OneBuild*>(
      plugin_->getCapabilityInterface(nvinfer1::PluginCapabilityType::kBUILD));
  ASSERT_NE(build, nullptr);
  std::vector<nvinfer1::DynamicPluginTensorDesc> descriptors(inputs_.size() + 2);
  ...
  for (int i = 0; i < descriptors.size(); ++i) {
    EXPECT_TRUE(build->supportsFormatCombination(
        i, descriptors.data(), inputs_.size(), 2)) << i;
  }
  EXPECT_EQ(build->configurePlugin(descriptors.data(), inputs_.size(),
                                   descriptors.data() + inputs_.size(), 2), 0);
  std::array<nvinfer1::DataType, 2> output_types;
  EXPECT_EQ(build->getOutputDataTypes(output_types.data(), 2, input_types.data(),
                                      input_types.size()), 0);
  ...
  EXPECT_FALSE(build->supportsFormatCombination(
      5, descriptors.data(), inputs_.size(), 2));
...
TEST_F(CacheUpdatePluginTest, RequiresOrderingBuffersBeforeLaunchAndAfterClone) {
...
TEST_F(CacheUpdatePluginTest, ForwardsFloatingPointShapeAndTypeWithExactCounts) {
  ...
  for (const auto type : types) {
    for (const auto& shape : shapes) {
      ...
      for (int i = 0; i < 9; ++i) {
        EXPECT_TRUE(build->supportsFormatCombination(i, descriptors.data(), 6, 3));
      }
      EXPECT_EQ(build->configurePlugin(descriptors.data(), 6,
                                       descriptors.data() + 6, 3), 0);
      EXPECT_EQ(build->getOutputDataTypes(output_types.data(), 3,
                                          input_types.data(), 6), 0);
...
TEST_F(CacheUpdatePluginTest, RejectsMalformedAndOverflowingForwardDescriptors) {
  plugin_.reset(CreateCacheUpdatePlugin(true, true, /*forward_read=*/true));
  runtime_ = static_cast<nvinfer1::IPluginV3OneRuntime*>(
      plugin_->getCapabilityInterface(nvinfer1::PluginCapabilityType::kRUNTIME));
  inputs_.push_back(inputs_[0]);
  ...
  for (const auto shape : {
           nvinfer1::Dims{-1, {}}, nvinfer1::Dims{9, {}},
           nvinfer1::Dims{1, {0}}, nvinfer1::Dims{1, {-1}},
...
TEST_F(CacheUpdatePluginTest, RoundTripsFieldsAndRejectsInvalidSerialization) {
    ...
    for (bool transposed : {false, true}) {
      for (bool forward : {false, true}) {
        ...
        auto* runtime = static_cast<nvinfer1::IPluginV3OneRuntime*>(
            source->getCapabilityInterface(nvinfer1::PluginCapabilityType::kRUNTIME));
        std::unique_ptr<nvinfer1::IPluginV3> copy(creator->createPlugin(
            "copy", runtime->getFieldsToSerialize(),
            nvinfer1::TensorRTPhase::kRUNTIME));
        ASSERT_NE(copy, nullptr);
        source.reset();
        auto* copied_runtime = static_cast<nvinfer1::IPluginV3OneRuntime*>(
            copy->getCapabilityInterface(nvinfer1::PluginCapabilityType::kRUNTIME));
        ...
        EXPECT_EQ(*static_cast<const int32_t*>(fields->fields[1].data), transposed);
        EXPECT_EQ(*static_cast<const int32_t*>(fields->fields[2].data), forward);
        auto* build = static_cast<nvinfer1::IPluginV3OneBuild*>(
            copy->getCapabilityInterface(nvinfer1::PluginCapabilityType::kBUILD));
        EXPECT_EQ(build->getNbOutputs(), forward ? 3 : 2);
      }
    }
  }
  EXPECT_EQ(creator->createPlugin("bad", nullptr, nvinfer1::TensorRTPhase::kRUNTIME),
            nullptr);
  int32_t invalid = 2;
  std::array<nvinfer1::PluginField, 3> fields = {{{
      "ring_buffer", &invalid, nvinfer1::PluginFieldType::kINT32, 1}, {
      "transposed_value_cache", &invalid, nvinfer1::PluginFieldType::kINT32, 1}, {
  ...
  EXPECT_EQ(creator->createPlugin("bad", &collection,
                                  nvinfer1::TensorRTPhase::kRUNTIME), nullptr);
  int32_t valid = 0;
