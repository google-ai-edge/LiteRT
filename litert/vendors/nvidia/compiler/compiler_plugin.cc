...
constexpr uint32_t kNvidiaCompilerCacheSchemaVersion = 1;
...
Expected<bool> ExistingArtifactMatches(const std::string& path,
  ...
  *exists = true;
  ...
  if (mapping == MAP_FAILED) {
    return Error(kLiteRtStatusErrorFileIO,
                 "Failed to map TensorRT AOT artifact " + path + ": " +
                     std::strerror(mmap_errno));
  }
  const bool matches = std::memcmp(mapping, data, size) == 0;
  munmap(mapping, size);
  return matches;
}
...
Expected<void> PersistArtifactFile(const std::string& path, const uint8_t* data,
                                   size_t size) {
  bool exists = false;
  LITERT_ASSIGN_OR_RETURN(bool matches,
                          ExistingArtifactMatches(path, data, size, &exists));
  if (matches) {
    return {};
  }
  if (exists) {
  ...
  if (close(fd) != 0) {
    ...
    unlink(temporary_path.c_str());
    return Error(kLiteRtStatusErrorFileIO,
                 "Failed to close TensorRT AOT artifact " + temporary_path +
                     ": " + std::strerror(saved_errno));
  }
  ...
  if (link(temporary_path.c_str(), path.c_str()) != 0) {
    ...
    LITERT_ASSIGN_OR_RETURN(
        bool concurrent_matches,
        ExistingArtifactMatches(path, data, size, &concurrent_exists));
    if (!concurrent_exists || !concurrent_matches) {
      return Error(kLiteRtStatusErrorFileIO,
  ...
  for (size_t i = 0; i < bytecodes.size(); ++i) {
    ...
    LITERT_ASSIGN_OR_RETURN(
        ...
        litert::nvidia::PackTensorRtAotLocator(
            {path, static_cast<uint64_t>(artifact_bytes), fingerprint}));
    total_artifact_bytes += artifact_bytes;
    total_locator_bytes += locator.size();
    LITERT_LOG(LITERT_INFO,
               "NVIDIA TensorRT-RTX persisted AOT artifact %zu/%zu: "
               "artifact_bytes=%zu locator_bytes=%zu path=%s",
               i + 1, bytecodes.size(), artifact_bytes, locator.size(),
               path.c_str());
...
LiteRtStatus LiteRtCompilerPluginCompile(
  ...
  if (!aot_cache_dir.empty()) {
    litert::nvidia::LogMemoryProfile("compiler", "aot_cache_lookup_begin",
                                     soc_model);
    ...
    if (aot_cache_key.has_value()) {
      ...
      if (cached_manifest.has_value()) {
        ...
        if (call_layout_matches) {
          ...
          litert::nvidia::LogMemoryProfile("compiler", "aot_cache_hit",
                                           soc_model);
      ...
      LITERT_LOG(LITERT_INFO, "NVIDIA TensorRT-RTX AOT cache miss: index=%s",
                 aot_manifest_path.c_str());
      litert::nvidia::LogMemoryProfile("compiler", "aot_cache_miss", soc_model);
    } else {
      LITERT_LOG(
          LITERT_INFO,
  ...
  if (!aot_cache_dir.empty()) {
    litert::nvidia::LogMemoryProfile("compiler", "aot_persist_begin",
                                     soc_model);
    LITERT_RETURN_IF_ERROR(
        PersistAotArtifacts(aot_cache_dir, result->bytecodes));
    litert::nvidia::LogMemoryProfile("compiler", "aot_persist_end", soc_model);
    if (aot_cache_key.has_value()) {
      ...
      litert::nvidia::LogMemoryProfile("compiler", "aot_index_published",
                                       soc_model);
    }
  }
