...
std::string BuildCompilerSdkVersion() {
  const std::string base_version =
      ...
      std::to_string(NV_TENSORRT_PATCH) + " cache-schema=" +
      std::to_string(kNvidiaCompilerCacheSchemaVersion);
...
Expected<std::vector<std::string>> ReadOnlyValueCacheInputs(
  ...
  for (uint32_t i = 0; i < layout->num_inputs; ++i) {
    if (layout->input_names[i] == nullptr || layout->input_names[i][0] == '\0') {
      return Error(kLiteRtStatusErrorInvalidArgument,
...
LiteRtStatus LiteRtCompilerPluginCompile(
  ...
  for (LiteRtParamIndex i = 0; i < num_partitions; ++i) {
    ...
    if (shared_weights) {
      ...
      auto& weight_store = aot_cache_dir.empty() ? shared_weight_store
                                                : shard_weight_store;
      ...
      for (auto& weight : engine.refit_weights) {
        ...
        const uint32_t shared_index =
            weight_store.Add(std::move(weight));
  ...
  if (aot_artifacts_persisted) {
    LITERT_LOG(LITERT_INFO,
               ...
               "streamed=1",
               result->bytecodes.size(), total_bytecode_bytes,
               total_locator_bytes, total_shard_weight_bytes);
