...
TEST(ProfilingTest, CpuTimerElapsedMillisecondsAreNondecreasing) {
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    const double upper_bound_ms =
        std::chrono::duration<double, std::milli>(after_sample - before).count();
...
TEST(ProfilingTest, MemoryLogTimestampsUseTheMonotonicClock) {
  ...
  ASSERT_EQ(std::sscanf(output.c_str() + first_pos, "monotonic_ns=%llu",
                        &first_ns),
            1);
  ASSERT_EQ(std::sscanf(output.c_str() + second_pos, "monotonic_ns=%llu",
                        &second_ns),
            1);
