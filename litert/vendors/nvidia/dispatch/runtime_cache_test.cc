 ...
 protected:
  void SetUp() override {
    ...
    ASSERT_TRUE(profile->setDimensions(
        "input", nvinfer1::OptProfileSelector::kMIN,
        nvinfer1::Dims2{1, kWidth}));
    ASSERT_TRUE(profile->setDimensions(
  ...
  void RunShape(int rows) {
    ...
    ASSERT_EQ(cudaMemcpyAsync(output.data(), output_,
  ...
  void Persist(const std::vector<uint8_t>& bytes) {
    auto saved = PersistValidatedRuntimeCache(
        *config_, bytes.data(), bytes.size(), path_);
...
TEST_F(RuntimeCacheTest, RenameFailurePreservesDestinationAndRemovesTemporary) {
  ...
  auto initial = PersistValidatedRuntimeCache(
      *config_, candidate_.data(), candidate_.size(), previous_path);
