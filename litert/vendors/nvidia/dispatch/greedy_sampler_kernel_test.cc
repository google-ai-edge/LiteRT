...
int RunGpuArgMax(const std::vector<float>& values, bool fp16 = false) {
  ...
  if (cudaMalloc(&device_values, input_bytes) != cudaSuccess ||
      ...
      cudaMalloc(reinterpret_cast<void**>(&device_result), sizeof(int32_t)) !=
          cudaSuccess ||
      cudaMemcpy(device_values, input, input_bytes, cudaMemcpyHostToDevice) !=
          cudaSuccess) {
  ...
  const cudaError_t launch_status =
      fp16 ? LiteRtNvidiaLaunchF16ArgMax(
                 static_cast<const uint16_t*>(device_values), values.size(),
                 workspace, workspace_bytes, device_result, /*stream=*/nullptr)
           : LiteRtNvidiaLaunchF32ArgMax(
                 static_cast<const float*>(device_values), values.size(),
                 workspace, workspace_bytes, device_result, /*stream=*/nullptr);
  int32_t result = -1;
