...
class AotEngineResidencyTest : public ::testing::Test {
 protected:
  ...
  void TearDown() override {
    ...
    if (device_) {
      EXPECT_EQ(api_.interface->device_context_destroy(device_), kLiteRtStatusOk);
    }
  ...
  Expected<std::vector<uint8_t>> BuildBytecode(const std::string& name,
    ...
    biases.fill(bias);
    nvinfer1::Weights weights{nvinfer1::DataType::kFLOAT, biases.data(), kElements};
    auto* input = network->addInput("input", nvinfer1::DataType::kFLOAT,
                                     nvinfer1::Dims{1, {kElements}});
    auto* constant = network->addConstant(nvinfer1::Dims{1, {kElements}}, weights);
    if (!input || !constant) {
    ...
    if (stripped) {
      if (!network->setWeightsName(weights, "bias") ||
        ...
        return Error(kLiteRtStatusErrorRuntimeFailure, "Cannot configure refit");
      }
    ...
    TensorRtSharedWeight shared_weight{TensorRtWeightDataType::kFloat, kElements,
                                        std::vector<uint8_t>(kBytes)};
    std::memcpy(shared_weight.data.data(), biases.data(), kBytes);
    TensorRtBundleEntry entry{name, {"input"}, {"output"}, serialized->data(),
                                serialized->size(), nullptr, {{"bias", 0}}};
  ...
  Expected<std::vector<uint8_t>> Locator(const std::string& path,
    ...
    locator.file_identity = TensorRtAotFileIdentity{
  ...
  void CreateDevice() {
    ASSERT_EQ(api_.interface->device_context_create(&runtime_, nullptr, &device_),
              kLiteRtStatusOk);
  ...
  void Bind(LiteRtDispatchInvocationContext context) {
    ...
    ASSERT_EQ(LiteRtGetTensorBufferRequirementsSupportedTensorBufferType(
                  requirements, 0, &first_type), kLiteRtStatusOk);
  ...
  void Run(LiteRtDispatchInvocationContext context, float input_base, float bias) {
  ...
  Expected<std::vector<uint8_t>> BuildValueCacheBytecode() {
    ...
    auto* cache = network->addInput(input_name.c_str(), nvinfer1::DataType::kFLOAT,
    ...
    auto* layer = network->addKVCacheUpdate(
        *cache, *update, *position, nvinfer1::KVCacheMode::kLINEAR);
  ...
  void CheckValueCacheContract(bool lazy) {
    ...
    ASSERT_EQ(api_.interface->register_tensor_buffer(
                  device_, update_.handle(), &update_handle), kLiteRtStatusOk);
    ASSERT_EQ(api_.interface->register_tensor_buffer(
                  device_, position_.handle(), &position_handle), kLiteRtStatusOk);

    auto cache_bytes = BuildValueCacheBytecode();
    ASSERT_TRUE(cache_bytes) << cache_bytes.Error().Message();
    auto parsed = ParseTensorRtBytecode(cache_bytes->data(), cache_bytes->size(),
                                        "cache_a");
    ...
    ASSERT_EQ(api_.interface->register_tensor_buffer(
              ...
              kLiteRtStatusOk);
    ASSERT_EQ(api_.interface->register_tensor_buffer(
                  device_, native.handle(), &native_handle), kLiteRtStatusOk);
    EXPECT_EQ(api_.interface->register_tensor_buffer(
                  device_, invalid.handle(), &invalid_handle),
              kLiteRtStatusErrorUnsupported);
    for (auto context : {a_, b_}) {
      for (bool output : {false, true}) {
        const auto get_requirements = output
            ? api_.interface->get_output_requirements
            : api_.interface->get_input_requirements;
        LiteRtTensorBufferRequirements requirements = nullptr;
        ASSERT_EQ(get_requirements(context, 0, &unannotated.type, &requirements),
                  kLiteRtStatusOk);
        int num_strides = 0;
        const uint32_t* strides = nullptr;
        ASSERT_EQ(LiteRtGetTensorBufferRequirementsStrides(
                      requirements, &num_strides, &strides), kLiteRtStatusOk);
        ...
        ASSERT_EQ(LiteRtGetTensorBufferRequirementsBufferSize(
                      requirements, &required_bytes), kLiteRtStatusOk);
    ...
    for (int round = 0; round < 3; ++round) {
      ...
      ASSERT_EQ(cudaMemcpy(result.data(), same_buffer ? input_.device_ptr
                                                     : output_.device_ptr,
                             kBytes, cudaMemcpyDeviceToHost), cudaSuccess);
      for (int h = 0; h < 2; ++h) {
        for (int s = 0; s < 4; ++s) {
          for (int d = 0; d < 4; ++d) {
            const int offset = h * 16 + s * 4 + d;
            EXPECT_FLOAT_EQ(result[offset], s == position ? update[h * 4 + d]
                                                          : cache[offset]);
          }
        }
      }
    ...
    EXPECT_EQ(api_.interface->unregister_tensor_buffer(device_, native_handle),
              kLiteRtStatusOk);
    EXPECT_EQ(api_.interface->unregister_tensor_buffer(device_,
                                                       unannotated_handle),
              kLiteRtStatusOk);
  }
...
TEST_F(AotEngineResidencyTest, ReloadsBothPlainAndStrippedPlansWithSameBuffers) {
  PreparePair();
  ASSERT_FALSE(HasFatalFailure());
  for (int round = 0; round < 3; ++round) {
...
TEST_F(AotEngineResidencyTest, MissingAndChangedArtifactsFailClosedAndCanRetry) {
  PreparePair();
  ASSERT_FALSE(HasFatalFailure());
  Run(a_, 0.0f, 1.0f);
  Run(b_, 0.0f, 2.0f);
...
TEST_F(AotEngineResidencyTest, RejectsNonAotBacking) {
  ...
  EXPECT_EQ(CreateContext(*bytes, "a", &context), kLiteRtStatusErrorUnsupported);
}
...
TEST_F(AotEngineResidencyTest, RefitFailureReleasesEngineAndAllowsOtherInvocations) {
  PreparePair();
  ASSERT_FALSE(HasFatalFailure());
  ...
  TensorRtBundleEntry entry{"bad", parsed->input_names, parsed->output_names,
                              parsed->engine_data, parsed->engine_size,
                              nullptr, {{"bias", 0}}};
