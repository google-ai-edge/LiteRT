...
class GreedySamplerCApiTest : public testing::Test {
 protected:
  ...
  void TearDown() override {
    ...
    // Do not leave the module pointing to this fixture's runtime function table.
  ...
  void SetLogits(const std::vector<float>& values, int rows, bool fp16,
    ...
    logits_.packed_size = values.size() * (fp16 ? sizeof(__half) : sizeof(float));
...
TEST_F(GreedySamplerCApiTest, PreservesNanInfinityAndFp16TieRulesPerRow) {
  ...
  for (bool fp16 : {false, true}) {
    SetLogits({nan, inf, 3, 4, -1, nan, inf, inf,
               -inf, -inf, -inf, -inf, -4, 9, 2, 9}, 4, fp16);
    ASSERT_FALSE(HasFatalFailure());
  ...
  EXPECT_EQ(id, 0);  // The first two distinct FP32 values round to the same FP16.
}
...
TEST_F(GreedySamplerCApiTest, RetainsSingleRowFp32Abi) {
  ...
  ASSERT_EQ(LiteRtDispatchNvidiaGreedySamplerSampleF32(
                sampler_, logits_.handle(), 4, &id), kLiteRtStatusOk);
  EXPECT_EQ(id, 1);
  SetLogits({-4, 7, 7, 2}, 1, true);
  ASSERT_FALSE(HasFatalFailure());
  EXPECT_EQ(LiteRtDispatchNvidiaGreedySamplerSampleF32(
                sampler_, logits_.handle(), 4, &id), kLiteRtStatusErrorUnsupported);
  EXPECT_EQ(id, 1);
  SetLogits({-4, 7, 7, 2}, 2, false);
  ASSERT_FALSE(HasFatalFailure());
  EXPECT_EQ(LiteRtDispatchNvidiaGreedySamplerSampleF32(
                sampler_, logits_.handle(), 2, &id), kLiteRtStatusErrorUnsupported);
}
...
TEST_F(GreedySamplerCApiTest, RejectsUnsupportedTypesLayoutsAndNonDeviceMemory) {
  SetLogits({-4, 7, 7, 2}, 1, false);
  ASSERT_FALSE(HasFatalFailure());
  const auto original = logits_;
  int32_t id = -123;
  for (LiteRtElementType type : {kLiteRtElementTypeBFloat16,
...
TEST_F(GreedySamplerCApiTest, RejectsInvalidArgumentsWithoutWritingIds) {
  ...
  EXPECT_EQ(LiteRtDispatchNvidiaGreedySamplerSampleBatched(
                sampler_, nullptr, 1, 2, &id), kLiteRtStatusErrorInvalidArgument);
  EXPECT_EQ(LiteRtDispatchNvidiaGreedySamplerSampleBatched(
...
TEST_F(GreedySamplerCApiTest, RejectsChangingDeviceWithoutChangingCurrentDevice) {
  int device_count = 0;
...
TEST_F(GreedySamplerCApiTest, WaitsForProducerEventAndPropagatesWaitFailure) {
  ...
  ASSERT_EQ(cudaMemcpyAsync(logits_.allocation, updated.data(), logits_.packed_size,
