...
LiteRtStatus LiteRtDispatchDeviceContextT::Destroy() {
  if (!registered_graphs_.empty()) {
    LITERT_LOG(LITERT_ERROR,
               "Cannot destroy device context with graphs still registered");
    return kLiteRtStatusErrorRuntimeFailure;
  }
  ...
  if (bytecode_buffer.fd >= 0) {
    GT_LOG_RETURN_IF_SB_ERROR(
        thrLoadSqContainerFdWithOffset(
            thr_context_, thr_type, bytecode_buffer.fd,
    ...
    GT_LOG_RETURN_IF_SB_ERROR(
        thrLoadSqContainer(
            thr_context_, thr_type, sq_bytecode, bytecode_buffer.size,
            &exec_handle),
...
LiteRtStatus LiteRtDispatchDeviceContextT::UnloadExecutable(
    LiteRtDispatchExecutableHandle exec_handle) {
  GT_LOG_RETURN_IF_SB_ERROR(
      thrUnloadSqContainer(thr_context_, exec_handle),
