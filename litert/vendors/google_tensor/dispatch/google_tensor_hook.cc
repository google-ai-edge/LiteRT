...
LiteRtStatus GetHooks(LiteRtDispatchDeviceContext device_context,
  ...
  *user_data = context.get();
  // Ownership of this context memory is transferred out of this function.
  // It will be re-captured and freed by `std::unique_ptr` inside the hook
  // callback when we receive `kLiteRtHookTypeStopAndProcess`.
  if (device_context) {
