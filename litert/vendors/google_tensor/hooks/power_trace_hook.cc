...
absl::Status InitializePowerStatsInternal(PowerTraceContext* context) {
...
absl::Status HandlePowerRuntimeStartInternal(PowerTraceContext* context) {
  ...
  if (!context->initialized) {
    ...
    if (auto status = InitializePowerStatsInternal(context); !status.ok()) {
      return status;
    }
  }
  ...
  if (context->power_stats) {
    ...
    if (!energy_or.ok()) return energy_or.status();
...
void HandlePowerRuntimeStart(PowerTraceContext* context) {
  if (auto status = HandlePowerRuntimeStartInternal(context); !status.ok()) {
...
absl::Status HandlePowerRuntimeStopInternal(PowerTraceContext* context) {
  if (!context) return absl::OkStatus();
...
void HandlePowerRuntimeStop(PowerTraceContext* context) {
  if (auto status = HandlePowerRuntimeStopInternal(context); !status.ok()) {
    LITERT_LOG(LITERT_ERROR,
               "Google TensorHook failed HandlePowerRuntimeStop: %s",
               std::string(status.message()).c_str());
  }
}
