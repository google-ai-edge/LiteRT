...
Expected<void> AdapterOdc::Compile(
  ...
  // Parse soc model.
  EdgeTpuCompilerOptions::SocModel soc_model =
      EdgeTpuCompilerOptions::SOC_MODEL_UNSPECIFIED;
  if (soc_model_size > 0 && soc_model_data != nullptr) {
    if (strcmp(soc_model_data, "Tensor_G3") == 0) {
      soc_model = EdgeTpuCompilerOptions::SOC_MODEL_TENSOR_G3;
    } else if (strcmp(soc_model_data, "Tensor_G4") == 0) {
      soc_model = EdgeTpuCompilerOptions::SOC_MODEL_TENSOR_G4;
    } else if (strcmp(soc_model_data, "Tensor_G5") == 0) {
      soc_model = EdgeTpuCompilerOptions::SOC_MODEL_TENSOR_G5;
    } else {
      soc_model = EdgeTpuCompilerOptions::SOC_MODEL_UNSPECIFIED;
    }
  }

  LITERT_ASSIGN_OR_RETURN(auto edgetpu_compiler_options,
                          CreateEdgeTpuCompilerOptions(options));
  edgetpu_compiler_options.set_soc_model(soc_model);
