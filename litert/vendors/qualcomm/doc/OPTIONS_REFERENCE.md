...
| ----------------- | ------------------------- | ----------------------- |
| **Compile time**  | Compiler plugin           | `use_conv_hmx`,         |
:                   : (`apply_plugin_main`, AOT : `optimization_level`,   :
:                   : / JIT graph build)        : `vtcm_size`,            :
:                   :                           : `graph_priority`        :
| **Dispatch time** | Runtime dispatcher        | `htp_performance_mode`  |
:                   : (on-device execution)     :                         :
| **Both**          | Compiler plugin and       | `log_level`, `backend`, |
:                   : runtime dispatcher        : `profiling`             :
...
| Category          | Options                                            |
| ----------------- | -------------------------------------------------- |
| **General / SDK** | `log_level`, `backend`, `graph_priority`,          |
:                   : `custom_op_package`, `enable_just_in_time`,        :
:                   : `graph_io_tensor_mem_type`, `profiling`            :
| **HTP**           | `use_conv_hmx`, `use_fold_relu`, `htp_p_point`,    |
:                   : `htp_performance_mode`, `optimization_level`,      :
:                   : `vtcm_size`, `num_hvx_threads`,                    :
:                   : `use_int64_bias_as_int32`, `enable_weight_sharing` :
...
| `target`                | QNN backend — **must** match `backend` (`HTP`, |
:                         : `GPU`, …)                                      :
...
QNN intermediate-representation dumps — diagnostic artifacts produced at compile
time.

| Option  | CLI flag         | Default   | Phase   | Notes                 |
:         : (`--qualcomm_…`) :           :         :                       :
| ------- | ---------------- | --------- | ------- | --------------------- |
| DLC dir | `dlc_dir`        | *(empty)* | compile | If set, compile QNN   |
:         :                  :           :         : graphs as DLC to this :
:         :                  :           :         : dir.                  :
...
:              : (`--qualcomm_…`)   :           :         :                    :
| ------------ | ------------------ | --------- | ------- | ------------------ |
| Saver output | `saver_output_dir` | *(empty)* | compile | If set, compile    |
: dir          :                    :           :         : `saver_output.c` + :
:              :                    :           :         : `params.bin`       :
:              :                    :           :         : (records all QNN   :
:              :                    :           :         : API calls for      :
...
:                 : (`--qualcomm_…`)  :           :         :                 :
| --------------- | ----------------- | --------- | ------- | --------------- |
| Dump tensor IDs | `dump_tensor_ids` | *(empty)* | compile | Comma-separated |
:                 :                   :           :         : tensor IDs to   :
:                 :                   :           :         : mark as extra   :
:                 :                   :           :         : graph outputs   :
:                 :                   :           :         : (per-layer dump :
...
:                 :                   :           :         : `-1` = dump all :
:                 :                   :           :         : op outputs.     :
| IR JSON dir     | `ir_json_dir`     | *(empty)* | compile | If set, compile |
:                 :                   :           :         : QNN IR as QNN   :
:                 :                   :           :         : JSON to this    :
:                 :                   :           :         : dir.            :
...
| ----------------------- | ---------------------------------- | ------------- |
| `use_htp_preference`    | `--qualcomm_use_htp_preference`    | Transform a   |
:                         : (retired)                          : LiteRT op     :
:                         :                                    : into the      :
:                         :                                    : HTP-preferred :
:                         :                                    : pattern       :
| `use_qint16_as_quint16` | `--qualcomm_use_qint16_as_quint16` | Auto-convert  |
:                         : (retired)                          : quantized     :
:                         :                                    : int16 model → :
:                         :                                    : quint16       :
...
what `LrtCreateQualcommOptionsFromToml` consumes (and what the C API
serializes).
