...
constexpr std::uint32_t kStepOutputRank = 2;
constexpr std::uint32_t kSequenceOutputRank = 3;
...
std::vector<OpWrapper> BuildLstmOp(
  ...
  // MasterOpDef allows a 2D single step or a 3D sequence. Any other rank would
  // otherwise fall through to the 2D path and be silently mislowered.
  const std::uint32_t output_rank = output_tensor.GetRank();
  if (output_rank != kStepOutputRank && output_rank != kSequenceOutputRank) {
    QNN_LOG_ERROR(
        "LiteRT Lstm lowering expects a rank %u or %u output, got rank %u.",
        kStepOutputRank, kSequenceOutputRank, output_rank);
    return res;
  }
  ...
  const bool is_sequence = output_rank == kSequenceOutputRank;
