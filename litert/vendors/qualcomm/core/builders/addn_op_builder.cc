...
std::vector<OpWrapper> BuildAddNOp(
  ...
  if (inputs.size() < kMinNumInputs || outputs.size() != 1) {
    QNN_LOG_ERROR("AddN op expects at least 2 inputs and 1 output.");
    return res;
  }

  for (const auto& input : inputs) {
    if (input.get().IsQuant()) {
      QNN_LOG_ERROR("AddN op does not support quantized inputs/outputs.");
      return res;
    }
  }
  ...
  const TensorWrapper* accumulator =
      &tensor_pool.CloneNativeTensorFrom(inputs[0]);
  ...
  for (size_t i = kMinNumInputs; i < inputs.size(); ++i) {
    ...
    const TensorWrapper& add_output =
