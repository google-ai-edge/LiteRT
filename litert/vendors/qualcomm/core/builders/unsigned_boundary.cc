...
using NeuronBackendKey = std::pair<std::uint32_t, BackendType>;
...
const absl::flat_hash_set<NeuronBackendKey>& UnsignedOnlyNeurons() {
...
// The `operation` of an ElementWiseNeuron op, stored as its scalar param 0.
std::optional<std::uint32_t> NeuronOperation(const OpWrapper& op) {
  ...
  return param.scalarParam.uint32Value;
}

bool NeedsUnsignedActivations(const OpWrapper& op, BackendType backend) {
  if (op.GetOpCode() == QnnOpCode::kElementWiseNeuron) {
    const std::optional<std::uint32_t> operation = NeuronOperation(op);
    return operation.has_value() &&
           UnsignedOnlyNeurons().contains({*operation, backend});
  }
  return UnsignedOnlyOps().contains({op.GetOpCode(), backend});
}
...
void InsertUnsignedActivationBoundaries(BackendType backend,
  ...
  for (auto& op : ops) {
    if (!NeedsUnsignedActivations(op, backend)) {
