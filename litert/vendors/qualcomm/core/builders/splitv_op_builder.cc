...
std::vector<OpWrapper> BuildSplitVOp(
  ...
  // QNN_OP_SPLIT requires a non-empty split_index (at least one cut), but TFLite
  // SPLIT_V permits num_splits == 1, where the sole size_splits entry equals the
  // axis dimension and the output is identical to the input. Lower that case to
  // reshape instead of QNN_OP_SPLIT.
  if (num_splits == 1) {
