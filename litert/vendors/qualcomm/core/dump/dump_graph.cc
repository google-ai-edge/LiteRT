...
bool IsFixedPointType(Qnn_DataType_t data_type) {
  switch (data_type) {
    case QNN_DATATYPE_SFIXED_POINT_2:
    case QNN_DATATYPE_SFIXED_POINT_4:
    case QNN_DATATYPE_SFIXED_POINT_8:
...
bool IsSignedFixedPointType(Qnn_DataType_t data_type) {
  switch (data_type) {
    case QNN_DATATYPE_SFIXED_POINT_2:
    case QNN_DATATYPE_SFIXED_POINT_4:
    case QNN_DATATYPE_SFIXED_POINT_8:
...
nlohmann::json SerializeScaleOffsetJson(float scale, int32_t offset,
                                        uint32_t bw, Qnn_DataType_t data_type) {
  if (bw == 0) {
    QNN_LOG_WARNING("Quantization bitwidth is 0 in Qnn Json dump");
    return {};
  }
  if (bw >= 63) {
    QNN_LOG_WARNING("Quantization bitwidth: %u is too large in Qnn Json dump",
                    bw);
    return {};
  }
...
nlohmann::json SerializeQuantParamToJson(
  ...
  if (quant_params.quantizationEncoding ==
    ...
    qnn_quant_params["scale_offset"] = SerializeScaleOffsetJson(
    ...
    for (uint32_t i = 0; i < encoding.numScaleOffsets; ++i) {
      scale_offsets.emplace_back(SerializeScaleOffsetJson(
    ...
    for (uint32_t i = 0; i < encoding.numElements; ++i) {
      scale_offsets.emplace_back(
          SerializeScaleOffsetJson(encoding.scales[i], encoding.offsets[i],
