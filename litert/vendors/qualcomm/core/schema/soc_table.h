// Copyright (c) Qualcomm Innovation Center, Inc. All Rights Reserved.
// SPDX-License-Identifier: Apache-2.0

#ifndef ODML_LITERT_LITERT_VENDORS_QUALCOMM_CORE_SCHEMA_SOC_TABLE_H_
#define ODML_LITERT_LITERT_VENDORS_QUALCOMM_CORE_SCHEMA_SOC_TABLE_H_
#include <cstddef>
#include <cstdint>
namespace qnn {
enum class SnapdragonModel {
  UNKNOWN_SDM = 0,
  SDM865 = 21,
  SM6350 = 29,
  SA8295 = 39,
  SM8350 = 30,
  SM8450 = 36,
  SM8475 = 42,
  SM8550 = 43,
  SA8255 = 52,
  SXR2230P = 53,
  SM8650 = 57,
  SSG2125P = 58,
  SC8380XP = 60,
  SM8750 = 69,
  SXR2330P = 75,
  QCS9100 = 77,
  QCS8300 = 82,
  SM8850 = 87,
  SAR2230P = 95,
  SW6100 = 96,
};

enum class DspArch {
  NONE = 0,
  V65 = 65,
  V66 = 66,
  V68 = 68,  // HTP supported device
  V69 = 69,  // HTP supported device
  V73 = 73,  // HTP supported device
  V75 = 75,  // HTP supported device
  V79 = 79,  // HTP supported device
  V81 = 81,  // HTP supported device
};

// Version of the LPAI (Low Power AI) hardware subsystem shipped on a SoC.
// Mirrors the QNN SDK's QnnLpaiBackend_HwVersion_t. A SocInfo whose
// lpai_hw_version is kUnknown has no LPAI subsystem.
enum class LpaiHardwareVersion {
  kUnknown = 0,
  kV5 = 5,
  kV6 = 6,
};

struct SocInfo {
  const char* soc_name;
  SnapdragonModel soc_model;
  DspArch dsp_arch;
  std::uint32_t vtcm_size_in_mb;
  LpaiHardwareVersion lpai_hw_version;

  constexpr SocInfo(
      const char* soc_name, SnapdragonModel soc_model, DspArch dsp_arch,
      std::size_t vtcm_size_in_mb,
      LpaiHardwareVersion lpai_hw_version = LpaiHardwareVersion::kUnknown)
      : soc_name(soc_name),
        soc_model(soc_model),
        dsp_arch(dsp_arch),
        vtcm_size_in_mb(vtcm_size_in_mb),
        lpai_hw_version(lpai_hw_version) {}
};

extern const SocInfo kSocInfos[];
extern const uint64_t kNumSocInfos;
}  // namespace qnn
#endif  // ODML_LITERT_LITERT_VENDORS_QUALCOMM_CORE_SCHEMA_SOC_TABLE_H_
