...
TEST_P(TensorWrapperEqualTest, EqualityOperator) {
  ...
  auto makeData = [](const std::vector<uint32_t>& dims, uint8_t fill) {
    uint32_t count = 1;
    for (auto dim : dims) count *= dim;
    return std::vector<uint8_t>(count, fill);
  };
  std::vector<uint8_t> data1 = makeData(dims1, 1);
  std::vector<uint8_t> data2 = makeData(dims2, 1);
...
INSTANTIATE_TEST_SUITE_P(TensorWrapperTest_Combinations, TensorWrapperEqualTest,
                         ::testing::Values(
                             // All fields same
                             TensorEqualParams{QNN_TENSOR_TYPE_APP_WRITE,
                                               QNN_TENSOR_TYPE_APP_WRITE,
                                               QNN_DATATYPE_UFIXED_POINT_8,
                                               QNN_DATATYPE_UFIXED_POINT_8,
                                               {1, 1, 3},
