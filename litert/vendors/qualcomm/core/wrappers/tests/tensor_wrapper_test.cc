...
void RunQnnTensorImplicitCopyTest(Qnn_DataType_t datatype) {
  ...
  if constexpr (bitwidth == kQuantBitWidth4) {
    auto int8_data = UnpackIntData<kQuantBitWidth4>(data.data(), data.size());
    ...
    auto int8_data = UnpackIntData<kQuantBitWidth2>(data.data(), data.size());
    ValidateTensor<int8_t, true>(cloned_tensor, QNN_DATATYPE_SFIXED_POINT_8,
                                 kDims, int8_data);
    ValidateTensor<int8_t, true>(ref_tensor, QNN_DATATYPE_SFIXED_POINT_8, kDims,
                                 int8_data);
...
TEST_P(TensorWrapperEqualTest, EqualityOperator) {
  ...
  auto make_data = [](const std::vector<uint32_t>& dims,
    ...
    return std::vector<uint8_t>(count * GetDataTypeSize(data_type), fill);
  };
  std::vector<uint8_t> data1 = make_data(dims1, data_tensor_type1, 1);
  std::vector<uint8_t> data2 = make_data(dims2, data_tensor_type2, 1);
