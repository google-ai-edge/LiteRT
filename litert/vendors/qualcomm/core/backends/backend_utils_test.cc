...
TEST(VotingThreadTest, DebounceDownvoteFiresAfterTimeout) {
  ...
  VotingThread vt([&](VotingThread::VoteType v) {
    if (v == VotingThread::VoteType::kDownVote)
      down_count.fetch_add(1);
  });
...
TEST(VotingThreadTest, ImmediateDownvoteFiresWithoutWaiting) {
  std::atomic<int> down_count{0};
  VotingThread vt([&](VotingThread::VoteType v) {
    if (v == VotingThread::VoteType::kDownVote)
      down_count.fetch_add(1);
  ...
  vt.Enqueue(VotingThread::VoteType::kDownVote, /*debounce=*/false);
  std::this_thread::sleep_for(std::chrono::milliseconds(50));
