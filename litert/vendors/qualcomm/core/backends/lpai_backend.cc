...
bool LpaiBackend::Init(const Options& options,
  ...
  // TODO (chunhsue-qti): Remove after backend fix this issue.
  QNN_LOG_WARNING(
  ...
  QNN_LOG_INFO(
      ...
      "SoC hw_version; the LPAI driver uses the silicon it runs on.",
  ...
  if (hw_version == LpaiHardwareVersion::kUnknown) {
    QNN_LOG_ERROR(
        "No LPAI hardware version could be resolved for SoC \"%.*s\" "
        "(soc_model %u); cannot initialize the LPAI backend.",
        static_cast<int>(soc_info_.soc_name.size()), soc_info_.soc_name.data(),
        soc_info_.soc_model);
    return false;
  }
...
bool LpaiBackend::ConfigureGraphAfterRetrieve(const GraphConfigContext& ctx,
  ...
  QnnLpaiGraph_CustomConfig_t perf_custom_config =
  ...
  auto& core_affinity = builder.Store<QnnLpaiGraph_CoreAffinity_t>(
  ...
  QnnLpaiGraph_CustomConfig_t affinity_custom_config =
      QNN_LPAI_GRAPH_CUSTOM_CONFIG_INIT;
  ...
  if (auto status = QnnApi()->graphSetConfig(
          ctx.graph, builder.GetNullTerminatedConfigs().data());
      status != QNN_SUCCESS) {
