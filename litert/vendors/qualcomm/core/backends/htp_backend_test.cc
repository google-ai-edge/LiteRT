...
QnnHtpDevice_DeviceInfoExtension_t test_htp_device_info_extension = {
    ...
    .onChipDevice = {.arch = QNN_HTP_DEVICE_ARCH_V81, .socModel = 87}};

QnnDevice_CoreInfo_t test_device_core_info = {
    .version = QNN_DEVICE_CORE_INFO_VERSION_1,
    .v1 = {.coreId = 0, .coreType = 0}};

QnnDevice_HardwareDeviceInfo_t test_device_hardware_device_info = {
    .version = QNN_DEVICE_HARDWARE_DEVICE_INFO_VERSION_1,
    .v1 = {.deviceId = 0,
           .numCores = 1,
           .cores = &test_device_core_info,
           .deviceInfoExtension = &test_htp_device_info_extension}};

QnnDevice_PlatformInfo_t test_device_platform_info = {
    .version = QNN_DEVICE_PLATFORM_INFO_VERSION_1,
    .v1 = {.numHwDevices = 1, .hwDevices = &test_device_hardware_device_info}};
...
Qnn_ErrorHandle_t MockDeviceFree(void* handle) {
  if (handle == &test_device_platform_info) {
    return QNN_SUCCESS;
  }
  return QNN_COMMON_ERROR_GENERAL;
}
...
class HtpBackendPerfBaseTest : public testing::TestWithParam<HtpPerfParams> {
 public:
  void SetUp() override {
    ...
    real_qnn_api_ = const_cast<QNN_INTERFACE_VER_TYPE*>(
        ResolveQnnApi(handle_.get(), HtpBackend::GetExpectedBackendVersion()));
    ASSERT_TRUE(real_qnn_api_);

    // Hook functions if not already hooked
    if (real_qnn_api_->deviceGetInfrastructure != MockDeviceGetInfrastructure) {
      real_device_get_infrastructure = real_qnn_api_->deviceGetInfrastructure;
      real_qnn_api_->deviceGetInfrastructure = MockDeviceGetInfrastructure;
    }

    if (real_qnn_api_->deviceGetPlatformInfo != MockDeviceGetPlatformInfo) {
      real_device_get_platform_info = real_qnn_api_->deviceGetPlatformInfo;
      real_qnn_api_->deviceGetPlatformInfo = MockDeviceGetPlatformInfo;
    }

    if (real_qnn_api_->deviceFree != MockDeviceFree) {
      real_device_free = real_qnn_api_->deviceFree;
      real_qnn_api_->deviceFree = MockDeviceFree;
    }
  }
  ...
  void TearDown() override {
    if (real_qnn_api_) {
      ...
      if (real_device_get_infrastructure) {
        real_qnn_api_->deviceGetInfrastructure = real_device_get_infrastructure;
      }
      if (real_device_get_platform_info) {
        real_qnn_api_->deviceGetPlatformInfo = real_device_get_platform_info;
      }
      if (real_device_free) {
        real_qnn_api_->deviceFree = real_device_free;
      }
    }
  }
 ...
 protected:
  DLHandle handle_;
  QNN_INTERFACE_VER_TYPE* real_qnn_api_ = nullptr;
};
...
TEST_P(HtpBackendRPCPollingPerfParamTest, InitWithPerfMode) {
  ...
  HtpBackend backend(real_qnn_api_);
