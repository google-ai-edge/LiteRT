  ...
  void SetUp() {
    ...
    switch (GetParam()) {
      case BackendType::kHtpBackend:
        handle =
            std::make_unique<DlHandle>(::qnn::HtpBackend::GetLibraryName());
        backend =
            std::make_unique<::qnn::HtpBackend>(handle->LoadAndResolveQnnApi(
                ::qnn::HtpBackend::GetExpectedBackendVersion()));

        break;
      case BackendType::kIrBackend:
        handle = std::make_unique<DlHandle>(::qnn::IrBackend::GetLibraryName());
        backend =
            std::make_unique<::qnn::IrBackend>(handle->LoadAndResolveQnnApi(
...
TEST_P(QnnBackendTest, InitializeWithLogLevelOffTest) {
  Options options;
...
TEST_P(QnnBackendTest, InitializeWithLogLevelVerboseTest) {
  Options options;
