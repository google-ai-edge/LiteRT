 ...
 private:
  ...
  void InitDownVotePowerConfigs(HtpPerformanceMode performance_mode) {
    switch (performance_mode) {
      ...
      case HtpPerformanceMode::kExtremePowerSaver:
        SetPowerConfigs(
            down_vote_power_configs_, power_config_id_, PowerConfig::kDcvsEnable,
            QNN_HTP_PERF_INFRASTRUCTURE_POWERMODE_POWER_SAVER_MODE,
...
bool HtpBackend::Init(const Options& options, std::optional<SocInfo> soc_info) {
  ...
  std::optional<SocInfo> soc_info_online;
  ...
  if (local_qnn_device_platform_info) {
    ...
    soc_info_online =
        FindSocInfo(static_cast<SnapdragonModel>(online_soc_model));
    QNN_LOG_INFO(
