 ...
 public:
  ...
  bool SetPerfModeAndApplyIfManual(const Options& options) {
    ...
    if (options.GetHtpPerfCtrlMode() == HtpPerfCtrlMode::kManual) {
      ...
      if (new_mode == HtpPerformanceMode::kDefault) {
        DownVote();
        // Record the down-vote so a later re-vote to the previous mode is not
        // skipped by the same-mode fast path (burst -> default -> burst).
...
void HtpBackend::ScheduleUpVote() { htp_perf_control_->ScheduleUpVote(); }

void HtpBackend::ScheduleDownVote() { htp_perf_control_->ScheduleDownVote(); }
