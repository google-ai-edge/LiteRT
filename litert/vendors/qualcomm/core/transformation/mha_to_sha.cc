...
// Pass k_slice_attn_update_convert_op=nullptr to skip the
// KSliceAttnUpdateConvert Pass v_slice_attn_update_convert_op=nullptr to skip
// the VSliceAttnUpdateConvert.
TensorWrapper& BuildSingleSHAGemma4B(
    ...
    const OpWrapper& qk_vcache_matmul,
    const OpWrapper* v_slice_attn_update_convert_op,
    const OpWrapper& qk_vslice_matmul, const OpWrapper& qkv_add,
    const OpWrapper& convert_out) {
  ...
  const auto build_slice_ranges =
      [&tensor_pool,
       num_attn_per_kv_heads](const OpWrapper& slice) -> TensorWrapper& {
  ...
  new_ops.emplace_back(CreateOpWithSameParams(q_kcache_matmul, {query, k_cache},
                                              {qk_cache_output}));
  ...
  if (k_slice_attn_update_convert_op != nullptr) {
    auto& k_slice_cv_output = tensor_pool.CloneNativeTensorFrom(
        k_slice_attn_update_convert_op->GetOutputTensor(0),
        k_slice.GetDimensions());
    new_ops.emplace_back(CreateOpWithSameParams(
        *k_slice_attn_update_convert_op, {k_slice}, {k_slice_cv_output}));
  ...
  new_ops.emplace_back(CreateConcatenationOp({qk_cache_output, qk_slice_output},
                                             concat_output, 2));
  ...
  if (v_slice_attn_update_convert_op != nullptr) {
    auto& v_slice_cv_output = tensor_pool.CloneNativeTensorFrom(
        v_slice_attn_update_convert_op->GetOutputTensor(0),
        v_slice.GetDimensions());
    new_ops.emplace_back(CreateOpWithSameParams(
        *v_slice_attn_update_convert_op, {v_slice}, {v_slice_cv_output}));
  ...
  new_ops.emplace_back(CreateOpWithSameParams(convert_out, {qkv_add_output},
                                              {convert_out_output}));
...
size_t OptimizeGQAGemma4BPrefill(
  ...
  //   Case 2 (k_slice_cv): T R Cv M  Cv M  Cc Add Sm Sl Sl M  M  Add Cv R T
  //   (17) Case 3 (v_slice_cv): T R Cv M  M  Cc Add Sm Sl Sl M  Cv M  Add Cv R
  //   T (17) Case 4 (both):       T R Cv M  Cv M  Cc Add Sm Sl Sl M  Cv M  Add
  //   Cv R T (18)
  ...
  // Step 1: detect optional KSliceAttnUpdateConvert (between the two QK
  // matmuls).
  const bool has_k_slice_attn_update_convert =
      ops[start_index + 4].IsOpCode(QnnOpCode::kConvert);
  const size_t k_slice_attn_update_convert_idx =
      4;  // valid only if has_k_slice_attn_update_convert
  ...
  // Step 2: detect local/global mask (Concat+Reshape immediately after QK
  // Concat).
  ...
  // Step 4: detect optional VSliceAttnUpdateConvert (between the two QKV
  // matmuls).
  const bool has_v_slice_attn_update_convert =
      ops[start_index + qkv_cache_matmul_idx + 1].IsOpCode(QnnOpCode::kConvert);
  const size_t v_slice_attn_update_convert_idx =
      qkv_cache_matmul_idx +
      1;  // valid only if has_v_slice_attn_update_convert
  const size_t qkv_slice_matmul_idx =
      qkv_cache_matmul_idx + (has_v_slice_attn_update_convert ? 2 : 1);
  ...
  const OpWrapper* k_slice_attn_update_convert_op =
      has_k_slice_attn_update_convert
          ? &ops[start_index + k_slice_attn_update_convert_idx]
          : nullptr;
  const OpWrapper* v_slice_attn_update_convert_op =
      has_v_slice_attn_update_convert
          ? &ops[start_index + v_slice_attn_update_convert_idx]
          : nullptr;
  ...
  if (!validate_perm_0213(ops[start_index + kTranspose0Idx]) ||
    ...
    QNN_LOG_WARNING(
        "[G2G] GQA Gemma4B Prefill skipped: "
  ...
  const bool convert_to_kslice_matmul =
      !has_k_slice_attn_update_convert ||
  ...
  const bool convert_to_vslice_matmul =
      !has_v_slice_attn_update_convert ||
  ...
  auto query_unpack_outputs = UnpackTensor(
      tensor_pool, new_ops, ops[start_index + kTranspose0Idx].GetInputTensor(0),
      kQueryUnpackAxis);
  auto k_cache_unpack_outputs = UnpackTensor(
      tensor_pool, new_ops,
      ops[start_index + kQKCacheMatmulIdx].GetInputTensor(1), kKVUnpackAxis);
  // When the optional Convert is present, the matmul's input(1) is the
  // Convert's output (an intermediate tensor with id 0). We must unpack the
  // Convert's input instead, which is the actual static KV weight tensor.
  auto k_slice_unpack_outputs = UnpackTensor(
      tensor_pool, new_ops,
      has_k_slice_attn_update_convert
          ? k_slice_attn_update_convert_op->GetInputTensor(0)
          : ops[start_index + qk_slice_matmul_idx].GetInputTensor(1),
      kKVUnpackAxis);
  auto v_cache_unpack_outputs = UnpackTensor(
      tensor_pool, new_ops,
      ops[start_index + qkv_cache_matmul_idx].GetInputTensor(1), kKVUnpackAxis);
  auto v_slice_unpack_outputs = UnpackTensor(
      tensor_pool, new_ops,
      has_v_slice_attn_update_convert
          ? v_slice_attn_update_convert_op->GetInputTensor(0)
  ...
  const auto& mask_orig = ops[start_index + mask_add_idx].GetInputTensor(1);
  ...
  for (size_t i = 0; i < num_kv_heads; ++i) {
    for (size_t j = 0; j < num_attn_per_kv_heads; ++j) {
      ...
      sha_outputs.emplace_back(BuildSingleSHAGemma4B(
          ...
          mask_unpack_outputs[j], ops[start_index + kQKCacheMatmulIdx],
          k_slice_attn_update_convert_op,
          ops[start_index + qk_slice_matmul_idx],
          ops[start_index + qk_concat_idx], ops[start_index + mask_add_idx],
          ...
          v_slice_attn_update_convert_op,
          ops[start_index + qkv_slice_matmul_idx],
          ops[start_index + qkv_add_idx], ops[start_index + convert_out_idx]));
    }
  }
  ...
  // layout of [B, S, H, D].  This avoids both Pack and Transpose:
  // Concat(axis=2) is a block-copy (no strided scatter), so it doesn't have the
  // HTP accuracy issue that Pack(axis=2) exhibits. sha_outputs: num_attn_heads
  // tensors of [B, S_per_head, D].
  ...
  new_ops.emplace_back(CreateReshapeOp(
      concat_output, ops[start_index + transpose1_idx].GetOutputTensor(0)));
...
size_t OptimizeMHAFastVlmPrefill(
  ...
  if (!(is_connected(kKSliceAddIdx, 0, kQKSliceMatmulIdx, 1) &&
        ...
        ops[start_index + kKSliceAddIdx].IsOpCode(
...
size_t OptimizeMHAAttn(std::function<bool(OpWrapper&)> validate_op_config,
  ...
  for (size_t output_index = 0; output_index < num_out; ++output_index) {
    ...
    if (!(IS_CONNECTED(kAttnMulQ, 0, kAttnTransposeQ, 0) &&
          ...
          ops[start_index + kAttnMulQ].IsOpCode(
              QnnOpCode::kElementWiseMultiply) &&
          ops[start_index + kAttnMulK].IsOpCode(
              QnnOpCode::kElementWiseMultiply))) {
      QNN_LOG_ERROR("[G2G] Connection check failed.");
      return 1;
    }
