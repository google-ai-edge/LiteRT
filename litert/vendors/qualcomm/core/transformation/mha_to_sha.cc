...
size_t OptimizeMHAFastVlmPrefill(
    ...
    std::vector<OpWrapper>& ops, size_t start_index, TensorPool& tensor_pool,
    size_t pattern_size) {
  //   constexpr size_t reshape_1_index = 0;
  constexpr size_t add_1_index = -2;
  //   constexpr size_t transpose_1_index = 1;
  ...
  if (!(is_connected(add_1_index, 0, matmul_qk_index, 1) &&
        // is_connected(transpose_1_index, 0, matmul_v2_index, 1) &&
  ...
  QNN_LOG_WARNING("[G2G] MHA optimization (fast vlm Prefill)");

  // QKV Unpack
  const auto& matmul_v1 = ops[start_index + matmul_v1_index];
  const auto& matmul_v1_in = matmul_v1.GetInputTensor(1);
  auto unpack_1_dims = matmul_v1_in.GetDims();
  uint32_t num_kv_heads = unpack_1_dims[1];

  const auto& matmul_q = ops[start_index + matmul_q_index];
  const auto& matmul_q_in = matmul_q.GetInputTensor(1);
  auto unpack_2_dims = matmul_q_in.GetDims();

  const auto& mul = ops[start_index + mul_index];
  const auto& mul_in = mul.GetInputTensor(0);
  ...
  const auto& add_1_in_1 = add_1.GetInputTensor(0);
  auto unpack_4_dims = add_1_in_1.GetDims();

  const auto& add_1_in_2 = add_1.GetInputTensor(1);
  auto unpack_5_dims = add_1_in_2.GetDims();

  const auto& matmul_v2 = ops[start_index + matmul_v2_index];
  const auto& matmul_v2_in = matmul_v2.GetInputTensor(1);
  //   const auto& transpose_1_in =
  //       ops[start_index + transpose_1_index].GetInputTensor(0);
  ...
  QNN_LOG_WARNING("[G2G] num_kv_heads: %d\n", num_kv_heads);
  QNN_LOG_WARNING("[G2G] num_attn_heads: %d\n", num_attn_heads);
  QNN_LOG_WARNING("[G2G] num_attn_per_kv_heads: %d\n", num_attn_per_kv_heads);
  ...
  if (!(num_kv_heads == unpack_2_dims[1] &&
    ...
    QNN_LOG_WARNING("[G2G] num_kv_heads not match...\n");
    QNN_LOG_WARNING("[G2G] num_kv_heads: %d\n", num_kv_heads);
    QNN_LOG_WARNING("[G2G] unpack_2_dims: %d\n", unpack_2_dims[1]);
    QNN_LOG_WARNING("[G2G] unpack_3_dims: %d\n", unpack_3_dims[1] / 7);
    QNN_LOG_WARNING("[G2G] unpack_4_dims: %d\n", unpack_4_dims[1]);
    QNN_LOG_WARNING("[G2G] unpack_5_dims: %d\n", unpack_5_dims[1]);
    QNN_LOG_WARNING("[G2G] unpack_6_dims: %d\n", unpack_6_dims[1]);
    QNN_LOG_WARNING("[G2G] mha_output_dims: %d\n", (mha_output_dims[1] / 64));
    return 1;
  }
  QNN_LOG_WARNING("[G2G] num_kv_heads match...\n");
  ...
  //       sizeof(decltype(split_axis_data)::value_type),
  //       split_axis_data.data());
  //   auto split_op = BuildSplitOp(
  //       tensor_pool, {split_axis,
  //       const_cast<::qnn::TensorWrapper&>(mul_in)}, split_sha_inputs,
  //       num_heads);
  //   std::move(split_op.begin(), split_op.end(),
  //   std::back_inserter(new_ops));
  ...
  for (size_t i = 0; i < num_kv_heads; ++i) {
    for (size_t j = 0; j < num_attn_per_kv_heads; ++j) {
      ...
      EmplaceOpWithIO(
          new_ops, mul,
          {unpack_3_sha_inputs[i * num_attn_per_kv_heads + j], std::nullopt},
          {mul_output});

      //   // Reshape 1 XXX
      //   const auto& reshape_1 = ops[start_index + reshape_1_index];
      //   auto reshape_1_output_dims = reshape_1.GetOutputTensor(0).GetDims();
      //   reshape_1_output_dims.erase(reshape_1_output_dims.begin() + 1);
      //   auto& reshape_1_output = tensor_pool.CloneNativeTensorFrom(
      //       reshape_1.GetOutputTensor(0), reshape_1_output_dims);
      //   EmplaceOpWithIO(new_ops, reshape_1, {mul_output},
      //   {reshape_1_output});
      ...
      EmplaceOpWithIO(new_ops, matmul_q, {mul_output, unpack_2_sha_inputs[i]},
                      {matmul_q_output});

      auto printDims = [](const std::vector<uint32_t>& dims) {
      ...
      // QNN_LOG_WARNING("[G2G] unpack_2_sha_inputs[i]: %s",
      // printDims(unpack_2_sha_inputs[i].get().GetDims()).c_str());
      // QNN_LOG_WARNING("[G2G] reshape_1_output: %s",
      //                 printDims(reshape_1_output.GetDims()).c_str());
      // QNN_LOG_WARNING("[G2G] matmul_q_output_dims: %s",
      //                 printDims(matmul_q_output_dims).c_str());
      ...
      EmplaceOpWithIO(new_ops, add_1,
                      {unpack_4_sha_inputs[i], unpack_5_sha_inputs[i]},
                      {add_1_output});

      // Matmul k
      const auto& matmul_qk = ops[start_index + matmul_qk_index];
      ...
      //   auto reshape_2_output_dims = reshape_2.GetOutputTensor(0).GetDims();
      //   reshape_2_output_dims[0] = 1;
      //   auto& reshape_2_output = tensor_pool.CloneNativeTensorFrom(
      //       reshape_2.GetOutputTensor(0), reshape_2_output_dims);
      //   EmplaceOpWithIO(new_ops, reshape_2, {concat_output},
      //   {reshape_2_output});

      // Add 2
      const auto& add_2 = ops[start_index + add_2_index];
      ...
      // Reshape 3
      const auto& reshape_3 = ops[start_index + reshape_3_index];
      ...
      auto slice_1_param_param_data =
          slice_1_param.GetStaticTensorData<int32_t>();
      std::vector<int32_t> slice_1_ranges(
          slice_1_param_param_data.value().begin(),
          slice_1_param_param_data.value().end());
      slice_1_ranges.erase(slice_1_ranges.begin() + 3,
                           slice_1_ranges.begin() + 6);
      ...
      // Slice 2
      const auto& slice_2 = ops[start_index + slice_2_index];
      auto slice_2_param = slice_2.GetTensorPararm(0).GetTensor();
      auto slice_2_param_param_data =
          slice_2_param.GetStaticTensorData<int32_t>();
      std::vector<int32_t> slice_2_ranges(
          slice_2_param_param_data.value().begin(),
          slice_2_param_param_data.value().end());
      slice_2_ranges.erase(slice_2_ranges.begin() + 3,
                           slice_2_ranges.begin() + 6);
      ...
      EmplaceOpWithIO(new_ops, matmul_v1,
                      {slice_1_output, unpack_1_sha_inputs[i]},
                      {matmul_v1_output});

      //   // Transpose 1 XXX
      //   std::vector<uint32_t> transpose_1_perm_data = {0, 2, 1};
      //   const auto& transpose_1 = ops[start_index + transpose_1_index];
      //   auto transpose_1_perm = transpose_1.GetTensorPararm(0).GetTensor();
      //   auto& transpose_1_perm_tensor = tensor_pool.CreateStaticTensor(
      //       transpose_1_perm.GetDataType(),
      //       transpose_1_perm.GetQuantParams(), {3},
      //       transpose_1_perm_data.size() * sizeof(transpose_1_perm_data[0]),
      //       transpose_1_perm_data.data());
      //   auto transpose_1_output_dims =
      //   transpose_1.GetOutputTensor(0).GetDims();
      //   transpose_1_output_dims.erase(transpose_1_output_dims.begin() + 1);
      //   auto& transpose_1_output = tensor_pool.CloneNativeTensorFrom(
      //       transpose_1.GetOutputTensor(0), transpose_1_output_dims);
      //   EmplaceOpWithIO(new_ops, transpose_1, {unpack_6_sha_inputs[i]},
      //                   {transpose_1_output});
      //   new_ops.back().ClearTensorParams();
      //   new_ops.back().AddTensorParam(QNN_OP_TRANSPOSE_PARAM_PERM,
      //                                 transpose_1_perm_tensor);
      ...
      EmplaceOpWithIO(new_ops, matmul_v2,
                      {slice_2_output, unpack_6_sha_inputs[i]},
                      {matmul_v2_output});
      // Can't optimize this since transpose_2 output to GraphOutputs
      //   new_ops.back().ClearScalarParams();
      //   new_ops.back().AddScalarParam<bool>(QNN_OP_MAT_MUL_PARAM_TRANSPOSE_IN0,
      //                                       false);
      //   new_ops.back().AddScalarParam<bool>(QNN_OP_MAT_MUL_PARAM_TRANSPOSE_IN1,
      //                                       false);

      // Add 3
      const auto& add_3 = ops[start_index + add_3_index];
      ...
      //   // Transpose 2
      //   std::vector<uint32_t> transpose_2_perm_data = {0, 2, 1};
      //   const auto& transpose_2 = ops[start_index + transpose_2_index];
      //   auto transpose_2_perm = transpose_2.GetTensorPararm(0).GetTensor();
      //   auto& transpose_2_perm_tensor = tensor_pool.CreateStaticTensor(
      //       transpose_2_perm.GetDataType(),
      //       transpose_2_perm.GetQuantParams(), {3},
      //       transpose_2_perm_data.size() * sizeof(transpose_2_perm_data[0]),
      //       transpose_2_perm_data.data());
      //   auto& transpose_2_output = tensor_pool.CloneNativeTensorFrom(
      //       transpose_2.GetOutputTensor(0),
      //       {add_3_output_dims[0], add_3_output_dims[2],
      //       add_3_output_dims[1]});
      //   EmplaceOpWithIO(new_ops, transpose_2, {add_3_output},
      //                   {transpose_2_output});
      //   new_ops.back().ClearTensorParams();
      //   new_ops.back().AddTensorParam(QNN_OP_TRANSPOSE_PARAM_PERM,
      //                                 transpose_2_perm_tensor);
      sha_outputs.emplace_back(add_3_output);
    }
  }
  ...
  const bool is_valid =
      std::all_of(new_ops.begin(), new_ops.end(),
                  [validate_op_config](::qnn::OpWrapper& op_wrapper) -> bool {
                    return op_wrapper.IsOpCode(QnnOpCode::kSplit) ||
                           op_wrapper.IsOpCode(QnnOpCode::kUnPack) ||
                           validate_op_config(op_wrapper);
                  });
  if (is_valid) {
    // Adjust the name to avoid a name collision in the Qnn JSON dump.
    ...
    ops.erase(ops.begin() + start_index,
              ops.begin() + start_index + pattern_size);
    QNN_LOG_WARNING("[G2G] Current Step size: %d", step_size);
    return step_size;
  }
