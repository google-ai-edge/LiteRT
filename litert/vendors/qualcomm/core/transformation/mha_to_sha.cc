...
size_t OptimizeMHAFastVlmPrefill(
  ...
  static constexpr size_t q_head = 14;
  static constexpr size_t kv_head = 2;
  if (!(kv_head ==
        ...
        q_head ==
            ops[start_index + KQScaleMulIdx].GetInputTensor(0).GetDimension(
                1) &&
        kv_head == ops[start_index + kQKVCacheMatmulIdx]
                       .GetInputTensor(1)
                       .GetDimension(1) &&
        kv_head ==
            ops[start_index + kQKVAddIdx].GetOutputTensor(0).GetDimension(1))) {
    QNN_LOG_WARNING(
        "[G2G] Pattern does not match Q heads: %d, KV heads &d. In pattern, "
        "k_slice_add_in_0_dims[1]: %d, k_slice_add_in_1_dims[1]: %d,"
        "q_scale_mul_dims[1]: %d, v_cache_dims[1]: %d, qkv_add_out_dims[1]: %d",
        q_head, kv_head,
