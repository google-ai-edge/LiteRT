...
TEST(MHASHATest, FastVlm) {
  ...
  auto& slice0_begin = tensor_pool.CreateStaticTensor(
      ...
      slice0_begin_data.size() * sizeof(slice0_begin_data[0]),
      slice0_begin_data.data());
  const std::array<int32_t, 4> slice0_size_data{1, 2, 896, 1408};
  auto& slice0_size = tensor_pool.CreateStaticTensor(
      QNN_DATATYPE_INT_32, {}, {slice0_size_data.size()},
      slice0_size_data.size() * sizeof(slice0_size_data[0]),
      slice0_size_data.data());
  ...
  auto& slice1_begin = tensor_pool.CreateStaticTensor(
      ...
      slice1_begin_data.size() * sizeof(slice1_begin_data[0]),
      slice1_begin_data.data());
  const std::array<int32_t, 4> slice1_size_data{1, 2, 896, 1408};
  auto& slice1_size = tensor_pool.CreateStaticTensor(
      QNN_DATATYPE_INT_32, {}, {slice1_size_data.size()},
      slice1_size_data.size() * sizeof(slice1_size_data[0]),
      slice1_size_data.data());
  ...
  for (size_t i = 0; i < sha_size; ++i) {
    ASSERT_TRUE(op_wrappers[8 + (sha_size - 1) * i].IsOpCode(
        QnnOpCode::kElementWiseMultiply));
    ASSERT_TRUE(
        op_wrappers[9 + (sha_size - 1) * i].IsOpCode(QnnOpCode::kMatMul));
    ASSERT_TRUE(op_wrappers[10 + (sha_size - 1) * i].IsOpCode(
        QnnOpCode::kElementWiseAdd));
    ASSERT_TRUE(
        op_wrappers[11 + (sha_size - 1) * i].IsOpCode(QnnOpCode::kMatMul));
    ASSERT_TRUE(
        op_wrappers[12 + (sha_size - 1) * i].IsOpCode(QnnOpCode::kConcat));
    ASSERT_TRUE(op_wrappers[13 + (sha_size - 1) * i].IsOpCode(
        QnnOpCode::kElementWiseAdd));
    ASSERT_TRUE(
        op_wrappers[14 + (sha_size - 1) * i].IsOpCode(QnnOpCode::kReshape));
    ASSERT_TRUE(
        op_wrappers[15 + (sha_size - 1) * i].IsOpCode(QnnOpCode::kSoftmax));
    ASSERT_TRUE(op_wrappers[16 + (sha_size - 1) * i].IsOpCode(
        QnnOpCode::kStridedSlice));
    ASSERT_TRUE(op_wrappers[17 + (sha_size - 1) * i].IsOpCode(
        QnnOpCode::kStridedSlice));
    ASSERT_TRUE(
        op_wrappers[18 + (sha_size - 1) * i].IsOpCode(QnnOpCode::kMatMul));
    ASSERT_TRUE(
        op_wrappers[19 + (sha_size - 1) * i].IsOpCode(QnnOpCode::kMatMul));
    ASSERT_TRUE(op_wrappers[20 + (sha_size - 1) * i].IsOpCode(
        QnnOpCode::kElementWiseAdd));
  }
