...
TEST(MHASHATest, FastVlmKVSwapped) {
  ...
  op_wrappers.emplace_back(CreateElementWiseMulOp(
      q_scale_in, q_scale_mul_const, q_scale_mul_output));
  ...
  op_wrappers.emplace_back(CreateMatmulOp(q_scale_reshape_output, k_cache,
                                          q_kcache_matmul_output, false,
                                          false));
  ...
  op_wrappers.emplace_back(CreateMatmulOp(q_scale_reshape_output, k_slice,
                                          q_kslice_matmul_output, false,
                                          false));
  ...
  op_wrappers.emplace_back(
      CreateConcatenationOp({q_kcache_matmul_output, q_kslice_matmul_output}, qk_concat_output, 3));
  ...
  auto& qk_vcache_matmul_output = tensor_pool.CreateNativeTensor(
      QNN_DATATYPE_SFIXED_POINT_16, quant_param, {1, 8, 256, 128});
  op_wrappers.emplace_back(CreateMatmulOp(qk_vcache_slice_output, v_cache,
                                          qk_vcache_matmul_output, false,
                                          false));
  ...
  auto& qk_vslice_matmul_output = tensor_pool.CreateNativeTensor(
      QNN_DATATYPE_SFIXED_POINT_16, quant_param, {1, 8, 256, 128});
  op_wrappers.emplace_back(CreateMatmulOp(qk_vslice_slice_output, v_slice,
                                          qk_vslice_matmul_output, false,
                                          false));
