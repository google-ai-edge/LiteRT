...
std::vector<OpWrapper> CreateSelectOp(
  ...
  std::vector<std::int16_t> all_negative_data(num_element,
                                              static_tensor_data.value()[0]);
  auto& input_2 = tensor_pool.CreateStaticTensor(
      QNN_DATATYPE_SFIXED_POINT_16, quant_param, tensor_dims,
      all_negative_data.size() * sizeof(all_negative_data[0]),
      all_negative_data.data());
...
size_t TransformQuantizeInMask(
  ...
  auto new_ops = CreateSelectOp(ops, start_index, tensor_pool, pattern_size);
  if (new_ops.empty()) {
    QNN_LOG_WARNING(
