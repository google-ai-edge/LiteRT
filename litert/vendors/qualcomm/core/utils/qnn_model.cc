...
bool QnnModel::ValidateOpConfig(std::vector<qnn::OpWrapper>& ops) {
  return std::all_of(
      ops.begin(), ops.end(), [this](OpWrapper& op_wrapper) -> bool {
        return QNN_SUCCESS == api_->backendValidateOpConfig(
