...
std::vector<std::int8_t> UnpackIntData(const void* src, size_t src_bytes) {
  constexpr uint32_t values_per_byte = 8 / BitWidth;
  std::vector<std::int8_t> dst;
  dst.reserve(src_bytes * values_per_byte);
  const std::uint8_t* byte_data = static_cast<const std::uint8_t*>(src);
  
  if constexpr (BitWidth == kQuantBitWidth2) {
    for (size_t i = 0; i < src_bytes; i++) {
      const int8_t* lut_entry = &kInt2LUT[byte_data[i] * 4];
      dst.emplace_back(lut_entry[0]);
      dst.emplace_back(lut_entry[1]);
      dst.emplace_back(lut_entry[2]);
      dst.emplace_back(lut_entry[3]);
    }
  } else if constexpr (BitWidth == kQuantBitWidth4) {
    for (size_t i = 0; i < src_bytes; i++) {
