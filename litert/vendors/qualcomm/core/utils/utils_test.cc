...
TEST(MiscTests, UnpackIntData_Int2) {
  ...
  // bits 6-7: 11 -> -1
  {
    ...
    auto dst = UnpackIntData<kQuantBitWidth2>(src, 1);
  ...
  //   bits 6-7: 00 ->  0
  {
    ...
    auto dst = UnpackIntData<kQuantBitWidth2>(src, 2);
...
TEST(MiscTests, UnpackIntData_Int4) {
  ...
  {
    const uint8_t src[] = {0xE4};
    auto dst = UnpackIntData<kQuantBitWidth4>(src, 1);
  ...
  // Multiple bytes: {0xE4, 0x87}
  {
    ...
    auto dst = UnpackIntData<kQuantBitWidth4>(src, 2);
    ASSERT_EQ(dst.size(), 4);
    EXPECT_EQ(dst[0], 4);
    EXPECT_EQ(dst[1], -2);
    EXPECT_EQ(dst[2], 7);
