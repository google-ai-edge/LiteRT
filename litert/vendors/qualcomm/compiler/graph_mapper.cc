...
LiteRtStatus GraphMapper::InitQnnGraph(absl::string_view qnn_graph_name,
  ...
  // Each backend owns its graph-create config logic behind the
  // QnnBackend::BuildGraphConfigs hook (HTP picks fp16/legacy, IR derives the
  // DLC path, LPAI supplies its prepare config, DSP returns none). The returned
  // builder owns the config storage; keep it alive across the graphCreate call.
  ::qnn::GraphConfigBuilder graph_configs;
  if (auto* backend = qnn_.GetBackend(); backend != nullptr) {
    graph_configs = backend->BuildGraphConfigs(options, qnn_graph_name);
  }

  LITERT_RETURN_STATUS_IF_QNN_NOT_OK(
      qnn_.Api()->graphCreate(context_handle_, qnn_graph_name.data(),
                              graph_configs.Configs().data(), &QnnGraph()));
