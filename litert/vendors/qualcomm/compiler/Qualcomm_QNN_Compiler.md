...
| `kLiteRtOpCodeTflConv2d` | Legalized to `QNN_OP_CONV_2D`. Supports fused activation. Weights are transposed. |
...
| `kLiteRtOpCodeTflDynamicUpdateSlice`| Legalized to a sequence of ops including `QNN_OP_REDUCE_SUM` and `QNN_OP_ELEMENT_WISE_SELECT`. |
...
| `kLiteRtOpCodeTflFullyConnected` | Legalized to `QNN_OP_FULLY_CONNECTED`. HTP optimization may convert it to `QNN_OP_CONV_2D`. |
...
| `kLiteRtOpCodeTflTransposeConv` | Legalized to `QNN_OP_TRANSPOSE_CONV_2D`. Supports fused activation. Weights are transposed. |
