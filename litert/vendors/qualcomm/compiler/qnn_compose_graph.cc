...
LiteRtStatus ConvertOp(const bool use_htp_preferences,
  ...
  switch (litert_op.Code()) {
    ...
    case LiteRtOpCode::kLiteRtOpCodeTflSum: {
      ...
      op_wrappers =
          ::qnn::BuildReduceOp(tensor_pool, input_tensors, output_tensors,
                               keep_dims, QNN_OP_REDUCE_SUM);
      break;
    }
    ...
    case LiteRtOpCode::kLiteRtOpCodeTflConv3d: {
      ...
      int32_t dilation_h_factor;
      LITERT_RETURN_IF_ERROR(
          LiteRtGetConv3dDilationWOption(litert_op.Get(), &dilation_h_factor));
      uint32_t fused_activation;
