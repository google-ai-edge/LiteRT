...
- `${QAIRT}/lib/hexagon-v81/unsigned/libQnnHtpPrepare.so`
...
For example, we execute a model on Android device with burst model via HTP Backend:
...
For example, we execute a model on IoT device with burst model via HTP Backend:
