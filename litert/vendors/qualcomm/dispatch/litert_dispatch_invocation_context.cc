...
LiteRtDispatchInvocationContextT::~LiteRtDispatchInvocationContextT() {
  // Manual: sync downvote handled by backend dtor. Auto: schedule final downvote.
...
Expected<void> LiteRtDispatchInvocationContextT::Execute() {
  // Auto mode does per-inference upvote + debounced downvote; manual mode is
  // unchanged (upvote at init).
...
Expected<void> LiteRtDispatchInvocationContextT::SetOptions(
  ...
  // Parse the LiteRtOptions into a qnn::Options struct using the same path as
  // the device-context initialisation in dispatch_api.cc.
  ...
  auto payload = opaque_options->FindOpaqueOptions(
  ...
  if (LrtCreateQualcommOptionsFromToml(
          reinterpret_cast<const char*>(payload.Value()),
  ...
  // Manual mode: forward to backend so a runtime mode change re-votes
  // (same-mode calls are skipped inside the backend). Auto mode defers to
  // Execute(). Only the active backend's ctrl mode matters.
  bool manual = false;
  switch (qnn_manager_.GetOptions().GetBackendType()) {
    case ::qnn::BackendType::kHtpBackend:
      manual = qnn_options.GetHtpPerformanceMode() !=
                   ::qnn::HtpPerformanceMode::kDefault &&
               qnn_options.GetHtpPerfCtrlMode() == ::qnn::HtpPerfCtrlMode::kManual;
      break;
    case ::qnn::BackendType::kDspBackend:
      manual = qnn_options.GetDspPerformanceMode() !=
