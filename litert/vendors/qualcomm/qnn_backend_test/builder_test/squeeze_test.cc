...
TEST_P(QnnModelTest, SqueezeRemovesMultipleDims) {
  ...
  auto ops = ::qnn::BuildSqueezeOp(
      tensor_pool_, {input_tensor}, {output_tensor}, {0, 2});
...
TEST_P(QnnModelTest, SqueezeQuantizedInt8) {
  ...
  auto ops = ::qnn::BuildSqueezeOp(
      tensor_pool_, {input_tensor}, {output_tensor}, {0});
  qnn_model_.MoveOpsToGraph(std::move(ops));
  ASSERT_TRUE(qnn_model_.Finalize());
  ...
  ASSERT_THAT(
      ...
      testing::Pointwise(testing::Eq(),
                         std::vector<std::int8_t>{10, 20, 30, 40}));
#endif
}
...
TEST_P(QnnModelTest, SqueezeNegativeDims) {
  ...
  auto ops = ::qnn::BuildSqueezeOp(
      tensor_pool_, {input_tensor}, {output_tensor}, {-4, -2});
  qnn_model_.MoveOpsToGraph(std::move(ops));
  ASSERT_TRUE(qnn_model_.Finalize());
...
TEST_P(QnnModelTest, SqueezeEmptyDims) {
  ...
  auto ops = ::qnn::BuildSqueezeOp(
      tensor_pool_, {input_tensor}, {output_tensor}, {});
  qnn_model_.MoveOpsToGraph(std::move(ops));
  ASSERT_TRUE(qnn_model_.Finalize());
