...
void TestBlockwiseFullyConnected(::qnn::QnnModel& qnn_model,
  ...
  const std::vector<std::int32_t> zero_points(4, 0);
  auto& input = tensor_pool.CreateInputTensorWithName(
      "input_" + suffix, QNN_DATATYPE_FLOAT_32, {}, {1, 32});
  ::qnn::BwFloatBlockQuantizeParamsWrapper weight_quant(bitwidth, {1, 16},
                                                        scales, zero_points);
  const auto packed_weights = PackLowBitWeights(bitwidth, weights);
  auto& weight = tensor_pool.CreateStaticTensor(
      QNN_DATATYPE_SFIXED_POINT_8, weight_quant, {2, 32}, packed_weights.size(),
      packed_weights.data());
  auto& output = tensor_pool.CreateOutputTensorWithName(
      "output_" + suffix, QNN_DATATYPE_FLOAT_32, {}, {1, 2});

  auto fc_ops = ::qnn::BuildFullyConnectedOp(
      tensor_pool, {input, weight}, {output}, false, false, kSdkVersion);
  ASSERT_EQ(fc_ops.size(), 3u);
  EXPECT_TRUE(fc_ops[0].IsOpCode(::qnn::QnnOpCode::kCast));
  EXPECT_TRUE(fc_ops[1].IsOpCode(::qnn::QnnOpCode::kFullyConnected));
  EXPECT_TRUE(fc_ops[2].IsOpCode(::qnn::QnnOpCode::kCast));
  const auto& qnn_quant =
      fc_ops[1].GetInputTensor(1).GetQnnTensor().v2.quantizeParams;
  ...
  qnn_model.MoveOpsToGraph(std::move(fc_ops));
...
TEST_P(QnnModelTest, BlockwiseW2Fp16) {
  TestBlockwiseFullyConnected(qnn_model_, tensor_pool_, 2);
}

TEST_P(QnnModelTest, BlockwiseW4Fp16) {
  TestBlockwiseFullyConnected(qnn_model_, tensor_pool_, 4);
}

TEST_P(QnnModelTest, BlockwiseW8Fp16) {
  TestBlockwiseFullyConnected(qnn_model_, tensor_pool_, 8);
}
