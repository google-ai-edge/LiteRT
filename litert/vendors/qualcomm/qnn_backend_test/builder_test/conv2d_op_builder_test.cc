...
void TestBlockwiseConv2d(::qnn::QnnModel& qnn_model,
  ...
  ::qnn::BwFloatBlockQuantizeParamsWrapper weight_quant(bitwidth, {1, 1, 1, 16},
                                                        scales, zero_points);
  const auto packed_weights = PackLowBitWeights(bitwidth, weights);
  auto& weight = tensor_pool.CreateStaticTensor(
      QNN_DATATYPE_SFIXED_POINT_8, weight_quant, {2, 1, 1, 32},
  ...
  auto conv_ops =
      ::qnn::BuildConv2dOp(tensor_pool, {input_fp16, weight}, {output_fp16}, 1,
                           1, 1, 1, ::qnn::PaddingType::Valid, false);
  ASSERT_EQ(conv_ops.size(), 1u);
  EXPECT_TRUE(conv_ops[0].IsOpCode(::qnn::QnnOpCode::kConv2d));
  const auto& qnn_quant =
      conv_ops[0].GetInputTensor(1).GetQnnTensor().v2.quantizeParams;
  ...
  qnn_model.MoveOpsToGraph(std::move(conv_ops));
...
TEST_P(QnnModelTest, BlockwiseW2Fp16) {
  TestBlockwiseConv2d(qnn_model_, tensor_pool_, 2);
}

TEST_P(QnnModelTest, BlockwiseW4Fp16) {
  TestBlockwiseConv2d(qnn_model_, tensor_pool_, 4);
}

TEST_P(QnnModelTest, BlockwiseW8Fp16) {
  TestBlockwiseConv2d(qnn_model_, tensor_pool_, 8);
}
