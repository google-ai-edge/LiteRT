...
Expected<OpWrapper> BuildGeneralGatherOp(const Op& op,
                                         const std::vector<int32_t>& input_order,
                                         int32_t& axis) {
...
Expected<OpWrapper> BuildGatherOp(const Op& op) {
  ...
  LITERT_ASSIGN_OR_RETURN(auto op_wrapper, BuildGeneralGatherOp(op, gather_input_order, axis));
...
Expected<OpWrapper> BuildEmbeddingLookupOp(const Op& op) {
  ...
  LITERT_ASSIGN_OR_RETURN(auto op_wrapper, BuildGeneralGatherOp(op, embeding_lookup_input_order, axis));
  return op_wrapper;
}
