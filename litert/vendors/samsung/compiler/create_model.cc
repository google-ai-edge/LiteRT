  ...
  for (int op_idx = 0; op_idx < ops.size(); ++op_idx) {
    ...
    switch (op.Code()) {
      ...
      case kLiteRtOpCodeTflGather:
        op_wrapper = std::move(BuildGatherOp(op));
        break;
      ...
      case kLiteRtOpCodeTflMaximum:
        op_wrapper = std::move(BuildMaxOp(op));
        break;
