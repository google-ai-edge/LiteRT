...
TEST(OpaqueOptionsTest, FindOpaqueOptions) {
  ...
  auto opq_opts = std::move(opq_opts_result.Value());

  auto result = opq_opts.FindOpaqueOptions(kDummyOpaqueOptionsId);
  ASSERT_TRUE(result.HasValue());
  EXPECT_EQ(reinterpret_cast<DummyOpaqueOptions*>(result.Value())->dummy_option,
            123);

  result = opq_opts.FindOpaqueOptions("not_found_id");
