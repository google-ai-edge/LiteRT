...
struct CaptureMatcher {
  ...
  CaptureMatcher(absl::string_view l, T* s, SubMatcher m)
...
template <typename T, typename Matcher>
auto m_Capture(T* s, Matcher&& m, absl::string_view label = "CaptureMatcher") {
  return CaptureMatcher<T, std::decay_t<Matcher>>(label, s,
                                                  std::forward<Matcher>(m));
}
...
struct SameAsMatcher {
  absl::string_view label;
  T* storage;
  explicit SameAsMatcher(absl::string_view l, T* s) : label(l), storage(s) {}
...
template <typename T>
auto m_SameAs(T* s, absl::string_view label = "SameAsMatcher") {
  return SameAsMatcher<T>(label, s);
}
