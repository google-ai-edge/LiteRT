...
struct OpVariadicMatcher {
  ...
  explicit OpVariadicMatcher(LiteRtOpCode c, InputMatchers... m)
      : code(c), input_matchers(std::move(m)...) {}
...
template <typename... Args>
auto m_OpVariadic(LiteRtOpCode code, Args&&... args) {
  return OpVariadicMatcher<std::decay_t<Args>...>(code,
...
struct CommutativeOpMatcher {
  ...
  CommutativeOpMatcher(LiteRtOpCode c, Matcher1 matcher1, Matcher2 matcher2)
      : code(c), m1(std::move(matcher1)), m2(std::move(matcher2)) {}
...
template <typename Matcher1, typename Matcher2>
auto m_CommutativeOp(LiteRtOpCode code, Matcher1&& m1, Matcher2&& m2) {
  return CommutativeOpMatcher<std::decay_t<Matcher1>, std::decay_t<Matcher2>>(
      code, std::forward<Matcher1>(m1), std::forward<Matcher2>(m2));
}
