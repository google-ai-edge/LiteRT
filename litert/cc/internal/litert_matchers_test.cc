...
TEST(MatchersTest, OpCodeMatch) {
  ...
  Op cc_op(&op);
  // m_Op checks exact input count (0), so it should fail.
  EXPECT_FALSE(Match(cc_op, m_Op(kLiteRtOpCodeTflAdd)));

  // m_OpCode only checks opcode.
...
TEST(MatchersTest, Combinators) {
  ...
  Op cc_op(&op);
  EXPECT_TRUE(Match(cc_op, m_AllOf(m_OpCode(kLiteRtOpCodeTflAdd), m_AnyOp())));
...
TEST(MatchersTest, OpVariadicMatching) {
  ...
  EXPECT_TRUE(Match(cc_op, m_OpVariadic(kLiteRtOpCodeTflConcatenation, m_Any(),
                                        m_Any(), m_Any())));

  // Match 4 inputs (fail)
  EXPECT_FALSE(Match(cc_op, m_OpVariadic(kLiteRtOpCodeTflConcatenation, m_Any(),
                                         m_Any(), m_Any(), m_Any())));
}
...
TEST(MatchersTest, OutputIndexMatching) {
  ...
  EXPECT_FALSE(Match(cc_out0, m_OutputIndex(0, m_OpCode(kLiteRtOpCodeTflAdd))));
}
...
TEST(MatchersTest, ComplexResnetBlock) {
  ...
  auto match_resnet_add =
      m_CommutativeOp(kLiteRtOpCodeTflAdd,
...
TEST(MatchersTest, VariadicTypedConcat) {
  ...
  EXPECT_TRUE(
      Match(root, m_OpVariadic(kLiteRtOpCodeTflConcatenation, float_tensor,
...
TEST(MatchersTest, AllInOneIntegration) {
  ...
  auto match_mul_inputs = m_CommutativeOp(
      kLiteRtOpCodeTflMul,
  ...
  auto match_root = m_AllOf(
      m_OpVariadic(kLiteRtOpCodeTflConcatenation, match_split_out0,
                   match_split_out1,
