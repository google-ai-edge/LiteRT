...
TEST(MatchersTest, AllMatchersFailureLogging) {
  ...
  Op captured(nullptr);
  VerifyFailure(
      DebugMatch(cc_op, m_Capture(&captured, m_OpCode<kLiteRtOpCodeTflMul>(),
                                  "LblCap")),
      m_Capture(&captured, m_OpCode<kLiteRtOpCodeTflMul>(), "LblCap"), cc_op,
      "OpCodeMatcher", "OpCode mismatch");
  ...
  VerifyFailure(
      DebugMatch(cc_tensor_float, m_SameAs(&cc_tensor_const_copy, "LblSame")),
      m_SameAs(&cc_tensor_const_copy, "LblSame"), cc_tensor_float, "LblSame",
      "Object mismatch");
