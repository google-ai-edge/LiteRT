...
#ifndef ODML_LITERT_LITERT_CC_INTERNAL_LITERT_DETAILED_MODEL_H_
#define ODML_LITERT_LITERT_CC_INTERNAL_LITERT_DETAILED_MODEL_H_
...
// DetailedModel. C++ equivalent of LiteRtModel.
class DetailedModel : public litert::Model {
 public:
  DetailedModel() = default;

  static DetailedModel CreateFromOwnedHandle(LiteRtModel model) {
    return DetailedModel(model, OwnHandle::kYes);
  }

  static DetailedModel CreateFromNonOwnedHandle(LiteRtModel model) {
    return DetailedModel(model, OwnHandle::kNo);
  }

  static Expected<DetailedModel> CreateFromFile(const std::string& filename) {
    LiteRtModel model;
  ...
  // the model.
  static Expected<DetailedModel> CreateFromBuffer(BufferRef<uint8_t> buffer) {
    LiteRtModel model;
    if (auto status =
 ...
 private:
  ...
  DetailedModel(LiteRtModel model, OwnHandle owned)
...
#endif  // ODML_LITERT_LITERT_CC_INTERNAL_LITERT_DETAILED_MODEL_H_
