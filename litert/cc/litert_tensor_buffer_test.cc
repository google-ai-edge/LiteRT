...
TEST(TensorBuffer, ClBuffer) {
  ...
  const RankedTensorType kTensorType(kTestTensorType);
  constexpr auto kTensorBufferType = TensorBufferType::OpenClBuffer;
...
TEST(TensorBuffer, Ahwb) {
  ...
  const RankedTensorType kTensorType(kTestTensorType);
  constexpr auto kTensorBufferType = TensorBufferType::Ahwb;
...
TEST(TensorBuffer, Ion) {
  ...
  const RankedTensorType kTensorType(kTestTensorType);
  constexpr auto kTensorBufferType = TensorBufferType::Ion;
...
TEST(TensorBuffer, DmaBuf) {
  ...
  const RankedTensorType kTensorType(kTestTensorType);
  constexpr auto kTensorBufferType = TensorBufferType::DmaBuf;
...
TEST(TensorBuffer, FastRpc) {
  ...
  const RankedTensorType kTensorType(kTestTensorType);
  constexpr auto kTensorBufferType = TensorBufferType::FastRpc;
...
TEST(TensorBuffer, ClBufferFromGlBuffer) {
  ...
  LITERT_ASSERT_OK_AND_ASSIGN(
      ...
      TensorBuffer::CreateManaged(
          user_gpu_env->GetEnvironment(), TensorBufferType::GlBuffer,
...
TEST(TensorBuffer, GetGlBufferFromAhwb) {
  ...
  LITERT_ASSERT_OK_AND_ASSIGN(
      ...
      TensorBuffer::CreateManaged(
          user_gpu_env->GetEnvironment(), TensorBufferType::Ahwb,
          RankedTensorType(kTestTensorType), sizeof(kTensorData)));
...
TEST(TensorBuffer, CreateManagedClBuffer) {
  ...
  LITERT_ASSERT_OK_AND_ASSIGN(
      TensorBuffer tensor_buffer,
      TensorBuffer::CreateManaged(
          user_gpu_env->GetEnvironment(), TensorBufferType::OpenClBuffer,
          RankedTensorType(kTestTensorType), sizeof(kTensorData)));
...
TEST(TensorBuffer, GetClBufferFromAhwb) {
  ...
  LITERT_ASSERT_OK_AND_ASSIGN(
      ...
      TensorBuffer::CreateFromClBuffer(user_gpu_env->GetEnvironment(),
                                       RankedTensorType(kTestTensorType),
                                       TensorBufferType::OpenClBufferPacked,
                                       cl_buffer, sizeof(kTensorData)));
...
TEST(TensorBuffer, GetAhwb) {
  ...
  LITERT_ASSERT_OK_AND_ASSIGN(
      TensorBuffer tensor_buffer,
      TensorBuffer::CreateManaged(env.Get(), TensorBufferType::Ahwb,
                                  RankedTensorType(kTestTensorType),
                                  sizeof(kTensorData)));
