...
TEST(CcRankedTensorSpecBuilderTest, TestBuild) {
  auto ranked_tensor_spec =
      RankedTensorSpecBuilder()
          .WithRankedTensorType(RankedTensorType(kTensorType))
...
TEST(CcRewriterTest, TestBuildRankedTensor) {
  ...
  auto ranked_tensor_spec = RankedTensorSpecBuilder()
                                .WithRankedTensorType(tensor_type)
                                .WithTensorName(std::string(kTensorName))
                                .Build();
...
TEST(CcRewriterTest, TestBuildWeights) {
  absl::Span<const float> data = absl::MakeConstSpan({1.0f, 2.0f, 3.0f});
  ...
  auto tensor_spec =
      RankedTensorSpecBuilder().WithRankedTensorType(tensor_type).Build();
