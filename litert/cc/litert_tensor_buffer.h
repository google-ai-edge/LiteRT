...
class TensorBuffer : public internal::BaseHandle<LiteRtTensorBuffer> {
 public:
  ...
  static TensorBuffer WrapCObject(LiteRtTensorBuffer tensor_buffer,
                                  OwnHandle owned) {
    LITERT_ASSIGN_OR_ABORT(auto env, Environment::Create({}));
    return TensorBuffer(std::move(env), tensor_buffer, owned);
  }
 ...
 private:
  ...
  /// @warning This is only used for legacy cases, and will be removed in the
  /// future.
  [[deprecated("Use the other constructor instead")]]
  explicit TensorBuffer(Environment&& litert_env,
                        LiteRtTensorBuffer tensor_buffer, OwnHandle owned)
      : internal::BaseHandle<LiteRtTensorBuffer>(
            tensor_buffer,
            [runtime = litert_env.GetHolder().runtime](
                LiteRtTensorBuffer tensor_buffer) {
              runtime->DestroyTensorBuffer(tensor_buffer);
            },
            owned),
        litert_env_(std::make_unique<Environment>(std::move(litert_env))),
        env_(litert_env_->GetHolder()) {}

  // This is only used for managing the lifetime of ad-hoc environment for
  // legacy cases.
  [[deprecated("Do not use this field.")]]
  std::unique_ptr<Environment> litert_env_;
