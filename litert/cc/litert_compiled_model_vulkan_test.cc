...
struct TestParams {
  ...
  LiteRtDelegateBufferStorageType storage_type;
};
...
Expected<Options> CreateGpuOptions(const TestParams& params) {
  ...
  LITERT_RETURN_IF_ERROR(gpu_options.SetDelegatePrecision(params.precision));
  LITERT_RETURN_IF_ERROR(gpu_options.SetBufferStorageType(params.storage_type));
...
void BasicTest(const TestParams& params) {
  ...
  LITERT_ASSERT_OK_AND_ASSIGN(auto options, CreateGpuOptions(params));
...
class CompiledModelVulkanTest : public ::testing::TestWithParam<TestParams> {};

TEST_P(CompiledModelVulkanTest, Basic) {
  // To workaround the memory leak in Nvidia's driver
  absl::LeakCheckDisabler disable_leak_check;
  BasicTest(GetParam());
}

void GpuEnvironmentTest(const TestParams& params) {
  LITERT_ASSERT_OK_AND_ASSIGN(
      auto model,
      Model::CreateFromFile(testing::GetTestFilePath(kModelFileName)));
  ...
  ABSL_LOG(INFO) << "Vulkan env: "
                 << reinterpret_cast<void*>(std::any_cast<int64_t>(vulkan_env));
}

TEST_P(CompiledModelVulkanTest, GpuEnvironment) {
  // To workaround the memory leak in Nvidia's driver
  absl::LeakCheckDisabler disable_leak_check;
  GpuEnvironmentTest(GetParam());
}

INSTANTIATE_TEST_SUITE_P(
    CompiledModelWebGpuTest, CompiledModelVulkanTest,
    ::testing::Values(
        TestParams{kLiteRtDelegatePrecisionDefault,
                   kLiteRtDelegateBufferStorageTypeDefault},
        TestParams{kLiteRtDelegatePrecisionFp16,
                   kLiteRtDelegateBufferStorageTypeBuffer},
        TestParams{kLiteRtDelegatePrecisionFp16,
                   kLiteRtDelegateBufferStorageTypeTexture2D},
        TestParams{kLiteRtDelegatePrecisionFp32,
                   kLiteRtDelegateBufferStorageTypeBuffer},
        TestParams{kLiteRtDelegatePrecisionFp32,
                   kLiteRtDelegateBufferStorageTypeTexture2D}));
