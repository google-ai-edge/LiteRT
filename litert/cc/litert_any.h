  ...
  return std::visit(
      [](auto&& arg) -> Expected<LiteRtAny> {
        ...
        if constexpr (std::is_same_v<T, std::monostate>) {
          return LiteRtAny{.type = kLiteRtAnyTypeNone};
        } else if constexpr (std::is_same_v<T, bool>) {
          return LiteRtAny{.type = kLiteRtAnyTypeBool, .bool_value = arg};
        } else if constexpr (std::is_same_v<T, int8_t> ||
                             std::is_same_v<T, int16_t> ||
                             std::is_same_v<T, int32_t> ||
                             std::is_same_v<T, int64_t>) {
          return LiteRtAny{.type = kLiteRtAnyTypeInt,
                           .int_value = static_cast<int64_t>(arg)};
        } else if constexpr (std::is_same_v<T, float> ||
                             std::is_same_v<T, double>) {
          return LiteRtAny{.type = kLiteRtAnyTypeReal,
                           .real_value = static_cast<double>(arg)};
        } else if constexpr (std::is_same_v<T, const char*>) {
          return LiteRtAny{.type = kLiteRtAnyTypeString, .str_value = arg};
        } else if constexpr (std::is_same_v<T, absl::string_view>) {
          return LiteRtAny{.type = kLiteRtAnyTypeString,
                           .str_value = arg.data()};
        } else if constexpr (std::is_same_v<T, const void*>) {
          return LiteRtAny{.type = kLiteRtAnyTypeVoidPtr, .ptr_value = arg};
