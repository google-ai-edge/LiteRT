...
TEST(F16InF23Test, Works) {
  ...
  static constexpr auto kSmallestF16SubNormal = 0.000000059604645;
  static constexpr auto kLargestF16Normal = 65504.0f;

  for (int i = 0; i < device.NumValues(); ++i) {
    const float f = std::abs(f16_gen(device));
    EXPECT_GE(f, kSmallestF16SubNormal);
    EXPECT_LE(f, kLargestF16Normal);
    const auto mant_bits =
        std::bitset<32>(*reinterpret_cast<const uint32_t*>(&f));
    for (int i = 0; i < 13; ++i) {
      EXPECT_FALSE(mant_bits.test(i));
    }
  }
}
