...
class IntelOpenVinoOptionsTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    options_ = std::make_unique<IntelOpenVinoOptions>(std::move(options_result.Value()));
  }
...
TEST_F(IntelOpenVinoOptionsTest, DefaultValues) {
  ...
  EXPECT_EQ(options_->GetPerformanceMode(), kLiteRtIntelOpenVinoPerformanceModeLatency);
}
...
TEST_F(IntelOpenVinoOptionsTest, SetAndGetPerformanceMode) {
  ...
  EXPECT_EQ(options_->GetPerformanceMode(), kLiteRtIntelOpenVinoPerformanceModeThroughput);

  options_->SetPerformanceMode(kLiteRtIntelOpenVinoPerformanceModeCumulativeThroughput);
  EXPECT_EQ(options_->GetPerformanceMode(), kLiteRtIntelOpenVinoPerformanceModeCumulativeThroughput);
}
...
TEST_F(IntelOpenVinoOptionsTest, SetAndGetConfigsMapOption) {
  ...
  std::map<std::string, std::string> expected_configs = {
    ...
    {"NPU_COMPILATION_MODE_PARAMS", "compute-layers-with-higher-precision=Sigmoid"},
  ...
  for (int i = 0; i < options_->GetNumConfigsMapOptions(); ++i) {
    ...
    ASSERT_TRUE(options_->GetConfigsMapOption(i, &key, &value));
...
TEST_F(IntelOpenVinoOptionsTest, ConfigsMapMultipleOptions) {
  ...
  for (int i = 0; i < options_->GetNumConfigsMapOptions(); ++i) {
    const char* key = nullptr;
    const char* value = nullptr;
    if (options_->GetConfigsMapOption(i, &key, &value)) {
      actual_configs[key] = value;
    }
  }
...
TEST_F(IntelOpenVinoOptionsTest, OptionsIdentifier) {
  EXPECT_EQ(std::string(IntelOpenVinoOptions::Discriminator()), "intel_openvino");
}
...
TEST(IntelOpenVinoOptionsCApiTest, CreateAndDestroy) {
  ...
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsGet(opaque_options, &options), kLiteRtStatusOk);

  // Test setting and getting values through C API
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsSetDeviceType(options, kLiteRtIntelOpenVinoDeviceTypeCPU), 
            kLiteRtStatusOk);

  LiteRtIntelOpenVinoDeviceType device_type;
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsGetDeviceType(options, &device_type), kLiteRtStatusOk);
...
TEST(IntelOpenVinoOptionsCApiTest, InvalidArguments) {
  ...
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsCreate(nullptr), kLiteRtStatusErrorInvalidArgument);

  LiteRtOpaqueOptions opaque_options;
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsCreate(&opaque_options), kLiteRtStatusOk);

  EXPECT_EQ(LiteRtIntelOpenVinoOptionsGet(nullptr, nullptr), kLiteRtStatusErrorInvalidArgument);

  LiteRtIntelOpenVinoOptions options;
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsGet(opaque_options, &options), kLiteRtStatusOk);

  // Test setting with null options
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsSetDeviceType(nullptr, kLiteRtIntelOpenVinoDeviceTypeCPU),
...
TEST(IntelOpenVinoOptionsCApiTest, ConfigsMapOptions) {
  ...
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsSetConfigsMapOption(options, "INFERENCE_PRECISION_HINT", "f16"),
            kLiteRtStatusOk);
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsSetConfigsMapOption(options, "NPU_COMPILATION_MODE_PARAMS", "compute-layers-with-higher-precision=Sigmoid"),
            kLiteRtStatusOk);
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsSetConfigsMapOption(options, "CACHE_DIR", "/tmp/cache"),
            kLiteRtStatusOk);

  // Test null pointer handling for configs_map
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsSetConfigsMapOption(nullptr, "KEY", "VALUE"),
            kLiteRtStatusErrorInvalidArgument);
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsSetConfigsMapOption(options, nullptr, "VALUE"),
            kLiteRtStatusErrorInvalidArgument);
  EXPECT_EQ(LiteRtIntelOpenVinoOptionsSetConfigsMapOption(options, "KEY", nullptr),
            kLiteRtStatusErrorInvalidArgument);
