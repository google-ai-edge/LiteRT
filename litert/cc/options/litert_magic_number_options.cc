...
Expected<MagicNumberConfigsPtr> CreateMagicNumberConfigs(
  ...
  if (num_configs >
    ...
    return Unexpected(
        Error(kLiteRtStatusErrorInvalidArgument,
              "Number of magic number configs exceeds supported range"));
  }
  ...
  auto* raw =
      static_cast<LiteRtMagicNumberConfigs*>(std::calloc(1, total_size));

  if (raw == nullptr) {
    return Unexpected(Error(kLiteRtStatusErrorMemoryAllocationFailure,
                            "Failed to allocate LiteRtMagicNumberConfigs"));
  }
...
Expected<MagicNumberVerificationsPtr> CreateMagicNumberVerifications(
  ...
  auto* raw =
      static_cast<LiteRtMagicNumberVerifications*>(std::calloc(1, total_size));
