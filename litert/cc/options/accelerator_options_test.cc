...
TEST(GpuAcceleratorCompilationOptions, SetModelToken) {
  LITERT_ASSERT_OK_AND_ASSIGN(GpuOptions options, GpuOptions::Create());
  LITERT_ASSERT_OK_AND_ASSIGN(LiteRtGpuOptionsPayload payload,
                              options.GetData<LiteRtGpuOptionsPayloadT>());
  // Check the default value.
  const char* model_token = nullptr;
  LITERT_ASSERT_OK(LiteRtGetGpuAcceleratorCompilationOptionsModelToken(
      &model_token, payload));
  EXPECT_EQ(model_token, nullptr);

  options.SetModelToken("model_cache");
  LITERT_ASSERT_OK(LiteRtGetGpuAcceleratorCompilationOptionsModelToken(
      &model_token, payload));
  EXPECT_EQ(model_token, "model_cache");
}
