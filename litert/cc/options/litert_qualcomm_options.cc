...
void QualcommOptions::SetVtcmSize(const std::uint32_t& vtcm_size) {
...
void QualcommOptions::SetHvxThread(const std::uint32_t& hvx_thread) {
  internal::AssertOk(LiteRtQualcommOptionsSetHvxThread, Data(),
                     hvx_thread);
}

std::uint32_t QualcommOptions::GetHvxThread() {
  std::uint32_t hvx_thread;
