...
std::vector<std::vector<LiteRtOp>> GetPartitionsFromFlatListV2(
  ...
  for (auto& op : subgraph->Ops()) {
    ...
    if (it == op_to_partition_index.end()) {
      sorted_ops.push_back({op, max_partition_index});
    } else {
      sorted_ops.push_back({op, op_to_partition_index[op]});
    }
  }
  ...
  for (auto& entry : sorted_ops) {
    if (entry.second != current_partition_index) {
      if (!current_partition.empty()) {
        partitions.push_back(current_partition);
        current_partition.clear();
      }
      current_partition_index = entry.second;
    }
    if (current_partition_index != max_partition_index) {
      current_partition.push_back(entry.first);
    }
  }
  if (!current_partition.empty()) {
    partitions.push_back(current_partition);
...
std::vector<std::vector<LiteRtOp>> GroupPartitions(
    const std::vector<LiteRtOpWithPartitionIndex>& ops,
    LiteRtSubgraph subgraph) {
  // TODO: @yunandrew - Add compiler option to select between disjoint sets and
  // v2 partitioning, because DisjointSets based partitioning can still give
  // better partitions.
