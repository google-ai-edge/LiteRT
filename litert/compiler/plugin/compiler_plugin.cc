...
Expected<void> ApplyPluginWithPartition(CompilerPlugin& compiler_plugin,
  ...
  LITERT_LOG(LITERT_INFO, "ApplyPluginWithPartition: Calling compiler_plugin.Compile");
  auto compiled_result = compiler_plugin.Compile(&sliced_model, soc_model);
  LITERT_LOG(LITERT_INFO, "ApplyPluginWithPartition: compiler_plugin.Compile returned");
  if (!compiled_result) {
    return compiled_result.Error();
  }

  LITERT_LOG(LITERT_INFO, "ApplyPluginWithPartition: Moving compiled_result to vector");
  result.compiled_results.push_back(std::move(*compiled_result));
  LITERT_LOG(LITERT_INFO, "ApplyPluginWithPartition: Moved compiled_result to vector");
  ...
  LITERT_LOG(LITERT_INFO, "ApplyPluginWithPartition: Calling NumByteCodeModules");
  auto num_byte_code = stored_result.NumByteCodeModules();
  LITERT_LOG(LITERT_INFO, "ApplyPluginWithPartition: NumByteCodeModules returned");
  if (!num_byte_code) {
