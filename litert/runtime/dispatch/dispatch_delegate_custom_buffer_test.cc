...
TEST(DispatchDelegateCustomBufferTest,
  ...
  // 0. Verify null environment returns error before initialization.
  EXPECT_EQ(LiteRtDispatchInitialize(LrtGetRuntimeContext(), /*env=*/nullptr,
                                     /*options=*/nullptr),
  ...
  // 2. Verify null environment returns error when dispatch is already
  // initialized.
  EXPECT_EQ(LiteRtDispatchInitialize(LrtGetRuntimeContext(), /*env=*/nullptr,
                                     /*options=*/nullptr),
            kLiteRtStatusErrorInvalidArgument);

  // 3. Second environment: env1 was destroyed, so its TensorBufferRegistry was
