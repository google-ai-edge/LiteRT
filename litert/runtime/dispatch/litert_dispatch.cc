...
LiteRtStatus LiteRtDispatchInitialize(
  ...
  // Register custom tensor buffer handlers into the provided environment
  // instance. TensorBufferRegistry is scoped to each LiteRtEnvironment, so this
  // must be performed for every environment even if TheApi was already
  // initialized in a previous environment.
