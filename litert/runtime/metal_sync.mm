...
#include "third_party/odml/litert/litert/runtime/metal_sync.h"
...
#include "third_party/odml/litert/litert/c/litert_tensor_buffer_types.h"
#include "third_party/odml/litert/litert/cc/litert_macros.h"
#include "third_party/odml/litert/litert/runtime/litert_gpu_util.h"
#include "third_party/tensorflow/lite/delegates/gpu/common/shape.h"
#include "third_party/tensorflow/lite/delegates/gpu/common/task/tensor_desc.h"
#include "third_party/tensorflow/lite/delegates/gpu/common/tensor.h"
#include "third_party/tensorflow/lite/delegates/gpu/metal/metal_device.h"
#include "third_party/tensorflow/lite/delegates/gpu/metal/metal_spatial_tensor.h"
...
absl::StatusOr<TensorDescriptor> CreateMetalTensorDescriptor(
  ...
  DataType data_type;
  LITERT_RETURN_IF_ERROR(ConvertLiteRtDataTypeToGpuDataType(tensor_type, &data_type, buffer_type).ok());
...
LiteRtStatus LiteRtMetalMemoryCreate(tflite::gpu::metal::MetalDevice* metal_device,
                                     const LiteRtRankedTensorType* tensor_type,
                                     LiteRtTensorBufferType buffer_type, size_t bytes,
                                     id<MTLBuffer>* metal_memory) {
  ...
  LITERT_RETURN_IF_ERROR(
      tflite::gpu::metal::CreateTensor(metal_device->device(), *tensor_desc, &tensor_memory).ok(),
      kLiteRtStatusErrorRuntimeFailure);

  *metal_memory = tensor_memory.GetBufferHandle();
...
LiteRtStatus LiteRtMetalGpuMemoryDownloadImpl(tflite::gpu::metal::MetalSpatialTensor& metal_tensor,
  ...
  LITERT_RETURN_IF_ERROR(metal_tensor.ToDescriptor(&desc, metal_device->device()).ok(),
                         kLiteRtStatusErrorRuntimeFailure);
...
LiteRtStatus LiteRtMetalMemoryDownload(tflite::gpu::metal::MetalDevice* metal_device,
                                       id<MTLBuffer> metal_memory,
                                       const LiteRtRankedTensorType* tensor_type,
                                       LiteRtTensorBufferType buffer_type, size_t bytes,
                                       void* data) {
  ...
  LITERT_RETURN_IF_ERROR(
      tflite::gpu::metal::CreateTensorSharedBuffer(metal_memory, *tensor_desc, metal_tensor.get())
          .ok(),
      kLiteRtStatusErrorRuntimeFailure);
...
LiteRtStatus LiteRtMetalGpuMemoryUploadImpl(tflite::gpu::metal::MetalSpatialTensor& metal_tensor,
  ...
  LITERT_RETURN_IF_ERROR(
      metal_tensor.UploadDescriptorData(descriptor_with_data, metal_device->device()).ok(),
      kLiteRtStatusErrorRuntimeFailure);
  return kLiteRtStatusOk;
}

LiteRtStatus LiteRtMetalMemoryUpload(GpuEnvironment* gpu_env, id<MTLBuffer> metal_memory,
                                     const LiteRtRankedTensorType* tensor_type,
                                     LiteRtTensorBufferType buffer_type, size_t bytes,
                                     const void* data) {
  auto tensor_desc = CreateMetalTensorDescriptor(tensor_type, buffer_type);
  ...
  LITERT_RETURN_IF_ERROR(tflite::gpu::metal::CreateTensorSharedBuffer(metal_memory, *tensor_desc,
                                                                      metal_spatial_tensor.get())
                             .ok(),
                         kLiteRtStatusErrorRuntimeFailure);
