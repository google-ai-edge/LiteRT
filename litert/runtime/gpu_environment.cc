...
GpuEnvironment::Create(const LiteRtEnvironmentOptionsT& options) {
  auto instance = std::make_unique<GpuEnvironment>();
  LITERT_ASSIGN_OR_RETURN(auto created_options, instance->Initialize(options));
  LITERT_LOG(LITERT_INFO, "Created LiteRT GpuEnvironment.");
  return std::make_pair(std::move(instance), std::move(created_options));
}

Expected<std::vector<LiteRtEnvOption>> GpuEnvironment::Initialize(
    const LiteRtEnvironmentOptionsT& environment_options) {
#if LITERT_HAS_OPENCL_SUPPORT
  // Set up OpenCL.
