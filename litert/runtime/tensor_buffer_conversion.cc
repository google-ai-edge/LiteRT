...
Expected<void> CopyGlToCl(GlBuffer& src, OpenClMemory& dest) {
  ...
  std::memcpy(host_dest, host_src, src.size_bytes());
...
Expected<LiteRtTensorBufferT::Ptr> TensorBufferConvertGlToCl(
    LiteRtEnvironment env, LiteRtTensorBufferT& tensor_buffer_gl) {
  // Create a new CL tensor buffer.
  LITERT_ASSIGN_OR_RETURN(
      LiteRtTensorBufferT::Ptr tensor_buffer_cl,
      LiteRtTensorBufferT::CreateManaged(
          env, kLiteRtTensorBufferTypeOpenClBufferPacked,
          tensor_buffer_gl.tensor_type(), tensor_buffer_gl.buffer_size()));
  LITERT_ASSIGN_OR_RETURN(OpenClMemory * cl_buffer,
                          tensor_buffer_cl->GetOpenClMemory());
...
Expected<LiteRtTensorBufferT::Ptr> TensorBufferConvertGlToAhwb(
  ...
  LITERT_ASSIGN_OR_RETURN(
      ...
      LiteRtTensorBufferT::CreateManaged(env, kLiteRtTensorBufferTypeAhwb,
...
Expected<void> CopyHostToGl(void* host_src, GlBuffer& dest) {
  LITERT_ASSIGN_OR_RETURN(void* host_dest, dest.Lock<char>());
  std::memcpy(host_dest, host_src, dest.size_bytes());
  return {};
}

Expected<LiteRtTensorBufferT::Ptr> TensorBufferConvertHostToGl(
    LiteRtEnvironment env, LiteRtTensorBufferT& tensor_buffer_host) {
  // Create a new GL tensor buffer.
  LITERT_ASSIGN_OR_RETURN(
      LiteRtTensorBufferT::Ptr tensor_buffer_gl,
      LiteRtTensorBufferT::CreateManaged(env, kLiteRtTensorBufferTypeGlBuffer,
...
#if LITERT_HAS_OPENGL_SUPPORT
      return TensorBufferConvertHostToGl(env, tensor_buffer);
#else
      return Unexpected(
...
#if LITERT_HAS_OPENGL_SUPPORT && LITERT_HAS_AHWB_SUPPORT
      return TensorBufferConvertGlToAhwb(env, tensor_buffer);
#else
      return Unexpected(
          kLiteRtStatusErrorRuntimeFailure,
          absl::StrFormat("This buffer conversion is not supported: %s -> %s",
...
#if LITERT_HAS_OPENGL_SUPPORT && LITERT_HAS_OPENCL_SUPPORT
      return TensorBufferConvertGlToCl(env, tensor_buffer);
#else
      return Unexpected(
          kLiteRtStatusErrorRuntimeFailure,
          absl::StrFormat("This buffer conversion is not supported: %s -> %s",
...
Expected<LiteRtTensorBufferT::Ptr> TensorBufferConvertTo(
  ...
  switch (tensor_buffer.buffer_type()) {
    case kLiteRtTensorBufferTypeHostMemory:
      return TensorBufferConvertHostTo(env, buffer_type, tensor_buffer);
    case kLiteRtTensorBufferTypeGlBuffer:
      return TensorBufferConvertGlTo(env, buffer_type, tensor_buffer);
    default:
      return Unexpected(
          kLiteRtStatusErrorRuntimeFailure,
          absl::StrFormat("This buffer conversion is not supported: %s -> %s",
