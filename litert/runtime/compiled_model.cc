  ...
  if (requirements) {
    ...
    for (auto& type : supported_types) {
      if (type == buffer->buffer_type()) {
        ...
        // Buffer needs to be duplicated so that the underlying buffer is not
        // destroyed if user destroys the tensor buffer. This duplication allows
        // shared ownership across the C API boundary. Note that within the
        // internal C++ runtime shared ownership should be handled by
        // LiteRtTensorBufferSharedPtr (as used below), not Duplicate().
        buffer->Duplicate();

        auto duplicated_buffer =
            litert::internal::CreateLiteRtTensorBufferSharedPtr(buffer);
