...
LiteRtStatus ExternalLiteRtBufferContext::RegisterTensorBuffer(
  ...
  auto [iter, inserted] =
      tensor_buffers_.try_emplace(tensor, std::move(tensor_buffer));
  LITERT_RETURN_IF_ERROR(inserted)
      << "Failed to register tensor buffer: Already inserted";
  return kLiteRtStatusOk;
}

litert::Expected<LiteRtTensorBufferSharedPtr>
...
ExternalLiteRtBufferContext::CreateBufferForTensor(
  ...
  return CreateLiteRtTensorBufferSharedPtr(litert_tensor_buffer);
}
