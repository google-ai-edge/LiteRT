...
bool IsSpaceToDepthSupported(
  ...
  if (!params) {
    *error = "Missing TfLiteSpaceToDepthParams.";
    return false;
  }
  // from CheckGpuDelegateCompatibility()
  if (params->block_size == 1) {
    *error = "SPACE_TO_DEPTH block_size = 1 is a no-op.";
    return false;
  }
  if (params->block_size < 1) {
    *error = "SPACE_TO_DEPTH block_size must be > 1.";
    return false;
  }
  return true;
}
