...
TEST_F(ReluOpTest, SupportsFp32) {
...
TEST_F(ReluOpTest, SupportsLeakyRelu) {
  StubContextBuilder context_builder;
  ...
  params.alpha = 0.1f;
  context_builder.SetOp(kTfLiteBuiltinLeakyRelu, 2, &params, {in}, {out});
  TfLiteContext* context = context_builder.Build();
  ASSERT_THAT(context, NotNull());
  EXPECT_THAT(GetSupportedNodes(context, kDefaultOptions), ElementsAre(0));
}

TEST_F(ReluOpTest, RejectsWrongNumberOfInputs) {
  StubContextBuilder context_builder;
  ...
  const int out = context_builder.AddTensor(kDefaultDtype, kDefaultOutputDims);
  context_builder.SetOp(kTfLiteBuiltinRelu, 2, nullptr, {in0, in1}, {out});
  TfLiteContext* context = context_builder.Build();
  ASSERT_THAT(context, NotNull());
  EXPECT_THAT(GetSupportedNodes(context, kDefaultOptions), IsEmpty());
}

TEST_F(ReluOpTest, RejectsWrongNumberOfOutputs) {
  StubContextBuilder context_builder;
  const int in = context_builder.AddTensor(kDefaultDtype, kDefaultInputDims);
  context_builder.SetOp(kTfLiteBuiltinRelu, 2, nullptr, {in}, {});
  TfLiteContext* context = context_builder.Build();
  ASSERT_THAT(context, NotNull());
  EXPECT_THAT(GetSupportedNodes(context, kDefaultOptions), IsEmpty());
}

TEST_F(ReluOpTest, RejectsUnsupportedDType) {
  StubContextBuilder context_builder;
...
TEST_F(ReluOpTest, RejectsConstInput) {
  StubContextBuilder context_builder;
  const int in =
...
TEST_F(ReluOpTest, RejectsWrongVersion) {
  StubContextBuilder context_builder;
  const int in = context_builder.AddTensor(kDefaultDtype, kDefaultInputDims);
  const int out = context_builder.AddTensor(kDefaultDtype, kDefaultOutputDims);
  context_builder.SetOp(kTfLiteBuiltinRelu, 3, nullptr, {in}, {out});
  TfLiteContext* context = context_builder.Build();
  ASSERT_THAT(context, NotNull());
  EXPECT_THAT(GetSupportedNodes(context, kDefaultOptions), IsEmpty());
}

TEST_F(ReluOpTest, RejectsMismatchedInputOutputShapes) {
  StubContextBuilder context_builder;
  const int in = context_builder.AddTensor(kDefaultDtype, kDefaultInputDims);
