...
TEST(NumInputOutputTests, Supports3Inputs1Output) {
  ...
  context_builder.SetOp(kTfLiteBuiltinTransposeConv, /*version=*/1,
                        /*params=*/nullptr, {output_shape, weights, input},
                        {output});
...
TEST(NumInputOutputTests, Supports4Inputs1Output) {
  StubContextBuilder context_builder;
  const int output_shape =
      context_builder.AddConst1dTensor<int>(kTfLiteInt32, kDefaultOutputDims);
  const int weights =
...
TEST(NumInputOutputTests, Rejects2Inputs) {
  ...
  const int output =
      context_builder.AddTensor(kDefaultDtype, kDefaultOutputDims);
  context_builder.SetOp(kTfLiteBuiltinTransposeConv, /*version=*/1,
                        /*params=*/nullptr, {output_shape, weights}, {output});
  TfLiteContext* context = context_builder.Build();
  ASSERT_THAT(context, NotNull());
...
TEST(NumInputOutputTests, Rejects5Inputs) {
  ...
  context_builder.SetOp(kTfLiteBuiltinTransposeConv, /*version=*/1,
                        /*params=*/nullptr,
                        {output_shape, weights, input, bias, extra}, {output});
  TfLiteContext* context = context_builder.Build();
  ASSERT_THAT(context, NotNull());
  EXPECT_THAT(GetSupportedNodes(context, kDefaultOptions), IsEmpty());
}
...
using SupportedDtypeTest = TestWithParam<
    std::tuple</*shape_dtype=*/TfLiteType, /*input_dtype=*/TfLiteType,
               /*weights_dtype=*/TfLiteType, /*bias_dtype=*/TfLiteType,
               /*output_dtype=*/TfLiteType>>;

TEST_P(SupportedDtypeTest, SupportsSupportedDtypes) {
  const auto [shapetype, wdtype, idtype, bdtype, odtype] = GetParam();
  StubContextBuilder context_builder;
  const int output_shape =
      context_builder.AddConst1dTensor<int>(shapetype, kDefaultOutputDims);
...
TEST_P(SupportedDtypeTest, SupportsSupportedDtypesWithoutOptionalBias) {
  const auto [shapetype, wdtype, idtype, _, odtype] = GetParam();
  StubContextBuilder context_builder;
  const int output_shape =
      context_builder.AddConst1dTensor<int>(shapetype, kDefaultOutputDims);
  const int weights = context_builder.AddTensor(wdtype, kDefaultWeightsDims);
...
TEST_P(SupportedDtypeTest, RejectsUnsupportedOutputShape) {
  const auto [_, wdtype, idtype, bdtype, odtype] = GetParam();
  StubContextBuilder context_builder;
  const int output_shape =
      context_builder.AddConst1dTensor<int>(kTfLiteNoType, kDefaultOutputDims);
  const int weights = context_builder.AddTensor(wdtype, kDefaultWeightsDims);
  const int input = context_builder.AddTensor(idtype, kDefaultInputDims);
  const int bias = context_builder.AddTensor(bdtype, kDefaultBiasDims);
  const int output = context_builder.AddTensor(odtype, kDefaultOutputDims);
...
TEST_P(SupportedDtypeTest, RejectsUnsupportedWeights) {
  const auto [shapetype, _, idtype, bdtype, odtype] = GetParam();
  StubContextBuilder context_builder;
  const int output_shape =
      context_builder.AddConst1dTensor<int>(shapetype, kDefaultOutputDims);
  const int weights =
...
TEST_P(SupportedDtypeTest, RejectsUnsupportedInput) {
  const auto [shapetype, wdtype, _, bdtype, odtype] = GetParam();
  StubContextBuilder context_builder;
  const int output_shape =
      context_builder.AddConst1dTensor<int>(shapetype, kDefaultOutputDims);
  const int weights = context_builder.AddTensor(wdtype, kDefaultWeightsDims);
  ...
  const int bias = context_builder.AddTensor(bdtype, kDefaultBiasDims);
  const int output = context_builder.AddTensor(odtype, kDefaultOutputDims);
  context_builder.SetOp(kTfLiteBuiltinTransposeConv,
                        /*version=*/1,
                        /*params=*/nullptr,
                        {output_shape, weights, input, bias}, {output});
  TfLiteContext* context = context_builder.Build();
...
TEST_P(SupportedDtypeTest, RejectsUnsupportedOutput) {
  const auto [shapetype, wdtype, idtype, bdtype, _] = GetParam();
  StubContextBuilder context_builder;
  const int output_shape =
      context_builder.AddConst1dTensor<int>(shapetype, kDefaultOutputDims);
  const int weights = context_builder.AddTensor(wdtype, kDefaultWeightsDims);
...
// Test suite for checking the output shape variable
TEST(OutputShapeTest, Rejects2dOutputShapeTensor) {
  StubContextBuilder context_builder;
  const int output_shape = context_builder.AddTensor(kTfLiteInt32, {4, 1});
  const int input = context_builder.AddTensor(kDefaultDtype, kDefaultInputDims);
  const int weights =
      context_builder.AddTensor(kDefaultDtype, kDefaultWeightsDims);
  const int output =
...
TEST(OutputShapeTest, RejectsOutputShapeMismatch) {
  StubContextBuilder context_builder;
  // length 5 instead of 4
  const int output_shape =
      context_builder.AddConst1dTensor<int>(kTfLiteInt32, {1, 5, 5, 1, 1});
  const int input = context_builder.AddTensor(kDefaultDtype, kDefaultInputDims);
  const int weights =
      context_builder.AddTensor(kDefaultDtype, kDefaultWeightsDims);
  const int output =
...
TEST(OutputShapeTest, RejectsOutputShapeValueMismatch) {
  StubContextBuilder context_builder;
  // Should be 1, 4, 4, 1
  const int output_shape =
      context_builder.AddConst1dTensor<int>(kTfLiteInt32, {1, 5, 4, 1});
  const int input = context_builder.AddTensor(kDefaultDtype, kDefaultInputDims);
  const int weights =
      context_builder.AddTensor(kDefaultDtype, kDefaultWeightsDims);
  const int output =
