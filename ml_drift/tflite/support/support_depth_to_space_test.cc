...
constexpr std::array<int, 4> kInputDims = {1, 2, 2, 4};
constexpr std::array<int, 4> kOutputDims = {1, 4, 4, 1};

class DepthToSpaceOpTest : public ::testing::Test {};

class DepthToSpaceOpSupportsDtypeTest
    : public ::testing::TestWithParam<TfLiteType> {};

TEST_P(DepthToSpaceOpSupportsDtypeTest, SupportsDtype) {
  TfLiteType dtype = GetParam();
  StubContextBuilder context_builder;
  TfLiteDepthToSpaceParams params = {.block_size = 2};
  const int in = context_builder.AddTensor(dtype, kInputDims);
  const int out = context_builder.AddTensor(dtype, kOutputDims);
  context_builder.SetOp(kTfLiteBuiltinDepthToSpace, /*version=*/1, &params,
                        {in}, {out});
...
INSTANTIATE_TEST_SUITE_P(DepthToSpaceOpTest, DepthToSpaceOpSupportsDtypeTest,
...
TEST_F(DepthToSpaceOpTest, RejectsWrongNumberOfInputs) {
  StubContextBuilder context_builder;
  TfLiteDepthToSpaceParams params = {.block_size = 2};
...
TEST_F(DepthToSpaceOpTest, RejectsWrongNumberOfOutputs) {
  StubContextBuilder context_builder;
  TfLiteDepthToSpaceParams params = {.block_size = 2};
  const int in = context_builder.AddTensor(kDefaultDtype, kInputDims);
  const int out = context_builder.AddTensor(kDefaultDtype, kOutputDims);
...
TEST_F(DepthToSpaceOpTest, RejectsUnsupportedInputDtype) {
  StubContextBuilder context_builder;
  TfLiteDepthToSpaceParams params = {.block_size = 2};
...
TEST_F(DepthToSpaceOpTest, RejectsUnsupportedOutputDtype) {
  StubContextBuilder context_builder;
  TfLiteDepthToSpaceParams params = {.block_size = 2};
  const int in = context_builder.AddTensor(kDefaultDtype, kInputDims);
  const int out = context_builder.AddTensor(kTfLiteInt64, kOutputDims);
...
TEST_F(DepthToSpaceOpTest, RejectsConstInput) {
  StubContextBuilder context_builder;
  TfLiteDepthToSpaceParams params = {.block_size = 2};
...
TEST_F(DepthToSpaceOpTest, RejectsBlockSize1) {
  StubContextBuilder context_builder;
  TfLiteDepthToSpaceParams params = {.block_size = 1};
  const int in = context_builder.AddTensor(kDefaultDtype, kInputDims);
  const int out = context_builder.AddTensor(kDefaultDtype, kOutputDims);
  context_builder.SetOp(kTfLiteBuiltinDepthToSpace, /*version=*/1, &params,
                        {in}, {out});
...
TEST_F(DepthToSpaceOpTest, RejectsBlockSize0) {
  StubContextBuilder context_builder;
  TfLiteDepthToSpaceParams params = {.block_size = 0};
  const int in = context_builder.AddTensor(kDefaultDtype, kInputDims);
  const int out = context_builder.AddTensor(kDefaultDtype, kOutputDims);
  context_builder.SetOp(kTfLiteBuiltinDepthToSpace, /*version=*/1, &params,
                        {in}, {out});
...
TEST_F(DepthToSpaceOpTest, RejectsMissingParams) {
  StubContextBuilder context_builder;
  const int in = context_builder.AddTensor(kDefaultDtype, kInputDims);
  const int out = context_builder.AddTensor(kDefaultDtype, kOutputDims);
  context_builder.SetOp(kTfLiteBuiltinDepthToSpace, /*version=*/1,
