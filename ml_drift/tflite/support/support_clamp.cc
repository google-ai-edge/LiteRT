...
bool IsClampSupported(const TfLiteContext* absl_nonnull context,
  ...
  const int max_id = inputs->data[1];
  const int min_id = inputs->data[2];
  ...
  const TfLiteTensor& max = context->tensors[max_id];
  if (!CheckTensorDtype(max, supported_dtypes, "inputs[1]", *error)) {
    return false;
  }
  const TfLiteTensor& min = context->tensors[min_id];
  if (!CheckTensorDtype(min, supported_dtypes, "inputs[2]", *error)) {
    return false;
  }
  ...
  // Check dims. Should either all be equal, or just input/output equal and
  // min/max have one value
  if (!TfLiteIntArrayEqual(input.dims, output.dims)) {
    *error = "Input and output shapes must be equal.";
    return false;
  }
  if (!TfLiteIntArrayEqual(input.dims, max.dims)) {
    // max can have only one value.
    if (max.dims->size != 1) {
      *error = "If max sh != input sh, max should have exactly one dimension.";
      return false;
    }
    if (max.dims->data[0] != 1) {
      *error = "If max sh != input sh, max should have exactly one value.";
      return false;
    }
  }
  if (!TfLiteIntArrayEqual(input.dims, min.dims)) {
    // min can have only one value.
    if (min.dims->size != 1) {
      *error = "If min sh != input sh, min should have exactly one dimension.";
      return false;
    }
    if (min.dims->data[0] != 1) {
      *error = "If min sh != input sh, min should have exactly one value.";
      return false;
    }
  }
  return true;
