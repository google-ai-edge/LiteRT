...
constexpr std::array<int, 3> kDefaultInputDims = {2, 3, 4};
constexpr std::array<int, 3> kDefaultOutputDims = {2, 3, 4};

// Test suite for dequantize ops for different number of I/O tensors.
...
INSTANTIATE_TEST_SUITE_P(
    DequantizeOps, NumInputOutputTest,
...
// Test suite for dequantize ops x supported subject dtypes.
...
TEST_P(SupportedDtypeTest, SupportsSupportedDtypes) {
  ...
  StubContextBuilder context_builder;
  const int a = context_builder.AddQuantizedTensor(idtype, kDefaultInputDims);
...
INSTANTIATE_TEST_SUITE_P(
    DequantizeOps, SupportedDtypeTest,
...
// Test suite for dequantize ops with different dims.
using DimsTest = TestWithParam<TfLiteBuiltinOperator>;

TEST_P(DimsTest, Supports4dInput) {
  const TfLiteBuiltinOperator op = GetParam();
  StubContextBuilder context_builder;
  const int a = context_builder.AddQuantizedTensor(kTfLiteInt8, {1, 2, 3, 4});
...
TEST_P(DimsTest, Rejects5dInput) {
  ...
  StubContextBuilder context_builder;
  const int a =
      context_builder.AddQuantizedTensor(kTfLiteInt8, {1, 1, 2, 3, 4});
  context_builder.SetConst(a);
  const int b = context_builder.AddTensor(kDefaultDtype, {1, 1, 2, 3, 4});
  context_builder.SetOp(op, /*version=*/1, /*params=*/nullptr,
                        /*inputs=*/{a}, /*outputs=*/{b});
  TfLiteContext* context = context_builder.Build();
  ASSERT_TRUE(context != nullptr);
...
TEST_P(DimsTest, Rejects1dInput) {
  const TfLiteBuiltinOperator op = GetParam();
  StubContextBuilder context_builder;
  const int a = context_builder.AddQuantizedTensor(kTfLiteInt8, {1});
  context_builder.SetConst(a);
...
TEST_P(DimsTest, RejectsSparseInput) {
  const TfLiteBuiltinOperator op = GetParam();
  StubContextBuilder context_builder;
  const int a =
      context_builder.AddQuantizedTensor(kTfLiteInt8, kDefaultInputDims);
  context_builder.SetConst(a);
  context_builder.SetSparsity(a);
  const int b = context_builder.AddTensor(kDefaultDtype, kDefaultOutputDims);
  context_builder.SetOp(op, /*version=*/1, /*params=*/nullptr,
                        /*inputs=*/{a}, /*outputs=*/{b});
  TfLiteContext* context = context_builder.Build();
...
// Test suite for dequantize ops with quantization parameters.
using QuantizationParamsTest = TestWithParam<TfLiteBuiltinOperator>;

TEST_P(QuantizationParamsTest, RejectsInputWithoutQuantizationParams) {
  const TfLiteBuiltinOperator op = GetParam();
  StubContextBuilder context_builder;
  const int a = context_builder.AddTensor(kTfLiteBool, kDefaultInputDims);
  context_builder.SetConst(a);
  const int b = context_builder.AddTensor(kDefaultDtype, kDefaultOutputDims);
  context_builder.SetOp(op, /*version=*/1, /*params=*/nullptr,
                        /*inputs=*/{a}, /*outputs=*/{b});
...
TEST_P(QuantizationParamsTest, RejectsInputWithUnsupportedScaleSize) {
  const TfLiteBuiltinOperator op = GetParam();
  StubContextBuilder context_builder;
  const int a = context_builder.AddQuantizedTensor(
      kTfLiteInt8, kDefaultInputDims, /*scale_size=*/2);
  const int b = context_builder.AddTensor(kDefaultDtype, kDefaultOutputDims);
  context_builder.SetOp(op, /*version=*/1, /*params=*/nullptr,
                        /*inputs=*/{a}, /*outputs=*/{b});
  TfLiteContext* context = context_builder.Build();
...
INSTANTIATE_TEST_SUITE_P(
    DequantizeOps, QuantizationParamsTest,
    ValuesIn<TfLiteBuiltinOperator>({
        // clang-format off
        // go/keep-sorted start
        kTfLiteBuiltinDequantize,
