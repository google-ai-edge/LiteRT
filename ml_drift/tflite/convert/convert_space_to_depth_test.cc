...
class ConvertSpaceToDepthTest : public ::testing::TestWithParam<TfLiteType> {
 protected:
  void SetUp() override {
...
TEST_P(ConvertSpaceToDepthTest, SpaceToDepth) {
  SingleOpInterpreterBuilder model(kTfLiteBuiltinSpaceToDepth, /*version=*/1);
  model.AddInput(GetParam(), {1, 2, 2, 1});
  model.AddOutput(GetParam(), {1, 1, 1, 4});

  TfLiteSpaceToDepthParams* params =
      reinterpret_cast<TfLiteSpaceToDepthParams*>(
          malloc(sizeof(TfLiteSpaceToDepthParams)));
  params->block_size = 2;
  ...
  EXPECT_EQ(op->name, "space_to_depth");
  ...
  ASSERT_TRUE(attr);
  EXPECT_EQ(attr->block_size, 2);

  ASSERT_EQ(interpreter->AllocateTensors(), kTfLiteOk);
  ASSERT_EQ(interpreter->Invoke(), kTfLiteOk);
}

INSTANTIATE_TEST_SUITE_P(ConvertSpaceToDepthTest, ConvertSpaceToDepthTest,
