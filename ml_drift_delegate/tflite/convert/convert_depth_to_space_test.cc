...
class ConvertDepthToSpaceTest : public ::testing::TestWithParam<TfLiteType> {
 protected:
  void SetUp() override {
...
TEST_P(ConvertDepthToSpaceTest, DepthToSpace) {
  SingleOpInterpreterBuilder model(kTfLiteBuiltinDepthToSpace, /*version=*/1);
  model.AddInput(GetParam(), {1, 1, 1, 4});
  model.AddOutput(GetParam(), {1, 2, 2, 1});

  TfLiteDepthToSpaceParams* params =
      reinterpret_cast<TfLiteDepthToSpaceParams*>(
          malloc(sizeof(TfLiteDepthToSpaceParams)));
  params->block_size = 2;
...
INSTANTIATE_TEST_SUITE_P(ConvertDepthToSpaceTest, ConvertDepthToSpaceTest,
