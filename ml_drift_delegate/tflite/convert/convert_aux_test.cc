...
TEST(ConvertAuxTest, AddConstInputInt32ScalarLayout) {
  ...
  const auto* t = std::get_if<::ml_drift::TensorInt32>(&attr->tensor);
  ASSERT_TRUE(t);
  // The payload has to carry the same shape as the IrTensor: it is what the
  // constant is uploaded from, so a disagreement means the data is laid out
  // along different axes than the tensor consuming it claims.
