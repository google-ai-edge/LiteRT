...
// Test suite for cumsum ops x supported version.
...
TEST_P(SupportedVersionTest, Supports) {
  ...
  context_builder.SetOp(kTfLiteBuiltinCumsum, GetParam().version,
                        /*params=*/nullptr,
                        /*inputs=*/{input, axis},
                        /*outputs=*/{output});
...
INSTANTIATE_TEST_SUITE_P(
    cumsumOps, SupportedVersionTest,
...
// Test suite for cumsum ops x unsupported version.
...
INSTANTIATE_TEST_SUITE_P(
    cumsumOps, UnsupportedVersionTest,
    ValuesIn<VersionTestCase>({
        {0},  // min-1
        {2},  // max+1
    }),
...
// Tests for cumsum ops for different number of I/O tensors.
...
// Test suite for cumsum op x supported subject dtypes.
...
TEST_P(SupportedDtypeTest, SupportsSupportedDtypes) {
  ...
  context_builder.SetOp(kTfLiteBuiltinCumsum,
                        /*version=*/1,
                        /*params=*/nullptr,
                        /*inputs=*/{input, axis},
                        /*outputs=*/{output});
...
TEST_P(SupportedDtypeTest, RejectsUnsupportedAxisDtype) {
  StubContextBuilder context_builder;
  const int input = context_builder.AddTensor(GetParam(), kDefaultInputDims);
...
class CumsumDimsTest : public testing::Test {};
TEST_F(CumsumDimsTest, SupportsNegIndices) {
  StubContextBuilder context_builder;
  const int input = context_builder.AddTensor(kDefaultDtype, kDefaultInputDims);
...
TEST_F(CumsumDimsTest, RejectsTooSmallNegAxis) {
  StubContextBuilder context_builder;
  const int input = context_builder.AddTensor(kDefaultDtype, kDefaultInputDims);
...
TEST_F(CumsumDimsTest, RejectsTooLargeAxis) {
  StubContextBuilder context_builder;
  const int input = context_builder.AddTensor(kDefaultDtype, kDefaultInputDims);
  const int axis = context_builder.AddConst1dTensor<int>(kTfLiteInt32, {4});
  const int output =
