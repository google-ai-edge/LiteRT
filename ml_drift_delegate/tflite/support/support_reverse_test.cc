...
// Test suite for reverse logical ops x supported version.
...
TEST_P(SupportedVersionTest, Supports) {
  ...
  const int axis = context_builder.AddConst1dTensor<int>(kTfLiteInt32, {3});
  const int output =
      context_builder.AddTensor(kDefaultDtype, kDefaultOutputDims);
  context_builder.SetOp(kTfLiteBuiltinReverseV2, GetParam().version,
                        /*params=*/nullptr,
                        /*inputs=*/{input, axis},
                        /*outputs=*/{output});
...
INSTANTIATE_TEST_SUITE_P(
    reverseOps, SupportedVersionTest,
...
// Test suite for reverse logical ops x unsupported version.
...
INSTANTIATE_TEST_SUITE_P(
    reverseOps, UnsupportedVersionTest,
    ValuesIn<VersionTestCase>({
        {0},  // min-1
        {2},  // max+1
    }),
...
// Tests for reverse ops for different number of I/O tensors.
TEST_F(NumInputOutputTest, Supports2Inputs1Output) {
  ...
  const int output =
      context_builder.AddTensor(kDefaultDtype, kDefaultOutputDims);
  context_builder.SetOp(kTfLiteBuiltinReverseV2, /*version=*/1,
                        /*params=*/nullptr,
                        /*inputs=*/{input, axis},
                        /*outputs=*/{output});
  TfLiteContext* context = context_builder.Build();
...
// Test suite for reverse op x supported subject dtypes.
...
TEST_P(SupportedDtypeTest, RejectsUnsupportedAxis) {
  StubContextBuilder context_builder;
  const int input = context_builder.AddTensor(GetParam(), kDefaultInputDims);
  const int axis = context_builder.AddConst1dTensor<int>(kTfLiteFloat32, {3});
  const int output = context_builder.AddTensor(GetParam(), kDefaultOutputDims);
  context_builder.SetOp(kTfLiteBuiltinReverseV2, /*version=*/1,
                        /*params=*/nullptr,
                        /*inputs=*/{input, axis},
...
TEST_F(DimsTest, RejectsDiffInputOutputDims) {
  StubContextBuilder context_builder;
  const int axis = context_builder.AddConst1dTensor<int>(kTfLiteInt32, {2});
  const int input = context_builder.AddTensor(kDefaultDtype, {2, 3, 4});
  const int output = context_builder.AddTensor(kDefaultDtype, {2, 3, 4, 1});
  context_builder.SetOp(kTfLiteBuiltinReverseV2, /*version=*/1,
                        /*params=*/nullptr,
                        /*inputs=*/{input, axis},
                        /*outputs=*/{output});
...
class ReverseDimsTest : public testing::Test {};
TEST_F(ReverseDimsTest, SupportsNegIndices) {
  StubContextBuilder context_builder;
  const int input = context_builder.AddTensor(kDefaultDtype, kDefaultInputDims);
  const int axis = context_builder.AddConst1dTensor<int>(kTfLiteInt32, {-4});
  const int output =
      context_builder.AddTensor(kDefaultDtype, kDefaultOutputDims);
  context_builder.SetOp(kTfLiteBuiltinReverseV2, /*version=*/1,
                        /*params=*/nullptr,
...
TEST_F(ReverseDimsTest, RejectsTooSmallNegAxis) {
  StubContextBuilder context_builder;
  const int input = context_builder.AddTensor(kDefaultDtype, kDefaultInputDims);
...
TEST_F(ReverseDimsTest, RejectsTooLargeAxis) {
  StubContextBuilder context_builder;
  const int input = context_builder.AddTensor(kDefaultDtype, kDefaultInputDims);
  const int axis = context_builder.AddConst1dTensor<int>(kTfLiteInt32, {4});
  const int output =
