...
bool HasEmptyTensor(const TfLiteContext* absl_nonnull context,
                    const TfLiteNode* absl_nonnull node) {
  auto has_empty = [&](const TfLiteIntArray* tensor_indices) {
    if (!tensor_indices) return false;
    for (int i = 0; i < tensor_indices->size; ++i) {
      const int tensor_id = tensor_indices->data[i];
  ...
  return has_empty(node->inputs) || has_empty(node->outputs);
}
