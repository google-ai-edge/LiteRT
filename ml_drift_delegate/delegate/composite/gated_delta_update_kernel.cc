...
std::unique_ptr<::ml_drift::GPUOperation> CreateGatedDeltaUpdate(
    const ::ml_drift::OperationDef& definition, int mode,
  ...
  int q_slices = (D_k + 3) / 4;

  op->batch_size_ = B;
  op->num_heads_ = H;
  op->v_slices_ = (D_v + 3) / 4;
  op->k_slices_ = q_slices;

  // WebGPU (WGSL) subgroupBroadcast requires sourceLaneIndex to be a
  // compile-time constant, so dynamic butterfly shuffle (k_slice ^ offset) is
  // only supported on Metal and OpenCL.
  ...
  std::string scratch_decl;
  std::string kv_reduction;
  std::string attn_reduction;

  if (can_use_metal_shuffle) {
    scratch_decl = "";
    kv_reduction =
        "    // Register-only butterfly XOR shuffle reduction (Zero shared "
        "memory, zero barriers)\n";
    for (int offset = 1; offset < q_slices; offset *= 2) {
      absl::StrAppend(&kv_reduction, "    kv_mem += simd_shuffle_xor(kv_mem, ",
                      offset, ");\n");
    }
    attn_reduction = "";
    for (int offset = 1; offset < q_slices; offset *= 2) {
      absl::StrAppend(&attn_reduction,
                      "    my_attn_out += simd_shuffle_xor(my_attn_out, ",
                      offset, ");\n");
    }
    absl::StrAppend(&attn_reduction, R"(    if (k_slice == 0) {
      args.output.Write(my_attn_out, t, h, v_slice, b);
    }
)");
  } else {
    scratch_decl = absl::StrCat("  __local Type scratch[", q_slices, "];\n");
    kv_reduction = absl::StrCat(R"(
    scratch[k_slice] = kv_mem;
    ucl::SyncThreads<WorkGroup, Local>();
    for (int stride = )",
                                q_slices / 2, R"(; stride > 0; stride /= 2) {
      if (k_slice < stride) {
...
$SCRATCH_DECL
  ...
  for (int t = 0; t < seq_len; ++t) {
    ...
    // Load full vec4 K for this thread's k_slice directly - no component extraction needed
...
$ATTN_REDUCTION  }
  ...
  // First pass: replace structural code blocks.
  absl::StrReplaceAll({{"$SCRATCH_DECL", scratch_decl},
                       {"$KV_REDUCTION", kv_reduction},
                       {"$ATTN_REDUCTION", attn_reduction}},
                      &code);

  // Second pass: replace data types and dimensions across the entire shader,
  // including within any substituted blocks above.
  absl::StrReplaceAll(
...
CreateGatedDeltaUpdateFromNode(const ::ml_drift::OperationDef& op_def,
  ...
  return CreateGatedDeltaUpdate(op_def, attr->mode, gpu_info);
}
