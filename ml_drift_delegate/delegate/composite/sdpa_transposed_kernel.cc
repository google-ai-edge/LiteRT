...
int GetOptimalWorkgroupSizeAppleMpp(const ::ml_drift::GpuInfo& gpu_info,
                                    int dst_slices) {
  return 32;
}
...
class SdpaFlashDecodeAppleMppSplitOp : public ::ml_drift::GPUOperation {
 public:
  explicit SdpaFlashDecodeAppleMppSplitOp(int wg_size = 32, int gqa_ratio = 1)
      : wg_size_(wg_size), gqa_ratio_(gqa_ratio) {
    work_group_size_ = ::ml_drift::int3(wg_size_, 1, 1);
  }

  ::ml_drift::int3 GetGridSize() const override {
    const int grid_x = dst_[0]->Width() * dst_[0]->Batch() * wg_size_;
    const int grid_y = (dst_[0]->Height() / gqa_ratio_) * dst_[0]->Depth();
    const int grid_z = 1;
    return ::ml_drift::int3(grid_x, grid_y, grid_z);
  }
...
std::unique_ptr<::ml_drift::GPUOperation>
CreateSdpaFlashDecodeAppleMppSplitKernel(
    const ::ml_drift::GpuInfo& gpu_info,
    const ::ml_drift::OperationDef& definition, bool has_mask, bool has_param,
    int src_end_ch_index, int k_cache_slices, int num_splits,
    int tokens_per_split, std::optional<float> softcap, int gqa_ratio) {
  const int dst_channels = definition.dst_tensors[0].GetBHWCShape().c;
  const int dst_slices = (dst_channels + 3) / 4;
  const int wg_size = GetOptimalWorkgroupSizeAppleMpp(gpu_info, dst_slices);
  const int s_iters = std::max(1, (dst_slices + wg_size - 1) / wg_size);

  auto op =
      std::make_unique<SdpaFlashDecodeAppleMppSplitOp>(wg_size, gqa_ratio);
  op->AddSrcTensor("q", definition.src_tensors[0]);
  op->AddSrcTensor("k", definition.src_tensors[1]);
  op->AddSrcTensor("v", definition.src_tensors[2]);
  if (has_mask) {
  ...
  code += "#define S_ITERS " + std::to_string(s_iters) + "\n";
  code += "#define Q_SLICES " + std::to_string(dst_slices) + "\n";
  code += "#define GQA_RATIO " + std::to_string(gqa_ratio) + "\n";
  code += "MAIN_FUNCTION($0) {\n";
  code += "  int w_idx = ucl::GetGroupId<0>();\n";
  code += "  int split_idx = w_idx % args.num_splits;\n";
  code += "  int X = w_idx / args.num_splits;\n";
  ...
  code +=
      "  if (w_idx >= args.partial_out.Width() || Y >= "
      "(args.partial_out.Height() / GQA_RATIO)) return;\n";
  ...
  code += R"(
  int split_start = split_idx * args.tokens_per_split;
  int split_end = min(active_kv_len, split_start + args.tokens_per_split);

  if (split_start >= active_kv_len) {
    if (tid == 0) {
      for (int qh = 0; qh < GQA_RATIO; ++qh) {
        int q_head_idx = Y * GQA_RATIO + qh;
        args.partial_meta::type meta_val = ucl::Init<args.partial_meta::type>(0.0f);
        meta_val.x = ucl::Convert<args.partial_meta::scalar_type>(-10000.0f);
        meta_val.y = ucl::Convert<args.partial_meta::scalar_type>(0.0f);
        args.partial_meta.Write(meta_val, w_idx, q_head_idx, 0);
      }
    }
    return;
  }
  ...
  __local args.q::type shared_q[GQA_RATIO][Q_SLICES];
  __local float shared_scores[GQA_RATIO][WG_SIZE];

  for (int qh = 0; qh < GQA_RATIO; ++qh) {
    int q_head_idx = Y * GQA_RATIO + qh;
    for (int s = tid; s < k_i_slices; s += WG_SIZE) {
      shared_q[qh][s] = args.q.Read(X, q_head_idx, s);
    }
  }
  ucl::SyncThreads<WorkGroup, Local>();

  float max_score[GQA_RATIO];
  float sum_exp[GQA_RATIO];
  args.partial_out::type out_val[GQA_RATIO][S_ITERS];
  for (int qh = 0; qh < GQA_RATIO; ++qh) {
    max_score[qh] = -10000.0f;
    sum_exp[qh] = 0.0f;
    for (int s_iter = 0; s_iter < S_ITERS; ++s_iter) {
      out_val[qh][s_iter] = ucl::Init<args.partial_out::type>(ucl::Convert<args.partial_out::scalar_type>(0.0f));
    }
  }
    ...
    for (int chunk_start = split_start; chunk_start < split_end; chunk_start += WG_SIZE) {
      ...
      if (k_pos < split_end) {
        ...
        for (int s = 0; s < k_i_slices; ++s) {
          args.k::type k_vec = args.k.Read(base_k_index);
          for (int qh = 0; qh < GQA_RATIO; ++qh) {
            score[qh] += ucl::Convert<float>(dot(shared_q[qh][s], k_vec));
          }
          base_k_index += k_stride;
        }
)";
  ...
  if (softcap.has_value()) {
    code += R"(
        for (int qh = 0; qh < GQA_RATIO; ++qh) {
          score[qh] = tanh(score[qh] / args.softcap) * args.softcap;
        }
)";
  }
  ...
  if (has_mask) {
    if (definition.src_tensors[3].GetDataType() == ::ml_drift::DataType::BOOL) {
      code += R"(
        int mask_slice = k_pos / 4;
        int mask_comp = k_pos % 4;
        bool4 mask_vec = args.mask.Read(X, 0, mask_slice);
        bool valid = mask_vec.x;
        if (mask_comp == 1) valid = mask_vec.y;
        if (mask_comp == 2) valid = mask_vec.z;
        if (mask_comp == 3) valid = mask_vec.w;
      ...
      code += R"(
        ...
        int mask_comp = k_pos % 4;
        args.mask::type mask_vec = args.mask.Read(X, 0, mask_slice);
        float m_elem = ucl::Convert<float>(mask_vec.x);
        if (mask_comp == 1) m_elem = ucl::Convert<float>(mask_vec.y);
        if (mask_comp == 2) m_elem = ucl::Convert<float>(mask_vec.z);
        if (mask_comp == 3) m_elem = ucl::Convert<float>(mask_vec.w);
        for (int qh = 0; qh < GQA_RATIO; ++qh) {
          score[qh] += m_elem;
        }
)";
  ...
  code += R"(
      ...
      for (int qh = 0; qh < GQA_RATIO; ++qh) {
        float chunk_max = -10000.0f;
        for (int i = 0; i < WG_SIZE; ++i) {
          if (chunk_start + i < split_end) {
            chunk_max = max(chunk_max, shared_scores[qh][i]);
          }
        }

        if (chunk_max > -9999.0f) {
          float new_max = max(max_score[qh], chunk_max);
          float alpha_old = exp(max_score[qh] - new_max);

          sum_exp[qh] *= alpha_old;
          for (int s_iter = 0; s_iter < S_ITERS; ++s_iter) {
            out_val[qh][s_iter] *= ucl::Init<args.partial_out::type>(ucl::Convert<args.partial_out::scalar_type>(alpha_old));
          }
          max_score[qh] = new_max;
        }
      }

      for (int i = 0; i < WG_SIZE; ++i) {
        int v_k_pos = chunk_start + i;
        if (v_k_pos >= split_end) break;
        ...
        for (int qh = 0; qh < GQA_RATIO; ++qh) {
          float sc = shared_scores[qh][i];
          if (sc > -9999.0f) {
            float p = exp(sc - max_score[qh]);
            if (tid == 0) {
              sum_exp[qh] += p;
            }
            args.partial_out::type p_vec = ucl::Init<args.partial_out::type>(ucl::Convert<args.partial_out::scalar_type>(p));
            for (int s_iter = 0; s_iter < S_ITERS; ++s_iter) {
              ...
              if (S < dst_slices) {
                int v_index = cur_v_offset + S * 4;
                args.v::type v_elem = args.v.Read(v_index);
                out_val[qh][s_iter] += p_vec * v_elem;
              }
            }
          }
        }
  ...
  for (int qh = 0; qh < GQA_RATIO; ++qh) {
    ...
    for (int s_iter = 0; s_iter < S_ITERS; ++s_iter) {
      int S = tid + s_iter * WG_SIZE;
      if (S < dst_slices) {
        args.partial_out.Write(out_val[qh][s_iter], w_idx, q_head_idx, S);
      }
    }

    if (tid == 0) {
      args.partial_meta::type meta_val = ucl::Init<args.partial_meta::type>(0.0f);
      meta_val.x = ucl::Convert<args.partial_meta::scalar_type>(max_score[qh]);
      meta_val.y = ucl::Convert<args.partial_meta::scalar_type>(sum_exp[qh]);
      args.partial_meta.Write(meta_val, w_idx, q_head_idx, 0);
    }
  }
}
...
std::unique_ptr<::ml_drift::GPUOperation>
CreateSdpaFlashDecodeAppleMppReduceKernel(
    const ::ml_drift::GpuInfo& gpu_info,
    const ::ml_drift::OperationDef& definition, int num_splits) {
  auto op = std::make_unique<::ml_drift::GPUOperation>();
  op->AddSrcTensor("partial_out", definition.src_tensors[0]);
...
absl::Status BuildTwoKernelFlashDecodeAppleMppSdpaGraph(
    const SdpaInputs& inputs, uint32_t output_id,
    ::ml_drift::GpuModelBuilder* model_builder) {
  const int dst_channels = inputs.q.tensor_desc.GetBHWCShape().c;
  const int dst_slices = (dst_channels + 3) / 4;
  const int wg_size =
      GetOptimalWorkgroupSizeAppleMpp(model_builder->gpu_info(), dst_slices);
  const int max_splits = 32;
  const int target_tokens_per_split = std::max(wg_size * 2, 64);
  const int num_splits =
      std::min(max_splits,
               std::max(1, (inputs.cache_size + target_tokens_per_split - 1) /
                               target_tokens_per_split));
  const int tokens_per_split =
      (inputs.cache_size + num_splits - 1) / num_splits;

  ::ml_drift::BHWC q_shape = inputs.q.tensor_desc.GetBHWCShape();
  ::ml_drift::BHWC k_shape = inputs.k.tensor_desc.GetBHWCShape();
  const int num_q_heads = q_shape.h;
  const int num_kv_heads = std::max(1, k_shape.h);
  const int gqa_ratio = std::max(1, num_q_heads / num_kv_heads);
  ...
  auto split_op = CreateSdpaFlashDecodeAppleMppSplitKernel(
      model_builder->gpu_info(), op1_def, inputs.has_mask(), inputs.has_param(),
      inputs.src_end_ch_index, inputs.k_cache_slices, num_splits,
      tokens_per_split, inputs.softcap, gqa_ratio);
  ...
  model_builder->AddGpuOperation(op1_srcs, {partial_out, partial_meta},
                                 std::move(split_op),
                                 "sdpa_flash_decode_apple_mpp_split");
  ...
  auto reduce_op = CreateSdpaFlashDecodeAppleMppReduceKernel(
      model_builder->gpu_info(), op2_def, num_splits);

  model_builder->AddGpuOperation({partial_out, partial_meta}, dst_handle,
                                 std::move(reduce_op),
                                 "sdpa_flash_decode_apple_mpp_reduce");
...
SdpaImplementationStrategy SelectSdpaStrategy(
  ...
  if (is_decode) {
    if (request_flash_decoding) {
      ...
      if (gpu_info.IsApple() && gpu_info.apple_info.IsMSeries()) {
        return SdpaImplementationStrategy::kTwoKernelFlashDecodeAppleMpp;
      }
  ...
  switch (strategy) {
    ...
    case SdpaImplementationStrategy::kTwoKernelFlashDecode:
      return BuildTwoKernelFlashDecodeSdpaGraph(inputs, output_id,
                                                model_builder);
    case SdpaImplementationStrategy::kTwoKernelFlashDecodeAppleMpp:
      return BuildTwoKernelFlashDecodeAppleMppSdpaGraph(inputs, output_id,
                                                        model_builder);
