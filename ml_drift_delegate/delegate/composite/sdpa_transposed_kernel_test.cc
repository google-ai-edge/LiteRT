...
TEST(SdpaTransposedKernelTest, SelectSdpaStrategy) {
  ::ml_drift::GpuInfo apple_m_gpu;
  apple_m_gpu.vendor = ::ml_drift::GpuVendor::kApple;
  apple_m_gpu.apple_info.gpu_type = ::ml_drift::AppleGpu::kM1;

  ::ml_drift::GpuInfo non_apple_gpu;
  non_apple_gpu.vendor = ::ml_drift::GpuVendor::kQualcomm;

  ::ml_drift::GpuInfo apple_a_gpu;
  apple_a_gpu.vendor = ::ml_drift::GpuVendor::kApple;
  apple_a_gpu.apple_info.gpu_type = ::ml_drift::AppleGpu::kA17Pro;

  // Prefill (non-decode): always composite fallback.
  EXPECT_EQ(SelectSdpaStrategy(apple_m_gpu, /*is_decode=*/false,
  ...
  EXPECT_EQ(SelectSdpaStrategy(apple_m_gpu, /*is_decode=*/true,
                               /*allow_single_kernel=*/true,
                               /*request_flash_decoding=*/false),
            SdpaImplementationStrategy::kCompositeMultiKernelFallback);
  EXPECT_EQ(SelectSdpaStrategy(apple_m_gpu, /*is_decode=*/true,
  ...
  // Two-kernel flash decode: Apple M-series vs others.
  EXPECT_EQ(SelectSdpaStrategy(apple_m_gpu, /*is_decode=*/true,
                               /*allow_single_kernel=*/false,
                               /*request_flash_decoding=*/true),
            SdpaImplementationStrategy::kTwoKernelFlashDecodeAppleMpp);
...
TEST_P(SdpaTransposedKernelExecuteTest, AppleMppDecode) {
  auto status = RunSdpaTransposedTest(
      *exec_env, precision(), storage(), allow_single_kernel_implementation(),
      request_flash_decoding(),
      /*BK=*/2, /*T=*/1, /*S=*/4, /*H=*/8, mask_mode(),
...
TEST_P(SdpaTransposedKernelExecuteTest, AppleMppDecodeLargerDimensions) {
  auto status = RunSdpaTransposedTest(
      *exec_env, precision(), storage(), allow_single_kernel_implementation(),
      request_flash_decoding(),
      /*BK=*/4, /*T=*/1, /*S=*/16, /*H=*/64, mask_mode(),
