...
#ifndef THIRD_PARTY_ODML_LITERT_ML_DRIFT_DELEGATE_SHARED_MEMORY_MANAGER_IR_GRAPH_ADAPTER_H_
#define THIRD_PARTY_ODML_LITERT_ML_DRIFT_DELEGATE_SHARED_MEMORY_MANAGER_IR_GRAPH_ADAPTER_H_
...
// GraphAdapter implementation backed by an ir::IrModel. Holds a reference to a
// caller-owned graph; the graph must outlive this adapter.
//
// ir::IrModel ids are size_t; they are narrowed to uint32_t across the
// GraphAdapter interface, which is safe in practice (ids are small indices).
class IrModelAdapter : public GraphAdapter {
 public:
...
#endif  // THIRD_PARTY_ODML_LITERT_ML_DRIFT_DELEGATE_SHARED_MEMORY_MANAGER_IR_GRAPH_ADAPTER_H_
