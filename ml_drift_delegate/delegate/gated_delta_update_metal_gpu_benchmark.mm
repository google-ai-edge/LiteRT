...
void FillRandom(TfLiteTensor* tensor, float min_val = 0.0f, float max_val = 1.0f) {
...
TEST(GatedDeltaUpdateMetalBenchmark, BenchmarkOp) {
  @autoreleasepool {
    id<MTLDevice> device = MTLCreateSystemDefaultDevice();
    if (!device) {
      GTEST_SKIP() << "Metal is not supported on this device";
    }
    ...
    env_options[0].tag = kLiteRtEnvOptionTagMetalDevice;
    env_options[0].value.type = kLiteRtAnyTypeVoidPtr;
    env_options[0].value.ptr_value = (__bridge void*)device;
    env_options[1].tag = kLiteRtEnvOptionTagMetalCommandQueue;
    env_options[1].value.type = kLiteRtAnyTypeVoidPtr;
    env_options[1].value.ptr_value = (__bridge void*)command_queue;

    LiteRtEnvironment environment = nullptr;
    ASSERT_EQ(LiteRtCreateEnvironment(2, env_options, &environment), kLiteRtStatusOk);

    ASSERT_EQ(LiteRtRegisterTensorBufferHandlers(
                  environment, kLiteRtTensorBufferTypeMetalBufferPacked, LiteRtCreateMetalMemory,
                  LiteRtDestroyMetalMemory, LiteRtLockMetalMemory, LiteRtUnlockMetalMemory,
                  LiteRtClearMetalMemory, LiteRtImportMetalMemory, kLiteRtEnvOptionTagMetalDevice,
                  kLiteRtEnvOptionTagMetalCommandQueue),
              kLiteRtStatusOk);
    ASSERT_EQ(LiteRtRegisterTensorBufferHandlers(
                  environment, kLiteRtTensorBufferTypeMetalBuffer, LiteRtCreateMetalMemory,
                  LiteRtDestroyMetalMemory, LiteRtLockMetalMemory, LiteRtUnlockMetalMemory,
                  LiteRtClearMetalMemory, LiteRtImportMetalMemory, kLiteRtEnvOptionTagMetalDevice,
                  kLiteRtEnvOptionTagMetalCommandQueue),
              kLiteRtStatusOk);
    ...
    resolver.AddCustom("gated_delta_update",
                       litert_torch::gdn_kernels::GetGatedDeltaUpdateRegistration());

    std::unique_ptr<tflite::Interpreter> interpreter;
    ASSERT_EQ(tflite::InterpreterBuilder(*model, resolver)(&interpreter), kTfLiteOk);
    ...
    auto get_tensor_id =
        [&](const void* target_tensor) -> litert::internal::TfLiteTensorIdentifier {
    ...
    auto options = litert::ml_drift::MlDriftMetalDelegateDefaultOptionsPtr();
    options->model_token = "benchmark_metal_token";
    options->serialization_dir = nullptr;
    options->runtime_context = LrtGetRuntimeContext();

    auto delegate = litert::ml_drift::CreateMlDriftMetalDelegate(std::move(options), environment);
    ASSERT_NE(delegate, nullptr);

    ASSERT_EQ(interpreter->ModifyGraphWithDelegate(std::move(delegate)), kTfLiteOk);
    ASSERT_EQ(interpreter->AllocateTensors(), kTfLiteOk);

    // Print shapes
    std::cerr << "\n================ GATED DELTA UPDATE METAL BENCHMARK ================\n";
    std::cerr << "Device: " << [[device name] UTF8String] << "\n";
    for (int i = 0; i < 6; ++i) {
    ...
    for (int i = 6; i < 8; ++i) {
      TfLiteTensor* t = interpreter->tensor(i);
      std::cerr << "Output " << (i - 6) << " (" << (t->name ? t->name : "unnamed") << "): [";
      for (int d = 0; d < t->dims->size; ++d) {
        std::cerr << t->dims->data[d] << (d + 1 < t->dims->size ? ", " : "");
      }
      std::cerr << "]\n";
    ...
    latencies_us.reserve(kBenchmarkIterations);

    for (int i = 0; i < kBenchmarkIterations; ++i) {
      absl::Time start = absl::Now();
      ASSERT_EQ(interpreter->Invoke(), kTfLiteOk);
      absl::Time end = absl::Now();
      double duration = absl::ToDoubleMicroseconds(end - start);
    ...
    double avg_us = sum_us / latencies_us.size();
    double median_us = latencies_us[latencies_us.size() / 2];
    double p90_us = latencies_us[static_cast<size_t>(latencies_us.size() * 0.9)];

    std::cerr << std::fixed << std::setprecision(3);
    std::cerr << "\n--- Metal GPU Latency (" << kBenchmarkIterations << " iterations) ---\n";
    std::cerr << "  Min:    " << min_us << " us (" << min_us / 1000.0 << " ms)\n";
    std::cerr << "  Avg:    " << avg_us << " us (" << avg_us / 1000.0 << " ms)\n";
    std::cerr << "  Median: " << median_us << " us (" << median_us / 1000.0 << " ms)\n";
