...
**LiteRT Quantizer** is a flexible, high-performance post-training
...
Nightly Release    | [![Nightly Release Status Badge](https://github.com/google-ai-edge/LiteRT/actions/workflows/nightly_release.yml/badge.svg?branch=main)](https://github.com/google-ai-edge/LiteRT/actions/workflows/nightly_release.yml) |
...
```bash
pip install litert-quantizer-nightly
```
...
### Python API
```python
from litert_quantizer import quantizer, recipe
...
* **dynamic quantization (recommended)**: weights are quantized while
  activations remain in a float format and are not processed by LiteRT
  Quantizer. The runtime kernel handles the on-the-fly quantization of
...
We include commonly used recipes in [recipe.py](litert_quantizer/recipe.py).
...
Quantizing a model with LiteRT Quantizer follows a structured 7-step lifecycle:
...
**Note on CLI Usage:** The `litert-quantizer` command-line tool executes
**Steps 1, 2, and4** in a single command for recipes that do not require
calibration (Dynamic Range, Weight-Only):

```bash
litert-quantizer --model_file="path/to/model.tflite" \
    --recipe=dynamic_wi8_afp32 \
    --output_dir="/path/to/output"
```
...
[quantize_toy_model.py](litert_quantizer/examples/mnist/quantize_toy_model.py).
...
beyond using a template in [recipe.py](litert_quantizer/recipe.py). For example,
...
in [recipe_manager.py](litert_quantizer/recipe_manager.py).

For advanced usage involving mixed quantization, the following API may be
useful:

* Use `Quantizer:load_quantization_recipe()` in
  [quantizer.py](litert_quantizer/quantizer.py) to load a custom recipe.
* Use `Quantizer:update_quantization_recipe()` in
  [quantizer.py](litert_quantizer/quantizer.py) to extend or override
...
LiteRT Quantizer offers two calibration modes and two calibration APIs:
...
```python
from litert_quantizer import calibrator, quantizer
import numpy as np
