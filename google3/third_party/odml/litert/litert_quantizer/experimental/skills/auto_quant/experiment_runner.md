...
different quantization recipes using LiteRT Quantizer.

Unlike PyTorch graph mode quantization that operates on `nn.Module` objects in
memory, LiteRT Quantizer operates by taking a `.tflite` model, applying a
...
3.  **The Algorithm Axis (Third Search Dimension):** Bit-width and skip lists
    are not the only levers. LiteRT Quantizer ships advanced algorithms (GPTQ, OCTAV,
...
4.  **Recipe Overrides & Resolution Precedence:** In LiteRT Quantizer (`RecipeManager`),
