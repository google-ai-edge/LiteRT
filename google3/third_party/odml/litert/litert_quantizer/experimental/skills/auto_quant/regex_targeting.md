...
# LiteRT Quantizer Regular Expression Targeting & Block Isolation
...
Converters such as `litert-torch` FUSE tensor names from multiple module
scopes into one op name, joined with `;`. LiteRT Quantizer matches a rule's regex anywhere
