 ...
 * Check for the piper to GitHub copybara transformations with the following
   command:
   ```
   /google/data/ro/teams/copybara/copybara copy.bara.sky g3folder_to_gitfolder $(g4 g4d)/.. --folder-dir /tmp/litertq_copybara_out --force
   ```
...
Follow these steps to confirm that all tests pass in the OSS build:
 ...
 2. Switch to the gitfolder (and run all commands from within that folder):
     ...
     default = true' > /tmp/litertq_copybara_out/uv.toml && pushd /tmp/litertq_copybara_out/ && $HOME/.local/bin/uv sync && $HOME/.local/bin/uv run pytest && popd
     ```
