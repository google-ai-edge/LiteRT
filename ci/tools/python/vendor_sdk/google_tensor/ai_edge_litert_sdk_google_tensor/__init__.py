...
def path_to_sdk_libs() -> Optional[pathlib.Path]:
  ...
  # The below return statement is expected to return the path to the `data/`
  # directory in the installed package. The compiler SO file will be located
  # directly within this `data/` directory, without any intermediate
  # subdirectories.
  return get_sdk_path()
...
def get_sdk_path() -> Optional[pathlib.Path]:
  ...
  if not (is_linux and is_x86_architecture):
    raise NotImplementedError(
        "Currently LiteRT NPU AOT for Google Tensor is only supported on Linux"
        " x86 architecture."
    )
  try:
    ...
    print(f"Error determining SDK path: {e!r}", file=sys.stderr)
    return None
