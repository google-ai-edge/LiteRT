...
IS_X86_ARCHITECTURE = platform.machine() in ('x86_64')
  ...
  with tempfile.TemporaryDirectory() as tmpdir:
    ...
    try:
      if tarball_url.endswith('.zip'):
        archive_type = 'zip'
      elif tarball_url.endswith(('.tar.gz', '.tgz', '.gz')):
        archive_type = 'tar.gz'
      else:
        print(
      ...
      if archive_type == 'zip':
        with zipfile.ZipFile(archive_name_local, 'r') as zipf:
          ...
          if not content_dir_found:
            print(
                'ERROR: No files found under'
                f" '{prefix_to_strip}/' in the zip file.",
                file=sys.stderr,
            )
        ...
        with tarfile.open(archive_name_local, 'r:gz') as tar:
          ...
          if members_to_extract:
            ...
            print(
                'ERROR: No files found under'
                f" '{prefix_to_strip}/' in the tarball.",
                file=sys.stderr,
            )
            # An exception is raised here to abort the installation process.
            # If we simply return None, the build process will continue, which
            # is not desired.
            raise SystemExit('Install SDK failed. Aborting installation.')
      ...
      print(f'ERROR: Failed to extract archive: {e!r}', file=sys.stderr)
      return
    except Exception as e:  # pylint: disable=broad-exception-caught
      print(
          f'ERROR: An unexpected error occurred during SDK extraction: {e!r}',
          file=sys.stderr,
      )
      # An exception is raised here to abort the installation process.
      # If we simply return None, the build process will continue, which is not
...
setuptools.setup(
    ...
    # TODO: b/475410468 - Revisit Python version support: 3.8, 3.9, 3.10, 3.12.
    classifiers=[
    ...
    packages=['ai_edge_litert_sdk_google_tensor'],
    package_dir={
        'ai_edge_litert_sdk_google_tensor': 'ai_edge_litert_sdk_google_tensor'
    },
