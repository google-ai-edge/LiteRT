...
#ifndef LITERT_TENSOR_EXAMPLES_GEMMA4_NATIVE_MODEL_GEMMA4_GRAPH_H_
#define LITERT_TENSOR_EXAMPLES_GEMMA4_NATIVE_MODEL_GEMMA4_GRAPH_H_
...
#include "tensor/examples/gemma4/native/model/helpers/bundle_matched_ops.h"
#include "tensor/examples/gemma4/native/model/helpers/transformer.h"
#include "tensor/examples/ops/transformer/transformer_ops.h"
#include "tensor/tensor.h"

namespace litert::tensor::examples::gemma4::native {

template <class... Mixins> struct Gemma4Inputs {
  // Artifact-only opt-in to the bundle CPU delegate's FP32→QD8/INT2 head.
...
template <class... Mixins> struct Gemma4Outputs {
...
Gemma4Outputs<Mixins...> BuildGemma4Graph(const Gemma4Inputs<Mixins...> &inputs,
  ...
  for (int layer_idx = 0; layer_idx < config.num_layers; ++layer_idx) {
    if (active_graph_context) {
      ...
      hidden_states = Tensor<Mixins...>({.name=absl::StrCat("active_hidden_",layer_idx),
                      .type=Type::kFP32,.shape=hidden_states.GetShape()});
    ...
    if (active_graph_context) {
      ...
      per_layer_input = Tensor<Mixins...>({.name=absl::StrCat("active_ple_",layer_idx),
                       .type=Type::kFP32,.shape=per_layer_input.GetShape()});
    ...
    if (active_graph_context) active_graph_context->layers[layer_idx].output = hidden_states;
  ...
  if (active_graph_context) {
    hidden_states = Tensor<Mixins...>({.name="active_final_hidden",.type=Type::kFP32,.shape=hidden_states.GetShape()});
  ...
  Tensor logits =
      inputs.bundle_dynamic_qd8_head
          ? BundleDynamicInt2FullyConnected(final_output, head)
...
#endif // LITERT_TENSOR_EXAMPLES_GEMMA4_NATIVE_MODEL_GEMMA4_GRAPH_H_
