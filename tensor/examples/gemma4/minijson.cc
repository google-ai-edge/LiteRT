...
*/
bool string_parser::next_byte_in_range(
    const std::initializer_list<int> ranges) {
...
static decimal parse_decimal(const char*& p) noexcept {
  ...
  while (is_integer(*p)) {
    if (answer.num_digits < max_digits) {
      answer.digits[answer.num_digits] = static_cast<uint8_t>(*p - '0');
    }
    answer.num_digits++;
    ++p;
  }
  if (*p == '.') {
    ...
    answer.decimal_point = static_cast<int32_t>(first_after_period - p);
  }
  if (answer.num_digits > 0) {
    ...
    answer.decimal_point += static_cast<int32_t>(answer.num_digits);
    answer.num_digits -= static_cast<uint32_t>(trailing_zeros);
  }
  if (answer.num_digits > max_digits) {
...
uint32_t number_of_digits_decimal_left_shift(decimal& h, uint32_t shift) {
  shift &= 63;
  static const uint16_t number_of_digits_decimal_left_shift_table[65] = {
...
static uint64_t round(decimal& h) {
  ...
  uint32_t dp = static_cast<uint32_t>(h.decimal_point);
  uint64_t n = 0;
...
static void decimal_left_shift(decimal& h, uint32_t shift) {
  ...
  int32_t read_index = static_cast<int32_t>(h.num_digits - 1);
  uint32_t write_index = h.num_digits - 1 + num_new_digits;
  uint64_t n = 0;

  while (read_index >= 0) {
    n += static_cast<uint64_t>(h.digits[read_index]) << shift;
    uint64_t quotient = n / 10;
    uint64_t remainder = n - (10 * quotient);
    if (write_index < max_digits) {
      h.digits[write_index] = static_cast<uint8_t>(remainder);
    } else if (remainder > 0) {
      h.truncated = true;
    }
    n = quotient;
  ...
  h.decimal_point += static_cast<int32_t>(num_new_digits);
  trim(h);
}
...
static void decimal_right_shift(decimal& h, uint32_t shift) {
  ...
  h.decimal_point -= static_cast<int32_t>(read_index - 1);
  ...
  while (read_index < h.num_digits) {
    uint8_t new_digit = static_cast<uint8_t>(n >> shift);
...
adjusted_mantissa compute_float(decimal& d) {
  ...
  while (d.decimal_point > 0) {
    uint32_t n = static_cast<uint32_t>(d.decimal_point);
  ...
  while ((minimum_exponent + 1) > exp2) {
    uint32_t n = static_cast<uint32_t>((minimum_exponent + 1) - exp2);
    if (n > max_shift) {
      n = max_shift;
    }
    decimal_right_shift(d, n);
    exp2 += static_cast<int32_t>(n);
  }
  ...
  // to shift back.
  if (mantissa >= (static_cast<uint64_t>(1) << mantissa_size_in_bits)) {
  ...
  if (mantissa <
      (static_cast<uint64_t>(1) << binary::mantissa_explicit_bits())) {
    answer.power2--;
  }
  answer.mantissa =
      mantissa &
      ((static_cast<uint64_t>(1) << binary::mantissa_explicit_bits()) - 1);
  return answer;
}
...
double from_chars(const char* first) noexcept {
  ...
  word |= static_cast<uint64_t>(am.power2)
          << binary_format<double>::mantissa_explicit_bits();
  word = negative ? word | (static_cast<uint64_t>(1)
...
boundaries compute_boundaries(FloatType value) {
  ...
  using bits_type =
      std::conditional_t<kPrecision == 24, std::uint32_t, std::uint64_t>;
...
inline int find_largest_pow10(const std::uint32_t n, std::uint32_t& pow10) {
  ...
  if (n >= 1000000000) {
    pow10 = 1000000000;
    return 10;
  } else if (n >= 100000000) {  // LCOV_EXCL_STOP
    pow10 = 100000000;
    return 9;
  } else if (n >= 10000000) {
    pow10 = 10000000;
