...
#ifndef LITERT_TENSOR_EXAMPLES_GEMMA4_LITERT_TRANSFORMER_H_
#define LITERT_TENSOR_EXAMPLES_GEMMA4_LITERT_TRANSFORMER_H_
...
#include "tensor/examples/gemma4/litert/attention.h"
#include "tensor/examples/gemma4/litert/feed_forward_network.h"
...
TransformerLayerOutput<Mixins...> TransformerLayer(
  ...
  Tensor normed_input =
      PrimitiveRmsNorm(input, pre_attn_norm_scale, eps_tensor);

  AttentionOutput<Mixins...> attn_out =
      Attention(normed_input, attention_mask, cos, sin, key_cache, value_cache,
                ...
                eps_tensor, active_graph_context);
  ...
  if (config.use_post_attn_norm) {
    Tensor post_attn_norm_scale = GetWeight(
        ...
        Type::kFP32, {config.embed_dim});
    attn_output =
        PrimitiveRmsNorm(attn_output, post_attn_norm_scale, eps_tensor);
  }
  ...
  Tensor normed_attn_output =
      PrimitiveRmsNorm(attn_residual, pre_ffn_norm_scale, eps_tensor);
  ...
  if (config.use_post_ffw_norm) {
    Tensor post_ffn_norm_scale = GetWeight(
        ...
        Type::kFP32, {config.embed_dim});
    ffn_output = PrimitiveRmsNorm(ffn_output, post_ffn_norm_scale, eps_tensor);
  }
  ...
  if (config.per_layer_input_dim > 0 && per_layer_input.GetStatus().ok() &&
    ...
    Tensor normed_projected =
        PrimitiveRmsNorm(projected, post_per_layer_input_norm, eps_tensor);
...
#endif  // LITERT_TENSOR_EXAMPLES_GEMMA4_LITERT_TRANSFORMER_H_
