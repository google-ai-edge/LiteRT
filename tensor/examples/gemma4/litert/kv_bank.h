...
// Token-major INT8 KV storage with transactional appends and active views.
#ifndef LITERT_TENSOR_EXAMPLES_GEMMA4_LITERT_ACTIVE_KV_BANK_H_
#define LITERT_TENSOR_EXAMPLES_GEMMA4_LITERT_ACTIVE_KV_BANK_H_
...
// 64 of accessible padding follow each full-capacity allocation;
...
class ActiveKvBank {
 public:
  static absl::StatusOr<ActiveKvBank> Create(std::vector<KvOwnerSpec> specs,
    ...
    if (capacity <= 0 || specs.empty()) {
      return absl::InvalidArgumentError(
          "Positive capacity and owners required");
    }
    ...
    for (size_t i = 0; i < bank.specs_.size(); ++i) {
      const size_t bytes =
          static_cast<size_t>(capacity) * bank.specs_[i].head_dim + 64;
      bank.buffers_[i].keys.Resize(bytes);
      bank.buffers_[i].values.Resize(bytes);
    }
    return bank;
  }
  ...
  absl::StatusOr<absl::Span<const int8_t>> PaddedKeys(int owner, int begin,
                                                      int valid_end,
                                                      int padded_end) const {
    return PaddedView(owner, begin, valid_end, padded_end, false);
  }

  absl::StatusOr<absl::Span<const int8_t>> PaddedValues(int owner, int begin,
                                                        int valid_end,
                                                        int padded_end) const {
 ...
 private:
  ...
  absl::StatusOr<absl::Span<const int8_t>> PaddedView(int owner, int begin,
                                                      int valid_end,
                                                      int padded_end,
                                                      bool value) const {
...
#endif  // LITERT_TENSOR_EXAMPLES_GEMMA4_LITERT_ACTIVE_KV_BANK_H_
