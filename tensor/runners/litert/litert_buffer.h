...
class LitertBuffer : public Buffer {
 public:
  ...
  // Allocates a managed hardware tensor buffer satisfying requirements with
  // shared Environment lifetime.
  static absl::StatusOr<std::shared_ptr<LitertBuffer>> CreateManaged(
      std::shared_ptr<litert::Environment> env,
  ...
  // Wraps an existing WebGPU buffer without copies.
  static absl::StatusOr<std::shared_ptr<LitertBuffer>> CreateFromWebGpuBuffer(
      const litert::Environment& env,
      const litert::RankedTensorType& tensor_type,
      litert::TensorBufferType buffer_type, LiteRtWGPUBuffer wgpu_buffer,
      size_t size_bytes) {
    auto tb_or = litert::TensorBuffer::CreateFromWebGpuBuffer(
        env, tensor_type, buffer_type, wgpu_buffer, size_bytes);
    if (!tb_or) {
      return absl::InternalError(tb_or.Error().Message());
    }
    return std::make_shared<LitertBuffer>(std::move(*tb_or));
