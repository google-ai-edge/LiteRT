...
To maximize execution performance in WebGPU JIT compilers and avoid slow CPU
memory copies, you can declaratively share and stream directly into WebGPU VRAM
buffers.
...
When compiled, the JS runtime automatically maps the eager tensor's
`.getWebGpuBuffer()` to alias the runner's internal graphics execution
buffer:

```javascript
await preprocess.compile(rawImage, scale, offset);

// Retrieve the native browser GPUBuffer instance directly from the
// placeholder!
const gpuBuffer = rawImage.getWebGpuBuffer();
```

#### 3. Stream Directly to VRAM
Inside your frame render loop, stream pixels directly into the shared
`GPUBuffer` and call the JIT compiled runner cleanly. The wrapper automatically
bypasses redundant C++ `setInput` transfers, executing at absolute zero-copy
speeds:
```javascript
...
If a single eager placeholder tensor is shared among multiple JIT compiled
runners (e.g. `preprocess(rawImage)` and `preview(rawImage)`):
*   The placeholder maintains a **Dynamic Multi-Runner Registry**.
*   **Primary Runner**: Automatically executes with **100% zero-copy** bypass
    speeds.
*   **Secondary Runners**: The dynamic aliasing detects the context change and
    **automatically falls back to executing a safe, self-healing GPU-to-GPU
    copy**, guaranteeing 100% mathematical correctness across the entire
    application:

```javascript
// 1. Compile primary runner
await preprocess.compile(rawImage, scale, offset);

// 2. Compile secondary runner (aliases the same placeholder!)
await preview.compile(rawImage, aspect);

// 3. Zero-copy pre-binding handles both dynamically under the hood!
const res1 = await preprocess(rawImage, scale, offset); // Zero-copy fast path!
const res2 = await preview(rawImage, aspect);            // Safe self-healing GPU-to-GPU copy fallback!
```
