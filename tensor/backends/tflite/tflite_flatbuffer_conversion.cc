...
absl::Status ModelFactory::Build() {
  ...
  for (const graph::Operation* operation : execution_plan_) {
    ...
    if (build_info.builtin_code == tflite::BuiltinOperator_SPLIT) {
      // TfLite SPLIT expects [axis, input_tensor].
      // LiteRT Split expects [input_tensor, axis].
      const TensorSerializationInfo& axis_info = tensors_[operation->inputs[1]];
      op.inputs.push_back(axis_info.index);
      const TensorSerializationInfo& input_info =
          tensors_[operation->inputs[0]];
      op.inputs.push_back(input_info.index);
    } else {
